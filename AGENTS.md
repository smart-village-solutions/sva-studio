...
- **Circuit-Breaker nach einer Review-Fixrunde:** Einen weiteren unmittelbar blockierenden, im vereinbarten Scope lokal behebbaren Befund weiterhin mit der kleinsten lokalen Korrektur lösen. Alle anderen Findings führen zu einem einfacheren Entwurf oder zu nicht blockierenden Follow-ups, nie zu einer zusätzlichen Implementierungsschicht im laufenden PR. Mehrere Befunde an derselben Invariante oder Systemgrenze lösen weiterhin die Review- und Fix-Stop-Regel aus `DEVELOPMENT_RULES.md` aus.
