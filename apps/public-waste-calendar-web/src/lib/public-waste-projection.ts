...
const deriveFractionOptions = (entries: readonly PublicWasteCalendarEntry[]): readonly PublicWasteFractionOption[] =>
  Array.from(new Map(entries.map((entry) => [entry.fractionId, { id: entry.fractionId, label: entry.fractionLabel }])).values()).sort(
    (left, right) => left.label.localeCompare(right.label, 'de')
  );
