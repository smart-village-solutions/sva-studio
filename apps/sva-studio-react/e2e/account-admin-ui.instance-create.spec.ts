...
test('instance create flow bootstraps tenant admin structure with selected modules', async ({
  page,
}) => {
  ...
  await page.getByRole('checkbox', { name: /Nachrichten/u }).check();
  await page.getByRole('checkbox', { name: /Veranstaltungen/u }).check();
