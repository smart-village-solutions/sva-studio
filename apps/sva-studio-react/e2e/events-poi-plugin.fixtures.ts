...
export const expectEventOrPoiEditorReady = async (
  page: Page,
  path: '/admin/events/new' | '/admin/poi/new'
) => {
  if (path === '/admin/events/new') {
    await expectPluginPageHeading(
      page,
      /Veranstaltung anlegen|events\.detail\.createTitle|events\.editor\.createTitle/
    );
...
export const expectCreateContentActionReady = async (page: Page) => {
  await expect(
    page.getByRole('link', { name: /Neuer Inhalt|content\.actions\.create/ })
  ).toBeVisible();
};
