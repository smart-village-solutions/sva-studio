...
export const routeUnifiedContentOverview = async (
  page: Page,
  ...
  await page.route('**/api/v1/iam/contents**', async (route) => {
    ...
    await route.fulfill({
      ...
      body: JSON.stringify({
        data: [...mapEventToUnifiedContent(getEvents()), ...mapPoiToUnifiedContent(getPois())],
        pagination: createPagination(getEvents().length + getPois().length),
      }),
    });
  });
...
export const routeEvents = async (route: Route, events: EventRecord[]) => {
  ...
  if (method === 'DELETE') {
    ...
    return route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify({ data: { id: detailMatch[1] } }),
    });
  }
  await route.fulfill({
    status: 404,
