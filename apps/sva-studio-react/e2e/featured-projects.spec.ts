...
test('creates, publishes, reorders and deletes a featured project with multiple images', async ({
  page,
}) => {
  test.slow();
  ...
  await page.route('**/api/v1/mainserver/projects**', async (route) => {
    ...
    const body =
      request.method() === 'GET' ? undefined : (request.postDataJSON() as Record<string, unknown>);
    ...
    if (path.endsWith('/project-1') && request.method() === 'DELETE') {
      ...
      return route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({ data: { id: 'project-1' } }),
      });
    }
    return route.fulfill({
      status: 200,
  ...
  await page
    ...
    .nth(0)
    .fill('https://example.test/one.jpg');
  await page
    .getByLabel(/Alternativtext|projects\.fields\.altText/)
    .nth(0)
    .fill('Brücke');
  await page
    .getByLabel(/Bild-URL|projects\.fields\.imageUrl/)
    .nth(1)
    .fill('https://example.test/two.jpg');
  await page
    .getByLabel(/Alternativtext|projects\.fields\.altText/)
    .nth(1)
    .fill('Baustelle');
  ...
  await expect(
    page.getByRole('textbox', { name: /Erstellen als|projects\.principal\.createAs/ })
  ).toHaveValue('Editor One');
  await page
    .getByRole('button', { name: /Projekt anlegen|projects\.actions\.create/ })
    .last()
    .click();
  ...
  await page.getByRole('tab', { name: /Inhalt|projects\.tabs\.content/ }).click();
  await page
    .getByRole('button', { name: /Bild nach oben|projects\.actions\.moveImageUp/ })
    .nth(1)
    .click();
  await page
    .getByRole('button', { name: /Projekt speichern|projects\.actions\.update/ })
    .last()
    .click();
  await expect
    .poll(() => (project?.images as Array<{ altText: string }> | undefined)?.[0]?.altText)
    .toBe('Baustelle');
  await page.getByRole('button', { name: /Projekt löschen|projects\.actions\.delete/ }).click();
  await page
    .getByRole('alertdialog')
    .getByRole('button', { name: /Projekt löschen|projects\.actions\.delete/ })
    .click();
