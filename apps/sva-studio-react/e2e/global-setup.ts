...
const expectedReadyStatuses = new Set([200, 301, 302, 303, 307, 308]);
...
const waitForAppRoute = async (): Promise<void> => {
  ...
  // Nitro creates the Vite SSR environment after the client asset is reachable.
  // Avoid triggering the first SSR render during that gap.
  await sleep(30_000);
  ...
  for (let attempt = 1; attempt <= 60; attempt += 1) {
    try {
      ...
      if (expectedReadyStatuses.has(response.status)) {
        return;
      }
