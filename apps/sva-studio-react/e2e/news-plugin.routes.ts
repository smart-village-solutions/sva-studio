...
export const fulfillContentRoute = async (route: Route, newsItems: NewsRecord[]) => {
  ...
  if (method === 'GET') return route.fulfill({ status: item ? 200 : 404, contentType: 'application/json', headers: item ? { 'X-SVA-Context-Binding': 'v1.loaded-context' } : undefined, body: JSON.stringify(item ? { data: item, meta: { access: { 'news.update': true, 'news.delete': true, 'content.publish': true, 'content.changeStatus': true } } } : { error: 'not_found' }) });
  if (method === 'PATCH') {
    if (!item) return route.fulfill({ status: 404, contentType: 'application/json', body: JSON.stringify({ error: 'not_found' }) });
