...
const refreshProjectedContentsForMainserverMutation = (
  input: Omit<
...
describe('content list projection', () => {
  ...
  let syncStates: Map<string, TestSyncState>;
  ...
  const buildScopeKey = (
    ...
    [
      ...
      ...(projectionScopeKeyColumnAvailable ? [row.projection_scope_key] : []),
    ].join('::');
  ...
  const applyProjectionFilters = (
    ...
    if (allowedOrganizationIds.length > 0 || visibilityOwnerUserIds.length > 0) {
      rows = rows.filter((row) => {
        if (allowsAllMainserverRows && row.source_system === 'mainserver') {
          return true;
        }
        if (
          allowsCredentialCompatibility &&
          row.source_system === 'mainserver' &&
          row.authorization_mode !== 'exact'
        ) {
          return true;
        }
  ...
  const schemaQueryResult = (text: string): TestQueryResult | null => {
    ...
    const row = text.includes("column_name = 'projection_scope_key'")
      ? { has_projection_scope_key: projectionScopeKeyColumnAvailable }
      : { has_sync_scope_key: syncScopeKeyColumnAvailable };
    return { rows: [row], rowCount: 1 };
  };

  const syncStateReadQueryResult = (
    text: string,
    values: readonly unknown[] | undefined
    ...
    const syncScopeKey = String(queryValue(values, 2));
    const row = storedSyncState(contentType, syncScopeKey);
    const selectedRow =
      row && !syncScopeKeyColumnAvailable ? { ...row, sync_scope_key: undefined } : row;
  ...
  const setSyncState = (contentType: string, syncScopeKey: string, value: TestSyncState): void => {
    syncStates.set(`${contentType}::${syncScopeKey}`, value);
    syncStates.set(contentType, value);
  };
  ...
  const storedSyncState = (
    ...
    const scopedState = syncStates.get(`${contentType}::${syncScopeKey}`);
    return scopedState === undefined ? syncStates.get(contentType) : scopedState;
  };
  ...
  const partialRunningSyncState = (
    ...
    refresh_run_id: String(queryValue(values, firstPayloadIndex)),
    refresh_phase: String(queryValue(values, firstPayloadIndex + 1, 'hot')) as
      'hot' | 'reconciliation',
  ...
  const failedInsertedSyncState = (
    ...
    last_error_code: String(queryValue(values, firstPayloadIndex)),
    last_error_message: String(queryValue(values, firstPayloadIndex + 1)),
  });
  ...
  const succeededSyncState = (
    ...
    const projectedCount = Number(queryValue(values, firstPayloadIndex, 0));
    return {
      ...current,
  ...
  const buildInsertedSyncState = (
    ...
    const current =
      syncStates.get(`${contentType}::${syncScopeKey}`) ?? initialSyncState(syncScopeKey);
    const firstPayloadIndex = syncScopeKeyColumnAvailable ? 3 : 2;
    if (text.includes("'partial_running'")) {
      return partialRunningSyncState(current, values, firstPayloadIndex);
    }
    if (text.includes('last_failed_at') && !text.includes('last_succeeded_at')) {
      return failedInsertedSyncState(current, values, firstPayloadIndex);
    }
    if (text.includes('last_succeeded_at')) {
      return succeededSyncState(current, values, firstPayloadIndex);
    }
    return { ...current, last_started_at: new Date().toISOString() };
  };

  const syncStateInsertQueryResult = (
    text: string,
    values: readonly unknown[] | undefined
  ): TestQueryResult | null => {
    ...
    const contentType = String(queryValue(values, 1));
    const syncScopeKey =
      typeof queryValue(values, 2) === 'string' && syncScopeKeyColumnAvailable
        ? String(queryValue(values, 2))
        : contentType;
    setSyncState(
      contentType,
      syncScopeKey,
      buildInsertedSyncState(text, values, contentType, syncScopeKey)
    );
  ...
  const buildUpdatedSyncState = (
    ...
    const firstPayloadIndex = syncScopeKeyColumnAvailable ? 4 : 3;
    ...
    if (text.includes('refresh_phase = $5') || text.includes('refresh_phase = $4')) {
      return {
        ...current,
        refresh_phase: String(queryValue(values, firstPayloadIndex)) as 'reconciliation',
      };
    }
    return {
      ...current,
      completed_page: Number(queryValue(values, firstPayloadIndex, 0)),
      available_count: Number(queryValue(values, firstPayloadIndex + 1, 0)),
      is_total_final: false,
    };
  };

  const syncStateUpdateQueryResult = (
    text: string,
    values: readonly unknown[] | undefined
  ): TestQueryResult | null => {
    ...
    const contentType = String(queryValue(values, 1));
    const syncScopeKey = syncScopeKeyColumnAvailable
      ? String(queryValue(values, 2, contentType))
      : contentType;
    const current = storedSyncState(contentType, syncScopeKey);
    const refreshRunId = String(queryValue(values, syncScopeKeyColumnAvailable ? 3 : 2));
    if (!current || current.refresh_run_id !== refreshRunId) {
      return { rows: [], rowCount: 0 };
    }
    setSyncState(contentType, syncScopeKey, buildUpdatedSyncState(text, values, current));
    return { rows: [], rowCount: 1 };
  };
  ...
  const scopedCountQueryResult = (
    ...
    const instanceId = String(queryValue(values, 0));
    const contentType = String(queryValue(values, 1));
    const projectionScopeKey = projectionScopeKeyColumnAvailable
      ? String(queryValue(values, 2))
      : null;
    const total = projectionRows.filter(
      (row) =>
        ...
        (!projectionScopeKeyColumnAvailable || row.projection_scope_key === projectionScopeKey)
    ).length;
  ...
  const deleteProjectionQueryResult = (
    ...
    const projectionScopeKey = projectionScopeKeyColumnAvailable
      ? String(queryValue(values, 2))
      : null;
    const entityValue = queryValue(values, projectionScopeKeyColumnAvailable ? 4 : 3, null);
    ...
    const retainedEntityIds = Array.isArray(entityValue)
      ...
      : null;
    projectionRows = projectionRows.filter((row) => {
      const matchingScope =
        row.source_system === 'mainserver' &&
        row.content_type === contentType &&
        (!projectionScopeKeyColumnAvailable || row.projection_scope_key === projectionScopeKey);
  ...
  const insertMappedProjectionRows = (text: string, rows: TestProjectionRow[]): void => {
    if (simulateConcurrentProjectionConflict && rows.length > 0) {
      projectionRows.push({ ...rows[0], id: 'concurrent-row' });
  ...
  const insertProjectionQueryResult = (
    ...
    const rawRows = JSON.parse(String(queryValue(values, 0, '[]'))) as Array<
      Record<string, unknown>
    >;
    projectionInsertPayloadSizes.push(rawRows.length);
  ...
  const createProjectionDatabaseQuery =
    () =>
    async <TRow>(text: string, values?: readonly unknown[]) => {
      const handlers = [
        schemaQueryResult,
        syncStateReadQueryResult,
        distinctContentTypesQueryResult,
        syncStateInsertQueryResult,
        syncStateUpdateQueryResult,
  ...
  const registerReadAuthorizationTests = (): void => {
    ...
    it('persists compatibility state and credential fingerprint without inventing ownership', async () => {
      ...
      expect(response.status).toBe(200);
      await vi.waitFor(() => expect(projectionInsertArgs).not.toBeNull());
    ...
    it('returns sync metadata together with an existing snapshot', async () => {
      ...
      syncStates.set('news.article::de-musterhausen::account-1::org-1::news.article', {
        sync_scope_key: 'de-musterhausen::account-1::org-1::news.article',
        last_started_at: null,
  ...
  registerReadAuthorizationTests();

  const registerRefreshSyncPersistenceTests = (): void => {
    ...
    it('upserts only the latest loaded page during progressive batch refreshes', async () => {
      ...
      expect(response.status).toBe(200);
      expect(projectionInsertPayloadSizes).toEqual([1, 1, 2]);
      expect(projectionRows).toEqual(
        expect.arrayContaining([
  ...
  registerRefreshSyncPersistenceTests();

  const registerMutationGenericItemTests = (): void => {
  ...
  registerMutationGenericItemTests();

  const registerReconciliationSyncPersistenceTests = (): void => {
    ...
    it('answers after the hot page while reconciliation continues', async () => {
      ...
      expect(projectionRows).toEqual([expect.objectContaining({ source_entity_id: 'news-hot-1' })]);
    ...
    it('does not mark a partially persisted progressive refresh as successful when a later page fails', async () => {
      state.listSvaMainserverNews
        ...
        .mockRejectedValueOnce(
          Object.assign(new Error('page 2 failed'), { code: 'upstream_down' })
        );
  ...
  registerReconciliationSyncPersistenceTests();
});
