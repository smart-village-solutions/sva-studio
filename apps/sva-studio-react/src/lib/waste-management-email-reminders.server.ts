...
import { loadWasteEmailReminderSettings } from './waste-management-email-reminder-config.server.js';
...
const countOverdueReminderRefresh = (
  refreshed: boolean
): Readonly<{
...
export const createMaterializeEmailRemindersOperation =
  (deps: WasteOperationRuntimeDeps): WasteManagementOperationRuntime['materializeEmailReminders'] =>
  async (instanceId, input) => {
    ...
    const referenceTime = input.referenceTime
    ...
    const details = await withWasteClient(
      deps,
      instanceId,
      async ({ client, repository, dataSource }) => {
        const unsubscribeSigningSecret =
          reminderSettings.unsubscribeSigningSecret ?? dataSource.databaseUrl;
        ...
        const [
          ...
          repository.listWasteTours({ status: 'published' }),
        ...
        for (const subscription of subscriptions) {
          ...
          for (const item of subscription.items) {
            ...
            const slot = fraction?.reminderConfig.email?.slots.find(
            ...
            for (const location of matchingLocations) {
              for (const pickup of pickupsByLocationFraction.get(
                `${location.id}::${fraction.id}`
              ) ?? []) {
            ...
            for (const [pickupDate, hints] of reminderHintsByDate) {
              ...
              const lookaheadBoundary = addDaysUtc(
                referenceTime,
                reminderConfig.materializationLookaheadDays
              );
...
export const createProcessEmailReminderOutboxOperation =
  ...
  ): WasteManagementOperationRuntime['processEmailReminderOutbox'] =>
  async (instanceId, input) => {
    const startedAt = Date.now();
    ...
    const retryDelayMinutes = Math.max(
      1,
      input.retryDelayMinutes ?? DEFAULT_OUTBOX_RETRY_DELAY_MINUTES
    );
        ...
        try {
          ...
          await reminderRepository.markOutboxEntryFailed({
            ...
            ...(shouldRetry
              ? {
                  retryAt: new Date(
                    referenceTime.getTime() + retryDelayMinutes * 60 * 1000
                  ).toISOString(),
                }
              : {}),
          });
