...
const resolveBoundWasteSchemaName = (
...
export const createInitializeDataSourceOperation =
  (deps: WasteOperationRuntimeDeps): WasteManagementOperationRuntime['initializeDataSource'] =>
  async (instanceId, input) => {
    ...
    const schemaInspection = await withWasteClient(
      deps,
      instanceId,
      async ({ client, dataSource: resolved }) =>
...
export const createApplyMigrationsOperation =
  (deps: WasteOperationRuntimeDeps): WasteManagementOperationRuntime['applyMigrations'] =>
  async (instanceId, input) => {
    const startedAt = Date.now();
...
export const createImportDataOperation =
  (deps: WasteOperationRuntimeDeps): WasteManagementOperationRuntime['importData'] =>
  async (instanceId, input, progressReporter) => {
    const startedAt = Date.now();
    if (input.sourceFormat === 'application/zip') {
      if (
        input.importProfileId !== wasteManagementOperationsContract.importProfileIds.dataPackage
      ) {
        throw new Error('waste_data_package_profile_required');
      }
      return importCanonicalWasteManagementPackage({
        deps,
        instanceId,
    ...
    if (input.sourceFormat === 'application/json') {
      if (
        input.importProfileId ===
          wasteManagementOperationsContract.importProfileIds.locationTourPickupDates ||
        input.importProfileId === wasteManagementOperationsContract.importProfileIds.dataPackage
      ) {
        throw new Error('unsupported_import_source_format:application/json');
      }
      return importCanonicalWasteManagementJson({
        deps,
        instanceId,
...
export const createSeedDataOperation =
  (deps: WasteOperationRuntimeDeps): WasteManagementOperationRuntime['seedData'] =>
  async (instanceId, input) => {
    const startedAt = Date.now();
    const details = await withWasteClient(deps, instanceId, async ({ repository }) => {
...
export const createSyncMainserverOperation =
  (deps: WasteOperationRuntimeDeps): WasteManagementOperationRuntime['syncMainserver'] =>
  async (instanceId, input, progressReporter) => {
    const startedAt = Date.now();
...
export const createSyncWasteTypesOperation =
  (deps: WasteOperationRuntimeDeps): WasteManagementOperationRuntime['syncWasteTypes'] =>
  async (instanceId, input) => {
    const startedAt = Date.now();
...
export const createResetDataOperation =
  (deps: WasteOperationRuntimeDeps): WasteManagementOperationRuntime['resetData'] =>
  async (instanceId, input) => {
    const startedAt = Date.now();
