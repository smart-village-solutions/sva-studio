    ...
    <Card className="space-y-3 p-4">
      <div className="space-y-1">
        ...
        <p className="text-xs text-muted-foreground">
          {t('admin.instances.flow.protocolSubtitle')}
        </p>
      </div>
            ...
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <div className="text-xs text-muted-foreground">
                  {run.mode} • {run.overallStatus} •{' '}
                  {run.requestId ?? t('shell.runtimeHealth.notAvailable')}
                </div>
              </div>
              <Button
                type="button"
                variant="secondary"
                onClick={() => void onLoadProvisioningRun(run.id)}
              >
        ...
        selectedInstance.provisioningRuns.map((run) => (
          <div key={run.id} className="rounded-lg border border-border p-3">
            ...
            <div className="text-muted-foreground">
              {t('admin.instances.detail.runStatus', {
                value: t(INSTANCE_STATUS_LABELS[run.status]),
              })}
            </div>
          </div>
        ))
