...
describe('ContentListPage', () => {
  ...
  it('renders contents, filters them and links to create and edit routes', () => {
    ...
    expect(screen.getByRole('heading', { name: 'Status ändern' })).toBeTruthy();
    expect(screen.getByRole('button', { name: 'Entwurf' })).toBeTruthy();
    expect(screen.getByRole('button', { name: 'Veröffentlicht Aktuell' })).toBeTruthy();
  ...
  it('updates the content list query when readable content types resolve', () => {
    ...
    useContentAccessMock.mockReturnValueOnce(createPendingAccessResult()).mockReturnValue({
      access: {
        state: 'editable',
        canRead: true,
        canCreate: true,
