...
- Unerwartete Fehler in `packages/routing` und `packages/auth` werden über `toJsonErrorResponse()` aus `@sva/sdk/server` als flacher Vertrag serialisiert: `{ "error": "<code>", "message"?: "<öffentliche Diagnose>" }`.
...
- `@sva/routing` hängt serverseitig zusätzlich von `@sva/sdk/server` ab.
