...
Der signierte Vertrag erlaubt zusätzlich ausschließlich das feste Datenbankziel `waste`. Ohne `database`-Feld bleibt der bestehende Studio-Vertrag unverändert; `database: "waste"` bindet Auftrag, Evidenz und Objektpräfix an `sva_waste`. Freie Datenbank-, Host-, Rollen- oder Bucketnamen bleiben ausgeschlossen. Waste-Dumps liegen unter `<umgebung>/waste/`, werden über `sva_waste_migrator` mit `SET ROLE sva_waste_owner` erstellt und verwenden getrennte Backup-/Restore-Secrets. Der Waste-Restorevertrag schreibt ausschließlich nach `sva_waste_restore_drill`, niemals in die aktive Datenbank. Restore-Reconciliation und Privilegproben decken dort `sva_waste_app` sowie die eingeschränkte Rolle `sva_waste_public_app` ab.
