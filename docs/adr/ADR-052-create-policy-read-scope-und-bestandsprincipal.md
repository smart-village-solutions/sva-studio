...
6. Ist der aktive Organisationskontext unvollständig oder widersprüchlich, bleibt die Principal-Auflösung sichtbar `unavailable`; es gibt keinen stillen persönlichen Fallback. Die Liste bleibt lesbar, darf aus der `credentialSource` einer Zeile aber keinen Mutationsprincipal rekonstruieren.
