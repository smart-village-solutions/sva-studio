...
- Neue asynchrone Keycloak-Aufträge tragen die von der App validierten Plugin-OIDC-Anforderungen als versionierten, unveränderlichen Queue-Snapshot. Der getrennte Provisioner verwendet diesen Snapshot unverändert. Für unversionierte Bestandsaufträge gilt während der Einführung einmalig die installierte Anforderungsliste des Workers; unvollständige versionierte Snapshots brechen vor der Ausführung fail-closed ab.
...
- Rechte werden tenantlokal über Rollen und Gruppen modelliert; direkte Nutzerrechte und fachliche `deny`-Permissions sind nicht Teil des Zielmodells.
...
### Fortschreibung 2026-04: Registrierte spezialisierte Content-Views für bestehende Content-Plugins

- Standardisierte Content-Plugins mit CRUD-Oberflächen veröffentlichen ihre produktiven Hauptpfade nicht mehr über freie `plugin.routes`, sondern über `adminResources` mit host-owned Materialisierung.
- Der bestehende Admin-Ressourcenvertrag wird dafür um einen optionalen Spezialisierungsblock `contentUi` erweitert, der einen registrierten plugin-eigenen `contentType` sowie optionale React-Bindings für `list`, `detail` und `editor` beschreibt.
