...
6. `packages/auth-runtime/src/iam-account-management/user-import-sync-handler.ts`
   ...
   - Seed-Lookup, Provider-Update und Persistenz bleiben an dieselbe `instanceId` und dasselbe Keycloak-Subject gebunden. Eine nach den zulässigen Fallbacks weiterhin fehlende E-Mail oder eine fehlgeschlagene erforderliche E-Mail-Reparatur bleibt fail-closed. Fehlende Vor- oder Nachnamen bleiben dagegen optional: Leerzeichen werden vor der IAM-Persistenz als abwesend normalisiert, und eine abgewiesene reine Namensreparatur blockiert weder Account-Upsert noch Membership. Es werden keine Default-Namen nach Keycloak oder IAM geschrieben. Logs enthalten nur einen gehashten Subject-Verweis, Ursachen und Reparaturflags.
