...
6. `packages/auth-runtime/src/iam-account-management/user-import-sync-handler.ts`
   ...
   - Seed-Lookup, Provider-Update und Persistenz bleiben fail-closed an dieselbe `instanceId` und dasselbe Keycloak-Subject gebunden. Eine nach den zulässigen Fallbacks weiterhin fehlende E-Mail geht ohne IAM-Persistenz in die manuelle Prüfung. Fehlende Vor- oder Nachnamen bleiben dagegen optional und blockieren weder Account-Upsert noch Membership; es werden keine Default-Namen nach Keycloak oder IAM geschrieben. Logs enthalten nur einen gehashten Subject-Verweis und Reparaturflags.
