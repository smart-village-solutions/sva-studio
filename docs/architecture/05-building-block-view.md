...
- Inhaltsverwaltung als Core-Element:
  ...
  - die interne Listenprojektion ist entlang ihrer Ownership in Modell, Repository, Source, Sync und Mutation getrennt; Repository-Details gliedern sich zusätzlich in Schema, Sync-State und statische SQL-Verträge, Source-Details in Loader, Binding und Mutationsquelle sowie Sync in Zustandsableitung und Worker. `iam-content-list-projection.server.ts` bleibt die kompatible HTTP- und Autorisierungsfassade
