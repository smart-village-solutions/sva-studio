...
5. Die App-Fassade liest hinter dieser Route ausschließlich aus der persistierten Listenprojektion `iam.content_list_projection`; lokale IAM-Inhalte landen dort triggerbasiert, Mainserver-News, -Events, -POI und -Generic-Items über einen deduplizierten Hintergrund-Sync pro `instanceId` und `contentType`.
...
9. Die Fachlisten und Detailseiten unter `/admin/news`, `/admin/events`, `/admin/poi` und `/admin/generic-items` rufen weiterhin ihre jeweiligen Host-Fassaden auf: `/api/v1/mainserver/news`, `/api/v1/mainserver/events`, `/api/v1/mainserver/poi` oder `/api/v1/mainserver/generic-items`.
