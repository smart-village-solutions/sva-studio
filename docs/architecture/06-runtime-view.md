...
18. Vor dem ersten Provider-Write bestätigt der Identity-Guard die aktuelle credential-versionierte DataProvider-Bindung. Ist sie nur deshalb konfliktbehaftet, weil sämtliche konkurrierenden Benutzer nach Hard Delete nicht mehr vorhanden oder endgültig gelöscht sind, historisiert Studio diese Bindungen unter dem bestehenden DataProvider-Lock, verifiziert die aktuelle Bindung atomar und setzt denselben Request fort. Reversibel inaktive, aktive, organisatorische oder unklare Konkurrenten bleiben ohne Statusänderung fail-closed.
