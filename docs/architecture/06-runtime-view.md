...
2. Nach Registrierung der Plugin-Operations-Handler startet ausschließlich der App-Prozess mit `default`-Lane den revisionsgebundenen Fleet-Reconcile im Hintergrund. Der privilegierte Provisioner lädt denselben Snapshot und seine Handler, startet aber weder diese Schleife noch eine konkurrierende Fleet-Metriksicht. Der Reconcile synchronisiert die aktuellen IAM-Verträge unabhängig davon, ob Aktivierungszeilen geändert wurden. Retrybare Fehler verwenden 1/5/15 Minuten Backoff, ausschließlich degradierte Zustände eine 30-minütige Kontrollprobe; bei gemischten Klassen bestimmt der kürzere retrybare Takt den nächsten Lauf. Unerwartete Abbrüche werden auf `plugin_activation_policy_reconcile_unknown` begrenzt und fail-closed als degradiert eingeordnet.
