...
18. Vor dem ersten Provider-Write bestätigt der Identity-Guard die aktuelle credential-versionierte DataProvider-Bindung. Ist sie nur deshalb konfliktbehaftet, weil sämtliche konkurrierenden Benutzer endgültig gelöscht sind, historisiert Studio diese Bindungen unter dem bestehenden DataProvider-Lock, verifiziert die aktuelle Bindung atomar und setzt denselben Request fort. Bei fehlenden Accounts belegt zusätzlich ein unveränderlicher erfolgreicher `user.deleted`-Eintrag aus dem aktuellen oder archivierten Activity-Log den abgeschlossenen Hard Delete. Reversibel inaktive, aktive, organisatorische, verwaiste oder unklare Konkurrenten bleiben ohne Statusänderung fail-closed.
