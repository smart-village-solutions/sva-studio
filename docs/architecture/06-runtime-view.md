...
4. Beim Abschluss beansprucht ein atomares `pending -> uploaded` mit neuem Fencing-Token genau einen synchronen Verarbeiter. Frische Claims bleiben exklusiv; ein seit zehn Minuten unveränderter `uploaded`-Claim kann durch einen späteren Abschlussaufruf lazy und atomar mit neuem Token übernommen werden, auch wenn die ursprüngliche Upload-URL inzwischen abgelaufen ist. Varianten werden unter claim-isolierten Storage-Keys erzeugt und erst durch die token-geschützte Finalisierung veröffentlicht. Abgelöste Verarbeiter verändern oder löschen keine Objekte eines anderen Claims und entfernen nur ihre eigenen unveröffentlichten Varianten. Wiederholungen eines bereits validierten Uploads bleiben idempotent.
...
6. Eine kurze Finalisierungstransaktion prüft die tatsächlichen Bytes atomar gegen die Quote und persistiert Varianten, Asset-, Session- und Usage-Zustand gemeinsam. Für provisorische Assets sperrt sie außerdem die Content-Speicheroperation und bricht die Veröffentlichung ab, sobald deren Abandonment begonnen hat. Bei Quotenüberschreitung entfernt der Host Original und erzeugte Varianten kompensierend.
...
- Ein fehlender manueller Retry-Vertrag erzeugt keine UI-Aktion. Automatische Runner-Retries erscheinen als `retrying` im bestehenden Job; Cancel-Konflikte antworten stabil mit `409`.
...
2. Nach Registrierung der Plugin-Operations-Handler startet ausschließlich der App-Prozess mit `default`-Lane den revisionsgebundenen Fleet-Reconcile im Hintergrund. Der privilegierte Provisioner lädt denselben Snapshot und seine Handler, startet aber weder diese Schleife noch eine konkurrierende Fleet-Metriksicht. Der Reconcile bindet den vollständigen Runtime-Snapshot als unveränderliche Sicht für alle Instanztransaktionen und synchronisiert die darin enthaltenen IAM-Verträge unabhängig davon, ob Aktivierungszeilen geändert wurden. Wechselt die globale Snapshot-Generation währenddessen, wird das alte Ergebnis nicht publiziert und die aktuelle Revision nach Ende des laufenden Durchgangs gestartet. Retrybare Fehler verwenden 1/5/15 Minuten Backoff, ausschließlich degradierte Zustände eine 30-minütige Kontrollprobe; bei gemischten Klassen bestimmt der kürzere retrybare Takt den nächsten Lauf. Unerwartete Abbrüche werden auf `plugin_activation_policy_reconcile_unknown` begrenzt und fail-closed als degradiert eingeordnet.
...
7. Mutationen gegen Fraktionen, Orte, Abholorte, Touren, Ausweichtermine und Bulk-Zuordnungen laufen immer über dieselbe Host-Fassade und erzeugen zentrale Audit-Events. Neue und duplizierte Touren beginnen als `draft`; Einzel- und Mehrfachänderungen auf `draft`, `published` oder `archived` prüfen Berechtigung, CSRF und die vollständige ID-Menge und schreiben atomar.
...
16. Kontextuelle Aktionen aus Tourenliste, Jahreskalender und Tourformular öffnen die bestehende Scheduling-Erstellung über einen sicheren nativen Link in einem neuen Browser-Tab. Search-Parameter belegen Tour und optional Ursprungsdatum genau einmal vor; Abbruch und Erfolg bereinigen diesen Kontext nur im neuen Tab.
17. Für jedes konkrete Tourvorkommen wählt `@sva/core` zuerst die jahresbezogene Ausnahme und andernfalls die auf das Jahr expandierte Grundregel. Studio-Kalender, Mainserver-Materialisierung und öffentlicher Abfallkalender verwenden dieselbe Auswahl vor globalen und Feiertagsregeln.
18. Ein Benutzer mit `waste-management.master-data.manage` kann unter den erweiterten Systemfunktionen den Job `waste-management.enrich-postal-codes` starten. Die Runtime lädt nur Orte mit leerer Postleitzahl, verwendet eingebettete fünfstellige Werte oder bis zu drei deterministische Straßenstichproben und akzeptiert ausschließlich plausible deutsche Konsenstreffer.
19. Provideraufrufe laufen seriell mit dem instanzbezogenen Rate-Limit. Mehrdeutige, unplausible und fehlende Treffer bleiben offen; unmittelbar vor dem Schreiben verhindert ein konditionales `UPDATE` das Überschreiben einer parallel gepflegten Postleitzahl.
20. Beim Öffnen der Abholortliste normalisiert das Plugin Filter, Sortiermodus, Richtung, Seite und Seitengröße aus den typsicheren Search-Params und ruft `GET /api/v1/waste-management/collection-locations` auf. Unbekannte oder widersprüchliche Direktparameter werden vor dem Repository mit `400 invalid_request` abgewiesen.
...
10. Webansicht, PDF und iCal konsumieren weiterhin dieselbe Repository-Ausgabe; Jahres- und Fraktionsfilter der Exporte greifen erst danach. Alle öffentlichen Abfragen beschränken Touren auf `published`; `draft` und `archived` liefern weder berechnete noch explizite Termine.
11. Ein externer Verbraucher kann alle öffentlich auswählbaren Abholortpfade veröffentlichter Touren über `GET /api/public-waste/locations` laden. Die Runtime dedupliziert und sortiert die reine Read-Projektion und liefert die vorhandenen Auswahl-IDs als `calendarQuery`.
...
8. Der Mainserver-Refresh lädt zuerst für alle sichtbaren Typen die jeweils erste Seite mit `pageSize = 25`; erst wenn dieser erste Block für alle Typen persistiert ist, folgen Seite 2, Seite 3 und so weiter im Round-Robin-Verfahren. Gefilterte GenericItem-Projektionen scannen dabei so viele Upstream-Seiten, bis die fachliche Seite gefüllt oder das Upstream-Ende erreicht ist; eine leere gefilterte Zwischenmenge beendet den Refresh nicht.
   Vor dem ersten Upstream-Aufruf muss das effektive Credential-Paar des exakten Principal-Scopes vollständig lesbar sein. Fehlende, partielle oder veraltete Credentials werden als scopegebundener Sync-Fehler persistiert; Scheduler, Reconciliation und gezielte Mutation-Follow-ups prüfen diesen Scope frühestens nach 15 Minuten erneut, ein manueller Refresh darf früher neu prüfen. Fällt der Follow-up einer bereits erfolgreichen Provider-Mutation in diesen Cooldown, bleibt der Upstream-Zugriff gesperrt und ein zuvor vollständig abgeschlossenes Mutationsjournal wird ausschließlich für die ausstehende lokale Projektion und Historie auf `reconciliation_required` gesetzt. Der nächste erfolgreiche Projektionslauf konsumiert den Eintrag nur bei identischem Principal-, Organisations- und Credential-Kontext sowie einem nicht jüngeren Provider-Snapshot idempotent; spezialisierte Generic-Item-Projektionen werden dabei auf ihren kanonischen Journal-Typ zurückgeführt. Andere Reconciliation-Gründe, bereits materialisierte History, noch ausstehende Transfer-Projektionen und zwischenzeitlich extern veränderte Inhalte bleiben offen.
...
12. Die Fachlisten und Detailseiten unter `/admin/news`, `/admin/events`, `/admin/poi`, `/admin/generic-items` und `/admin/projects` rufen weiterhin ihre jeweiligen Host-Fassaden auf. Die typisierten Read-Pfade liefern autorisierte Mainserver-Datensätze auch dann, wenn kein lokaler Content-Core und keine External-Content-Reference existieren.
13. Die tab-basierten Detail-/Editorseiten senden Create-, Update- und Delete-Requests an die jeweilige Fassade und Detailroute.
14. Die App-Fassade prüft Session, `instanceId`, aktiven Organisationskontext, plugin-spezifische IAM-Permission und Mainserver-Credentials serverseitig.
    Create sowie Einzel-, Bulk- und Organisations-Reprovisionierung melden Credential-Erfolg erst, wenn der Keycloak-Read-back denselben Fingerprint wie der zuvor geschriebene Wert ergibt.
...
18. Vor dem ersten Provider-Write bestätigt der Identity-Guard die aktuelle credential-versionierte DataProvider-Bindung. Ist sie nur deshalb konfliktbehaftet, weil sämtliche konkurrierenden Benutzer endgültig gelöscht sind, historisiert Studio diese Bindungen unter dem bestehenden DataProvider-Lock, verifiziert die aktuelle Bindung atomar und setzt denselben Request fort. Bei fehlenden Accounts belegt zusätzlich ein unveränderlicher erfolgreicher `user.deleted`-Eintrag aus `iam.activity_logs` den abgeschlossenen Hard Delete; archivierte Logs reichen nicht aus. Reversibel inaktive, aktive, organisatorische, verwaiste oder unklare Konkurrenten bleiben ohne Statusänderung fail-closed.
19. Nach erfolgreichen Mainserver-Mutationen lädt der Host gezielt genau den betroffenen Datensatz per typed Detailadapter nach und aktualisiert nur dessen Projektionszeile; bei einem geänderten GenericItem-Discriminator entfernt er zuvor passende fachliche oder generische Geschwisterzeilen. Delete-Pfade entfernen die Zeile identitätsbasiert ohne typweiten Vollrefresh.
20. Derselbe erfolgreiche Follow-up-Pfad kann die Mainserver-Identität an einen lokalen IAM-Content-Core binden und genau einen History-Eintrag mit Request-/Idempotenz-Korrelation finalisieren. Diese Folgearbeit ist optional: Ihr Fehlschlag ändert einen bestätigten Mainserver-Erfolg nicht. Providerfehler und Autorisierungsablehnungen erzeugen keinen sichtbaren Erfolgseintrag.
...
21. Beim Speichern von News laufen zwei technische Schritte: zuerst `createNews` oder `updateNews`, danach für den redaktionellen Zustand ein separater `changeVisibility(recordType: "NewsItem")`-Aufruf.
22. Die host-owned Studio-Newsliste liest denselben Pfad mit `includeInvisible=true` und filtert redaktionelle Stati (`Entwurf`, `Geplant`, `Veröffentlicht`) erst auf Studio-Seite aus Sichtbarkeit und `publishedAt`.
23. Besitzt der Redakteur `waste-management.read`, lädt der News-Editor aktive Abholorte erst beim Öffnen der Zielauswahl über den dedizierten `targeting`-Scope der Waste-Master-Data-Fassade. Dieser Scope liest die benötigten Hierarchien parallel und ohne Tour-Zuordnungen; der bestehende `locations`-Scope liefert Tour-Zuordnungen weiterhin für Waste-Filter und Zuordnungsansichten. Die UI hält IDs nur für Filterung und Auswahl; beim Speichern dedupliziert der Adapter stabile Adressschlüssel und führt sie verlustfrei mit dem vorhandenen News-Payload zusammen. Ohne Ziele fehlt `wasteLocationKeys`; bei einem tatsächlich ausgelösten Push bestätigt der Redakteur diesen globalen Zustand ausdrücklich. Nach bestätigter Zustellung bleibt die gespeicherte Empfängergruppe schreibgeschützt.
24. Der Mainserver bleibt fachlich führend; lokal werden nur Listenprojektion, Provider-Referenz und Studio-Mutationshistorie geführt. Das ist kein zweiter fachlicher Schreibpfad und keine vollständige externe Historie.
...
9. Für `new` leitet der Server Realm, technische Client-IDs und Issuer aus Instanz-ID, Baseline und konfiguriertem Keycloak-Endpunkt ab. Nach der Realm-Anlage wendet der privilegierte Provisioner die nicht geheime Baseline an; das fehlende SMTP-Passwort bleibt als nicht blockierende, aber offene manuelle Nacharbeit sichtbar. `reset_tenant_admin` ist bis zum erfolgreichen Wechsel auf `existing` unzulässig.
10. Die Fassade persistiert Run, Schritte und Audit-Event und setzt erst danach den Registry-Modus auf `existing`. Scheitert der Lauf vorher, wird ausschließlich der im selben Lauf neu angelegte Realm kompensierend entfernt; ein Löschfehler bleibt als manuell zu bereinigender Fehler sichtbar. Bestehende Realms und fremd vorgefundene Realms werden nie durch diese Compensation gelöscht.
...
5. Studio persistiert das normalisierte Credential-Paar zunächst verschlüsselt unter der aktiven Lease im nicht bereiten Zustand `provisioning`. Danach schreibt es die kanonischen Keycloak-Attribute, bestätigt deren Read-back, bindet den DataProvider mit `create_response`-Evidenz und setzt erst dann den Zustand auf `ready`. Ein fehlgeschlagener Read-back kann daher keine wirksamen Organisations-Credentials freigeben; ein expliziter Retry kann das bereits gesicherte Paar jedoch ohne erneute Upstream-Provisionierung verifizieren. Spätere Verifikation verwendet mit denselben Organisations-Credentials `/data_provider.json`.
...
4. Im Bereich `Medieninhalte` startet ein Upload den Host-Flow `initialize -> signed PUT -> complete`; im Review bleibt die Asset-Bearbeitung ohne `media.update` schreibgeschützt, die explizite Übernahme aber möglich.
5. Vor der Übernahme löst der Host eine persistierbare Delivery-URL auf. Ablauffähige oder signierte URLs werden fail-closed nicht in `mediaContents` geschrieben.
6. Beim Speichern persistiert der Editor zuerst den strukturierten POI-Write-Pfad inklusive `mediaContents` im Mainserver und ersetzt danach die geordneten `gallery_item`-Referenzen für die stabile POI-ID.
7. Ein Referenzteilfehler verändert den erfolgreichen Mainserver-Snapshot nicht; „Referenzen erneut speichern“ wiederholt keinen POI-Schreibzugriff.
...
2. Bei bestehenden Inhalten lädt der V2-Client den Detaildatensatz mit dem ressourcenbezogen bestätigten Principal. Der Host löst die credential-versionierte DataProvider-Identität auf und liefert für `update` und `delete` eine ressourcengebundene `meta.access`-Action-Map. Bei einer globalen Update-Berechtigung genügt für die Editorauflösung der Credential-Scope, über den derselbe Actor den Datensatz im aktuellen Organisationskontext geladen hat; `own` und `organization` benötigen weiterhin die exakte Ownership-Bindung. Kann der jeweils erforderliche Nachweis nicht belastbar geführt werden, bleibt die Aktion im Editor ausgeblendet, während ein bereits autorisierter Read weiterhin funktioniert.
...
5. Touren und alle kopierrelevanten Beziehungen werden mit stabilen IDs gemeinsam als `draft` angelegt. Große Beziehungsmengen werden in begrenzten JSON-Batches geschrieben. Jeder Fehler rollt die gesamte Transaktion zurück.
...
- Fehlende oder ungültige Authentisierung sowie unbekannte, gesperrte oder
  nicht bereite Instanzen liefern den versionierten Fehlervertrag ohne fremde
...
schreibfreien Readiness-Pfad um eine konstante Abfrage des persistierten
Projektionszustands und veröffentlicht den Mandanten nur, wenn dort mindestens
ein wirksam SSF-berechtigtes Subject bestätigt ist. Eine konvergierte leere
Projektion bleibt somit aus der Login-Auswahl, ohne den Runtime-Zugriff gültiger
Gäste-Sessions zu sperren. Root-Rechte werden dabei nicht automatisch auf
Tenant-Rechte ausgeweitet. Ein veraltetes `ready` wird erneut beansprucht und
repariert. Weder Directory noch Runtime lösen im Request-Pfad eine Keycloak-
Benutzerauflistung oder Projektion aus. Nicht bereite Ergebnisse bleiben im vorhandenen
