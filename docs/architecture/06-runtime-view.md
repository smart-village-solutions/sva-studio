...
8. Der Mainserver-Refresh lädt zuerst für alle sichtbaren Typen die jeweils erste Seite mit `pageSize = 25`; erst wenn dieser erste Block für alle Typen persistiert ist, folgen Seite 2, Seite 3 und so weiter im Round-Robin-Verfahren. Gefilterte GenericItem-Projektionen scannen dabei so viele Upstream-Seiten, bis die fachliche Seite gefüllt oder das Upstream-Ende erreicht ist; eine leere gefilterte Zwischenmenge beendet den Refresh nicht.
   Vor dem ersten Upstream-Aufruf muss das effektive Credential-Paar des exakten Principal-Scopes vollständig lesbar sein. Fehlende, partielle oder veraltete Credentials werden als scopegebundener Sync-Fehler persistiert; Scheduler, Reconciliation und gezielte Mutation-Follow-ups prüfen diesen Scope frühestens nach 15 Minuten erneut, ein manueller Refresh darf früher neu prüfen. Fällt der Follow-up einer bereits erfolgreichen Provider-Mutation in diesen Cooldown, bleibt der Upstream-Zugriff gesperrt und das vorhandene Mutationsjournal wird für die ausstehende lokale Projektion und Historie auf `reconciliation_required` gesetzt. Der nächste erfolgreiche Projektionslauf desselben Inhaltstyps konsumiert die passenden Journal-Einträge anhand der geladenen Source-IDs idempotent und materialisiert die fehlende Studio-Historie.
