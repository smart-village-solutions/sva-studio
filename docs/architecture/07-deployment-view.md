...
Der reguläre logische Dump läuft mit dem je Umgebung getrennt bereitgestellten PostgreSQL-Zugang `sva`. Für Restores meldet sich der Agent ausschließlich als `sva_restore` an und wechselt für Sicherheitsdump und Objektwiederherstellung zum fest verdrahteten Schema-Owner `sva`. Da Dumps ohne Privilegien erzeugt werden, rekonstruiert der Agent danach statisch die additiven IAM-Rechte des Runtime-Principals `sva_app` und validiert sie über PostgreSQL-Privilegprädikate. Owner, Runtime-Principal, Rolle und Grants sind nicht request-steuerbar; App-Zugangsdaten erhält der Agent nicht. Die Zugangsdaten des Restore-Principals bleiben ausschließlich als externe Swarm-Secrets am zentralen Backup-Agenten verfügbar.

Der dedizierte GitHub-Workflow rendert aus demselben unveränderlichen Stackvertrag zwei Zustände: `stopped` mit App und Provisioner auf null Replikate sowie `running` mit dem normalen Replikatvertrag. PostgreSQL bleibt in beiden Zuständen ein Singleton. Der Agent beendet die Stilllegung nie selbst. Production-Restore ist durch GitHub Environment und eine validierte Staging-Drill-Evidenz doppelt freigabepflichtig.
