...
- **Explizites Promote-Gate:** `promote.yml` bindet Git-Base/-Head, ausgecheckten Executor-Code und unveränderlichen Image-Digest vor jeder Mutation. `run` startet in Dev und Staging einen eindeutigen temporären One-shot-Stack auf `<stack>_internal`; eine Staging-Migration benötigt zusätzlich einen nicht-sensitiven, revisionsfähigen Wartungsfenster-Verweis. Production-`run` bleibt fail-closed.
