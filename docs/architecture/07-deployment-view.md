...
- Die Runtime erreicht die getrennte Datenbank `sva_waste` über das externe Overlay-Netz `studio_internal`; der Browser erhält zu keinem Zeitpunkt Datenbankzugang. Studio, Migration, öffentliche App und Owner verwenden getrennte Rollen mit minimalen Rechten.
...
Der zusätzliche allowlist-basierte Auftrag `database: "waste"` verwendet dieselben umgebungsgebundenen Buckets, aber das getrennte Objektpräfix `<umgebung>/waste/`, die aktive Backup-Quelle `sva_waste` und eigene Secret-Dateien. Der zugehörige Restore-Drill darf ausschließlich die isolierte Datenbank `sva_waste_restore_drill` ersetzen. Der Login `sva_waste_migrator` wechselt ausschließlich zu `sva_waste_owner`; nach einem Restore werden die CRUD-Rechte von `sva_waste_app`, die Leserechte von `sva_waste_public_app` und deren eng begrenzte Reminder-Schreibrechte statisch rekonstruiert und geprüft. Owner, Runtime-Principals, Datenbank und Grants sind in beiden Verträgen nicht request-steuerbar; App-Zugangsdaten erhält der Agent nicht.
