...
- **Deploy-Evidenz:** Reguläre Promotes schreiben redigierte GitHub-Step-Summaries und Action-Artefakte; lokale Incident-Recovery schreibt redigierte JSON- und Markdown-Reports unter `artifacts/runtime/deployments/`. Weder `.env`, `APP_CONFIG` noch unredigierte Remote-Logs oder personenbezogene Daten gehören in Evidenz. Der erfolgreiche Promote leitet die versionierte nicht-sensitive Config-Revision aus dem tatsächlich selektierten Deploy-Bundle ab, bindet sie über `sva.config.revision` an den laufenden App-Service und liest sie zusammen mit dem Digest aus demselben Service-Snapshot zurück. Nur der vorherige unveränderliche Digest zusammen mit seiner exakt live gebundenen Revision bildet einen zulässigen App-Rollback-Kandidaten. Fehlt oder scheitert diese Bindung, stoppt der Recovery-Vertrag fail-closed und verlangt einen separat geprüften Plan. Datenbank-Rollback ist nicht automatisiert.
- **Legacy-Seed-Grenze:** Staging und Production akzeptieren bei vorhandenem Live-Digest keinen mutierenden Promote ohne gültige, gleichzeitig erfasste `sva.config.revision` – weder bei Digestwechsel noch bei Same-Digest und weder im Standard- noch im Recovery-Modus. Die einzige engere Ausnahme ist der einmalige H4-Zwei-Run-Handshake für ein tatsächlich fehlendes Staging-Label: Ein geschützter `workflow_dispatch` muss zuerst mit Same-Digest, Standardmodus, `change_base=change_head`, Migration/Bootstrap `assert-none` und dem exakten H3-STOP eine schema-strikte Fehlerevidenz erzeugen; nur dessen genaue Run-ID und Attempt dürfen einen zweiten expliziten Seed-Dispatch autorisieren. Der vor dem Source-Wechsel konservierte Controller ergänzt über ein strukturell geprüftes Staging-only Overlay ausschließlich das Label. Candidate, Main-E2E, Backup-Capability, frische Backups, Deploy, Konvergenz, Smoke und gemeinsamer Digest-/Config-Readback bleiben blockierend. Der Seed erzeugt keine Production-fähige Parität und erfindet weder vorherige Config-Revision noch Rollback. Dev, Production, Recovery, Digestwechsel sowie vorhandene ungültige oder gültige Labels bleiben ausgeschlossen; nach Erfolg ist ein erneuter Seed technisch gesperrt.
