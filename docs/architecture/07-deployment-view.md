...
Der reguläre logische Dump läuft mit dem je Umgebung getrennt bereitgestellten PostgreSQL-Zugang `sva`. Für Restores meldet sich der Agent ausschließlich als `sva_restore` an und wechselt für Sicherheitsdump und Objektwiederherstellung zur fest verdrahteten App-Rolle `sva`. Dadurch bleiben Eigentum und Tabellenrechte nach dem Restore intakt, ohne einen frei wählbaren Datenbank- oder Rollenparameter zu eröffnen. Die Zugangsdaten bleiben ausschließlich als externe Swarm-Secrets am zentralen Backup-Agenten verfügbar.
