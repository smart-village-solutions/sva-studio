...
- **Legacy-Seed-Grenze:** Staging und Production akzeptieren bei vorhandenem Live-Digest keinen mutierenden Promote ohne gültige, gleichzeitig erfasste `sva.config.revision` – weder bei Digestwechsel noch bei Same-Digest und weder im Standard- noch im Recovery-Modus. H4 und H5 sind zwei getrennte einmalige Zwei-Run-Verträge. H4 ist ausschließlich Staging und bindet den Seed an kanonische Main-E2E-Evidenz. H5 ist ausschließlich Production und verlangt zuerst einen schema-strikten Shadow-Prepare-Nachweis: Config-Äquivalenz, Same-Digest, Labelzustand `missing`, Production-Readiness sowie tatsächlich bestandene Candidate-/Backup-Capability-Shadow-Gates. Erst der unmittelbar folgende Enforce-Run darf ein controller-eigenes Production-only Overlay verwenden. Beide Overlays ergänzen nach strukturellem Rendervergleich ausschließlich das Label; Seed-Läufe behalten Backups, Candidate, Capabilities, Deploy, Konvergenz, Smoke und gemeinsamen Digest-/Config-Readback bei, erzeugen keine Staging-Parität und erfinden weder vorherige Config-Revision noch Rollback. Dev, Recovery, Digestwechsel sowie vorhandene ungültige oder gültige Labels bleiben ausgeschlossen; nach Erfolg ist ein erneuter Seed technisch gesperrt.
