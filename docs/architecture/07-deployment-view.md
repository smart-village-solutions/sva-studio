...
Der logische Dump läuft mit dem je Umgebung getrennt bereitgestellten PostgreSQL-Administrationszugang `sva`. Der normale App-Principal `sva_app` ist dafür ungeeignet, weil dessen beabsichtigte RLS-Bindung keinen garantiert vollständigen Dump aller Mandantendaten zulässt. Die Zugangsdaten bleiben ausschließlich als externe Swarm-Secrets am zentralen Backup-Agenten verfügbar.
