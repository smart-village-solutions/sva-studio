...
- Für mutierende Staging-Rollouts ist GitHub Actions `Promote` der kanonische Kanal: Das geschützte Environment stellt die Credentials bereit, ein revisionsfähiger nicht-sensitiver Wartungsfenster-Verweis ist bei `migration_mode=run` Pflicht und der Workflow bindet Git-Base/-Head, ausgecheckten Executor-Code und unveränderliches Image-Digest vor jeder Mutation.
...
- Production erlaubt weiterhin ausschließlich den App-only-Pfad; beide One-shot-`run`-Modi sind bis zu einem separaten Change mit Staging-Parität, Freigabe, Backup-/Restore-Readiness und Production-Postconditions gesperrt.
