...
- Der persistierte Queue-Schritt bindet neue Runs über `pluginOidcSnapshotVersion` an den in der App validierten Plugin-OIDC-Snapshot. Unversionierte sowie unvollständige Aufträge werden vor Keycloak-Zugriff abgewiesen und vor einem Versionswechsel mit dem bisherigen Worker geleert; ein expliziter Queue-Snapshot hat an der Provisioner-Grenze Vorrang vor Prozessdefaults.
