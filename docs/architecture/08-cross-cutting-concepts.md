...
Der einmalige H4-Übergang behandelt ausschließlich ein fehlendes Config-Label am bereits mit dem Zieldigest laufenden Staging-Service. Seine Autorisierung ist kein freies Operator-Flag: Ein zweiter geschützter `workflow_dispatch` muss die genaue Run-ID und den Attempt eines unmittelbar vorherigen, auf `main` abgeschlossenen, am H3-Recovery-Vertrag fehlgeschlagenen Staging-Standard-Promotes referenzieren. Der Verifier akzeptiert nur das eindeutige nicht abgelaufene schema-v2-Artefakt mit der vollständigen erwarteten Pre-Mutation-Gatematrix und prüft Run, Attempt, Artefakt, Same-Digest sowie den Zustand `missing` vor und nach dem Download erneut. URLs, Actor, Gründe, freie Texte und der rohe Labelwert bleiben aus der allowlisteten Seed-Evidenz ausgeschlossen.

Der Seed übernimmt keine historische Config-Bindung: `previousConfigRevision` und Rollback bleiben leer. Das controller-eigene Staging-only Overlay darf strukturell ausschließlich `sva.config.revision` ergänzen; Dev, Production, Recovery, Digestwechsel und vorhandene ungültige oder gültige Labels sind ausgeschlossen. Main-E2E, Candidate, Backup-Capability, frische Studio-/Waste-Backups, Deploy, Swarm-Konvergenz, Runtime-Smoke und gemeinsamer finaler Digest-/Config-Readback bleiben blockierend. Weil der Lauf keine Production-fähige Staging-Parität veröffentlicht und ein gültiges Label jeden weiteren Seed ablehnt, kann diese Ausnahme weder zum allgemeinen Promote- noch zum Production-Pfad werden. Historische erfolgreiche Runs dürfen nur als read-only Cross-Check dienen und autorisieren den Seed nicht.
