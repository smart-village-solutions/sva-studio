...
- Der explizite IAM-Baseline-Reconcile erfasst fehlende Verträge vor dem regulären Reconcile der Modulaktivierungsregeln und persistiert anschließend die Core-Basis sowie alle Verträge der dann aktiven bekannten Module. Ein zuvor aktives unbekanntes Modul bleibt mit `unknown_module_contract:<moduleId>` sichtbar, erhält keine erfundenen Rechte und verliert bestehende modulbezogene Grants, wenn es durch die Aktivierungsregeln deaktiviert wird; normale Modulzuweisung, Modulentzug und Admin-Bootstrap bleiben dagegen vollständig atomar und fail-closed.
