...
- `Promote` trennt Vertrauens- und Releasegrenze mit zwei Git-Checkouts: Controller-Validierung und Evidenz stammen vollständig aus `${{ github.workflow_sha }}` unter `.promote-controller/`, releasegebundene Profile, Diffs, One-shots und Render aus dem vollständigen `change_head`-Workspace. Nur dort werden Abhängigkeiten installiert; eine selektive Controller-Dateikopie ist ausgeschlossen.
