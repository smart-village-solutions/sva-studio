...
- Tenant-Login-Probes übernehmen den vollständigen Issuer vorrangig aus dem Registry-Ziel. Nur wenn er dort fehlt, gelten die bestehenden Runtime-Fallbacks. Ein Authorization-Redirect ist ausschließlich als absolute HTTPS-URL ohne Userinfo oder Fragment und mit exakt derselben Origin sowie dem Issuer-Pfad `protocol/openid-connect/auth` vertrauenswürdig. Abweichungen stoppen vor dem Authorization-Request; Reports enthalten weder Redirect-URL noch Query-Werte. Damit vollzieht der Doctor die Registry- und Login-Grenzen aus [ADR-030](../adr/ADR-030-registry-basierte-instance-freigabe-und-provisioning.md) und [ADR-033](../adr/ADR-033-tenant-login-client-vs-tenant-admin-client.md), ohne einen zweiten Auth-Vertrag einzuführen.
