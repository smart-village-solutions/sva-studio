...
- Die Runtime übernimmt den Queue-Namen aus der hostvalidierten Jobdefinition. Auch der persistente Recovery-Pfad reiht bestehende nicht terminale Lifecycle-Jobs über die aktuell registrierte Queue und Worker-Lane erneut ein. Da Graphile eine benannte Queue seriell verarbeitet, dürfen Default- und privilegierte Worker-Lanes bei konkurrierenden Jobs nicht dieselbe Queue teilen; die Waste-Tenant-Provisionierung verwendet deshalb `waste-provisioning` statt `plugin-operations`.
...
- Die öffentliche Waste-Webversion ist betriebsseitig strikt vom Studio-Releasepfad getrennt: eigenes Image, eigene Zielstacks, eigener Workflow und eigene, GitHub-environment-getrennte Variablenräume.
...
- Git-Tags `waste-web-vX.Y.Z` sind die kanonische Freigabe für diese Bürgeroberfläche; ein Tag baut das Image einmal und verteilt denselben Tag über getrennte Jobs auf `web-waste-calendar` und `web-waste-calendar-frankfurt-oder`. Andere Branch- oder Studio-Releases dürfen diese Stacks nicht mitverändern.
...
- News laufen produktiv über die hostgeführte Mainserver-News-Fassade; dedizierte Mainserver-Felder und `contentBlocks` sind das redaktionelle Schreibmodell. Das Payload transportiert ergänzende technische Metadaten verlustfrei. Der Editor besitzt dabei ausschließlich `wasteLocationKeys`; fremde Schlüssel werden beim Update nicht entfernt. Waste-Zieldaten werden lazy geladen, dynamische Treffer barrierefrei angekündigt und nach bestätigter Zustellung nicht mehr verändert. Lokale IAM-Content-Validierung ist für News keine Persistenzquelle mehr
...
- Der explizite IAM-Baseline-Reconcile erfasst fehlende Verträge vor und nach dem regulären Reconcile der Modulaktivierungsregeln. Dessen IAM-Synchronisation wird in diesem Reparaturpfad bis zum anschließenden expliziten Baseline-Schritt zurückgestellt; dieser persistiert die Core-Basis sowie alle Verträge der dann aktiven bekannten Module. Das intern erzeugte Aktivierungs-Reconcile-Ergebnis wird nach dem Transaktions-Commit weiterhin an den bestehenden Lifecycle-Scheduler übergeben. Ein unbekanntes Modul bleibt mit `unknown_module_contract:<moduleId>` sichtbar, erhält keine erfundenen Rechte und verliert bestehende modulbezogene Grants unabhängig davon, ob die Aktivierungsregel es beibehält oder deaktiviert. Der Browser übernimmt betroffene Modul-IDs und Diagnosecodes ausschließlich über die Safe-Details-Allowlist und nennt sie in der vorhandenen Fehlermeldung; normale Modulzuweisung, Modulentzug und Admin-Bootstrap bleiben dagegen vollständig atomar und fail-closed.
- Fleet-Reconcile-Berichte und Metriken verwenden ausschließlich begrenzte `reason_code`- und `retry_class`-Dimensionen. Nur ein Prozess, der den Reconcile tatsächlich ausführt, beobachtet diese Metriken; der privilegierte Provisioner veröffentlicht keinen leeren oder konkurrierenden Fleet-Zustand. Unbekannte Abbrüche werden nicht als neue Fehlerdimension übernommen, sondern fail-closed auf `plugin_activation_policy_reconcile_unknown` und `degraded` abgebildet.
...
- Der persistierte Queue-Schritt bindet neue Runs über `pluginOidcSnapshotVersion` an den in der App validierten Plugin-OIDC-Snapshot. Unversionierte Bestandsaufträge werden als früherer Vertrag ohne Plugin-OIDC-Anforderungen gelesen; unvollständige versionierte Aufträge werden vor Keycloak-Zugriff abgewiesen. Installationen mit Plugin-OIDC-Anforderungen leeren unversionierte Aufträge vor dem Versionswechsel mit dem bisherigen Worker. Ein expliziter Queue-Snapshot hat an der Provisioner-Grenze Vorrang vor Prozessdefaults.
...
- Der sichere Keycloak-Proxy-Kontext ist eine gemeinsame Ingress- und Auth-Invariante: Traefik überschreibt `X-Forwarded-*`, Keycloak wertet `xforwarded` ausschließlich für das live verifizierte, nicht attachbare Ingress-Overlay-Subnetz aus und browserbasierte Auth-Cookies bleiben hinter der TLS-Terminierung `Secure`. Eingespeiste Client-Header dürfen diese Bewertung nicht herabstufen; Details und Abnahme stehen im [Keycloak-Reverse-Proxy-Runbook](../operations/keycloak-reverse-proxy-secure-context.md).
- Tenant-Login-Probes übernehmen Issuer und Browser-Client vorrangig aus dem Registry-Ziel. Ein Authorization-Redirect ist ausschließlich als absolute HTTPS-URL ohne Userinfo oder Fragment, mit exakt derselben Origin und dem Issuer-Pfad `protocol/openid-connect/auth`, passender Client-ID, `response_type=code`, `response_mode=query`, hostgebundener Redirect-URI sowie State, Nonce und PKCE S256 vertrauenswürdig. Abweichungen stoppen vor dem Authorization-Request; Reports enthalten weder Redirect-URL noch Query-Werte. Browser-Clients werden nicht mit `client_credentials` geprüft. Damit vollzieht der Doctor die Registry- und Login-Grenzen aus [ADR-030](../adr/ADR-030-registry-basierte-instance-freigabe-und-provisioning.md) und [ADR-033](../adr/ADR-033-tenant-login-client-vs-tenant-admin-client.md), ohne einen zweiten Auth-Vertrag einzuführen.
...
- Der Runtime-Doctor prüft für Remote-Profile ein rollierendes 15-Minuten-Loki-Fenster auf Keycloaks Warnung zum unsicheren Cookie-Kontext. Jeder Treffer macht `observability-readiness` fail-closed rot; vor einem grünen Ergebnis werden leere Loki-Antworten begrenzt wiederholt, um Ingestion-Lag abzudecken. Das release-blockierende Gate akzeptiert nur eine erfolgreiche Loki-Streams-Antwort mit strukturell gültigen String-Labels und Zeitstempel-/Logzeilen-Tupeln; unvollständige oder anders typisierte Payloads sind Fehler statt leere Evidenz. Diagnoseausgaben enthalten nur Treffer-Untergrenze, Abfragelimit und Fenstergröße, niemals die Logzeile. Diese Verschärfung konkretisiert den Loki-Nachweis aus [ADR-019](../adr/ADR-019-swarm-traefik-referenz-betriebsprofil.md).
...
- Die Profilreparatur beim tenantlokalen Keycloak-Import folgt deterministisch `Quellwert -> instanz- und subjectgebundener lokaler Seed -> syntaktisch gültiger Username nur als E-Mail`. Die reine Entscheidung ist von Lookup, Provider-Mutation, Savepoint, IAM-Persistenz und Reporting getrennt; an Keycloak werden ausschließlich tatsächlich geänderte Felder übertragen. Nur eine nach diesen Fallbacks weiterhin fehlende E-Mail verhindert Account und Membership und wird als `manual_review` behandelt; eine fehlgeschlagene erforderliche E-Mail-Reparatur bleibt fail-closed. Fehlende Vor- oder Nachnamen bleiben optional, werden einschließlich reiner Leerzeichen vor dem Upsert als abwesend normalisiert, verwenden den bestehenden Anzeigenamen-Fallback und werden nicht durch persistierte Default-Texte ersetzt. Nur wenn sämtliche strukturierten Feldfehler ausschließlich schreibgeschützte Vor- oder Nachnamen betreffen, wird eine reine Namensreparatur datensparsam protokolliert und blockiert IAM-Persistenz sowie Membership nicht; technische, retrybare, Not-found-, gemischte Validierungs- und unbekannte Fehler bleiben fail-closed.
...
- Inhalts-Historie nutzt ein eigenes Read-Modell statt Roh-Logs; jede erfolgreiche Erstellung, Aktualisierung und jeder Statuswechsel erzeugt zusätzlich Audit-Ereignisse im bestehenden IAM-Auditpfad. Sichtbare Einträge tragen `origin = studio`, `coverage = studio_mutations` und optional eine idempotente `mutation_ref`; `snapshot_json` bleibt intern. Audit-Payloads für Content-Aktionen enthalten additiv fachliche Capability, primitive Action, Ergebnis, Reason-Code und Korrelationsfelder, ohne bestehende Exportformate zu migrieren.
- Mainserver-History bezeichnet ausschließlich erfolgreiche Studio-Mutationen. Direkte externe Änderungen werden nicht erfunden; fehlgeschlagene oder verweigerte Versuche bleiben im Audit-/Diagnosepfad. Lesen prüft `content.readHistory` mit demselben Instance- und Ownership-Scope wie der Inhalt.
- Plugin-Contributions deklarieren die Historienverantwortung fail-closed als hostgeführt, fachgeführt oder mit stabilem Ausnahmegrund nicht erforderlich. Waste Management behält seine fachliche und technische Historie; alle übrigen bearbeitbaren Content-Plugins verwenden den gemeinsamen Host-Client und die gemeinsame schreibgeschützte UI.
...
- Reguläre Promotes erzeugen in GitHub Actions eigenständige redigierte Artefakte für Imagevertrag, tatsächliche Deploy-Basis, Phasenstatus, Backup, Migration, Bootstrap, Jobstatus, Swarm-Konvergenz, Postconditions, Runtime-Smoke und Digest-Prüfung
...
- One-shot-Evidenz enthält nur redigierte Job-/Task-ID, Failure-Klasse, Terminalzustand, Exit-Code, Dauer, Cleanup und Recovery-Hinweis. Sie wird auch beim terminalen Jobfehler vor der Stack-Bereinigung persistiert. `.env`, `APP_CONFIG`, Tokens, Task-Messages, vollständige Remote-Logs, SQL-Fehler mit sensitiven Daten und PII bleiben ausgeschlossen. Bei Migration, Bootstrap, Postcondition oder Verifikation wird vor dem App-Deploy fail-closed abgebrochen; automatisches Datenbank-Rollback ist ausgeschlossen.
...
- Mutierende Studio-Kommandos laufen regulär ausschließlich über GitHub Actions `Promote`; lokale Operator-Mutationen sind Incident-Recovery und kein Legacy- oder Parallelstandard
...
- `Promote` trennt Vertrauens- und Releasegrenze mit zwei Git-Checkouts: Fehlerklassifikation, Evidenz sowie controllerseitige Remote- und Konvergenzprüfungen stammen aus `${{ github.workflow_sha }}` unter `.promote-controller/`; Image-/Deploy-Contracts, Profile, Diffs, One-shots und Render bleiben im vollständigen `change_head`-Workspace. Der Controller samt eigener Node-Version bleibt auch bei einem fehlgeschlagenen Release-Checkout verfügbar und klassifiziert diesen fail-closed. Nur im Release-Workspace werden Abhängigkeiten installiert; eine selektive Controller-Dateikopie ist ausgeschlossen.
...
- `promote_mode=recovery` ist eine einzelne Production-Ausnahme für die initiale Readiness. Der Modus benötigt einen dokumentierten Grund, ist in Dev und Staging ungültig und lässt sämtliche nachfolgenden Gates einschließlich Staging-Parität, Backup, One-shots, Smoke sowie Digest-/Config-Prüfung blockierend.
- Staging-Parität akzeptiert nur noch Evidence v2 mit kanonischer erfolgreicher Main-E2E-Evidenz für exakt `change_head`. Neue Promote-Evidenz leitet Recovery-Werte direkt aus Modus, Live-Digest und gebundener Config-Revision ab; ein separates Recovery-Contract-Objekt und Legacy-/Shadow-/Disabled-Auswertungen entfallen.
...
- Vor dem kanonischen Docker-Build prüft `verify:runtime-artifact` den finalen Node-Output `apps/sva-studio-react/.output/server/index.mjs` mit Artefakt-Assertions, temporären Migrationen und Health-Probes. `Promote` bindet das veröffentlichte Image anschließend an Digest und OCI-Revision.
- `env:precheck:studio` dokumentiert im lokalen Incident-Recovery-Pfad ergänzend vorhandene Image-Verify-Evidenz; sie ist kein reguläres Promote-Gate
...
- Relevante Bot-Kommentare von `Copilot` und `chatgpt-codex-connector[bot]` werden im normalen GitHub-Review-Prozess fachlich geprüft und nachvollziehbar abgeschlossen
...
- **Getrennte Ingress-Freigabe:** DNS-Auflösung, explizite Traefik-Regel, konkretes TLS-Zertifikat und aktive Registry-Zuordnung sind getrennte Gates. Eine Ingress-Regel autorisiert keinen Tenant fachlich; die Runtime prüft weiterhin exakt und fail-closed gegen die Registry.
- **Betriebsgrenze:** Tenant-Erstellung erhält keine Rechte auf den gemeinsamen Traefik und keine AutoDNS-Credentials. Neue Production-Hosts werden ausschließlich als versionierte Compose-Änderung über `Build` und `Promote` freigegeben.
- **Studio-Root und Auth-Hosts:** `SVA_STUDIO_ROOT_HOST` legt optional den Studio-Root unabhängig von `SVA_PARENT_DOMAIN` fest; ohne Wert bleibt die Parent-Domain der Root. Die Basisdomain ist bei getrenntem Root kein zusätzlicher Studio-Root. `studio` und `auth` sind als Tenant-Hostnamen reserviert. Root-Verwaltung und globale Plugin-Konfiguration bleiben an Root-Host und Plattformrolle gebunden. Tenant-OIDC-Flows verwenden ihre Registry-Konfiguration und Callback/Logout auf dem jeweiligen Tenant-Host. Auth-Cookies bleiben host-only, ohne gemeinsame Domain. Der öffentliche Keycloak-Host ist ein eigener Dienst und kein Studio-Auth-Callback. Siehe [eigenständige SSF-Installation](../operations/ssf-standalone-hosts.md).
...
- Der interne Projektions-Scope schließt zusätzlich Principal-Typ und Credential-Fingerprint in die Source-/Binding-Entscheidung ein. Es gibt weder Principal- noch Credential-Fallbacks; fehlende oder fehlerhafte Bindings bleiben fail-closed im bestehenden Compatibility-Modus und werden nur mit technischen Metadaten protokolliert.
- Schema-Mode-Erkennung und der einmalige Legacy-/Scoped-Retry liegen ausschließlich im Projektions-Repository. Nutzwerte bleiben gebundene SQL-Parameter; dynamisch zusammengesetzte Klauseln stammen nur aus intern kontrollierten Selektoren.
...
Die abgeschlossenen Live-Config-Übergänge besitzen keine aktive Autorisierungsschnittstelle mehr. Wenn eine gleichzeitig mit dem Live-Digest gelesene `sva.config.revision` in Staging oder Production fehlt oder ungültig ist, stoppen Standard und Recovery vor Backup und Mutation. Historische Evidence-v2-Artefakte dürfen weiterhin schema-kompatibel gelesen werden, können aber keinen Workflowzweig aktivieren. Neue Evidence-v2-Artefakte setzen `seedPreparation` und `seedAuthorization` ausdrücklich auf `null`. Eine Wiederherstellung benötigt einen neuen, separat geprüften Recovery-Change innerhalb des bestehenden geschützten `Promote`-Pfads.
...
- Eine globale Update-Berechtigung darf den für denselben Actor und aktiven Organisationskontext beobachteten Projection-Scope zur Auswahl der Credential-Quelle verwenden. Das ersetzt weder den Same-Credential-Pre-Read noch die Autorisierung durch den Mainserver; eingeschränkte Scopes bleiben an die exakte Ownership-Bindung gebunden.
...
- Ausschließlich `pr-scope` erzeugt einmal pro PR-Head eine Scope-Evidenz mit Schema-Version, Base-SHA, Head-SHA, normalisierter Dateiliste, Gate-Modi und Fallback-Gründen. Alle PR-Gates konsumieren diese Entscheidung; sie führen keine zweite allgemeine Pfadpolicy. Fehlende Scope- oder Worker-Evidenz lässt die stabilen Unit-/Coverage-Aggregatoren fail-closed enden.
- Main und Nightly übernehmen weder PR-Scope noch PR-Cache und führen die nicht deploymentbezogenen Verträge vollständig aus. App-Build, Main-E2E und Promote bleiben außerhalb der konsolidierten Main-Topologie und behalten ihre bestehende Release-Ownership.
- Die abgeschlossene Shadow-Parität bleibt als historischer, Head-SHA-gebundener Nachweis unter `docs/reports/ci-gate-shadow-parity-2026-08.md` erhalten; produktive Workflows führen keine Paritätssammler oder Legacy-Scope-Berechnung mehr aus.
...
- Das Verzeichnis veröffentlicht nur aktive, vollständig loginbereite Registry-Einträge
  ...
  wirksam SSF-berechtigtes Subject im persistierten bestätigten Projektionszustand.
  Eine revisionsgleiche leere Projektion bleibt dadurch unveröffentlicht, während
  gültige Gäste-Sessions die Runtime-Konfiguration weiterhin abrufen können.
  Directory- und Runtime-Requests lösen keine zusätzliche Keycloak-Benutzerauflistung
  oder Projektion aus.
...
Client-Action `ssf.runtime-configuration.read`, aber keine menschliche
