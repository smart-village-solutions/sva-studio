...
- Die Profilreparatur beim tenantlokalen Keycloak-Import folgt deterministisch `Quellwert -> instanz- und subjectgebundener lokaler Seed -> syntaktisch gültiger Username nur als E-Mail`. Die reine Entscheidung ist von Lookup, Provider-Mutation, Savepoint, IAM-Persistenz und Reporting getrennt. Nur eine nach diesen Fallbacks weiterhin fehlende E-Mail verhindert Account und Membership und wird als `manual_review` behandelt; eine fehlgeschlagene erforderliche E-Mail-Reparatur bleibt fail-closed. Fehlende Vor- oder Nachnamen bleiben optional, werden einschließlich reiner Leerzeichen vor dem Upsert als abwesend normalisiert, verwenden den bestehenden Anzeigenamen-Fallback und werden nicht durch persistierte Default-Texte ersetzt. Nur eine deterministische Keycloak-Ablehnung wegen schreibgeschützter Attribute wird bei einer reinen Namensreparatur datensparsam protokolliert und blockiert IAM-Persistenz sowie Membership nicht; technische, retrybare, Not-found- und unbekannte Fehler bleiben fail-closed.
