...
- News laufen produktiv über die hostgeführte Mainserver-News-Fassade; dedizierte Mainserver-Felder und `contentBlocks` sind das redaktionelle Schreibmodell. Das Payload transportiert ergänzende technische Metadaten verlustfrei. Der Editor besitzt dabei ausschließlich `wasteLocationKeys`; fremde Schlüssel werden beim Update nicht entfernt, und lokale IAM-Content-Validierung ist für News keine Persistenzquelle mehr
