...
- Der persistierte Queue-Schritt bindet neue Runs über `pluginOidcSnapshotVersion` an den in der App validierten Plugin-OIDC-Snapshot. Unversionierte Bestandsaufträge verwenden für die Übergangslesbarkeit die installierte Anforderungsliste des Workers. Fehlende oder ungültige Daten in einem versionierten Snapshot werden vor Keycloak-Zugriff abgewiesen; ein expliziter Queue-Snapshot hat an der Provisioner-Grenze Vorrang vor Prozessdefaults.
