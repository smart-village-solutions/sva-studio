...
- Termine, Ortsdaten, Veranstalter, Beschreibung/Medien sowie die drei Repeater-Familien besitzen jeweils einen sichtbaren plugininternen Owner; der Root-Tab enthält keine parallele Inline-Implementierung mehr.
- Die produktive Zielstruktur umfasst 843 statt 854 TypeScript-/TSX-Zeilen, also netto −11 Zeilen. Der Refactor hat weder einen neuen öffentlichen Export noch eine Dependency oder Shared-Abstraktion eingeführt.
...
- Der pfadsensitive Fallow-New-only-Audit meldet die in neue Section-Dateien verschobenen Bestandsblöcke formal als eingeführt. Der direkte Produktionsvergleich weist jedoch neun Events-bezogene Clone-Gruppen in der Baseline und acht im Zielstand aus; neue kritische Komplexität, neuer Dead Code und neue Boundary-Verletzungen liegen nicht vor.
