...
- Keycloak-Proxy-Sicherheit:
  ...
  - Der Precheck folgt dem validierten HTTPS-Authorization-Redirect bis zu Keycloak, ohne URL oder Cookies zu protokollieren, und führt erst danach die Sicherheitsabfrage aus; eine HTTP-Erfolgsantwort ohne Loki-Status `success` und gültiges `data.result` gilt als blockierender Probe-Fehler.
