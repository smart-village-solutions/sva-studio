...
PostgreSQL-Diagnostik ist auf SQLSTATE, Tabelle, Spalte und Constraint begrenzt. Meldung, Detail, Hint, Query, Parameter, Stacktrace, Providerantworten, E-Mail-Adressen, Passwörter, Tokens und Connection-Strings sind ausgeschlossen. Das äußere Event `tenant_provisioning_step_exception` ergänzt `diagnostic_error`: Als PostgreSQL-Fehler erkannte Exceptions enthalten ausschließlich normalisierten Fehlertyp, Code, Tabelle, Spalte und Constraint. Node-Systemfehler enthalten Fehlertyp, Systemfehlercode, Systemaufruf und gegebenenfalls die kontrollierten Dateipfade; alle übrigen Exceptions ausschließlich den normalisierten Fehlertyp. Dieselbe Einschränkung gilt an der Ingress-Publish-Grenze. Retry-Klassifikation und persistierte Fehlermeldungen bleiben unverändert. Request-, Instanz- und Run-IDs werden ausschließlich im Log-Body geführt. Der stdio-MCP verwendet kein Console-Logging, weil `stdout` dem Protokoll vorbehalten ist.
