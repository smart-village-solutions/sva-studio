...
| `@sva/waste-management-contracts` | Gemeinsame Waste-Job-, Importprofil- und Abmeldetokenverträge                                                                           | `packages/waste-management-contracts`                                                                   | Enthält weder Browser-UI noch Jobausführung. Plugin und Runtime konsumieren `/job-definitions`; Studio und Public-Waste-App konsumieren den serverseitigen `/unsubscribe-token`-Export. Nur `@sva/plugin-waste-management` darf als owning Plugin diese fachliche Package-Kante zusätzlich zu den generischen Plugin-Einstiegspunkten verwenden.                     |
| `@sva/waste-management-runtime`   | Host-owned Waste-Management-Runtime für pluginbezogene Job-Ausführung und Runtime-Adapter                                               | `packages/waste-management-runtime`                                                                     | Kein Plugin-Package: hostseitige Runtime-Auflösung und Handler-Mapping bleiben außerhalb von `@sva/plugin-waste-management`; Studio nutzt `/server`. Die Runtime konsumiert deklarative Definitionen aus `@sva/waste-management-contracts` und hängt nicht vom Browser-Plugin ab.                                                                                    |
...
- Anwendungen importieren keine Quellmodule aus anderen Verzeichnissen unter `apps/`; mehrfach benötigte Verträge liegen in einem owning Workspace-Package.
