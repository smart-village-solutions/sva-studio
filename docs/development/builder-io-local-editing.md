...
Ursache ist, dass Builder beim Dateiscan auf Quellpfade außerhalb des App-Ordners trifft, zum Beispiel `../../packages/routing/...`. Diese Pfade entstehen durch die Monorepo-Auflösung der App und sind für die intern verwendete Ignore-Logik des Builder-CLI problematisch.
...
Das Script ruft intern auf:

```bash
npx builder.io@latest launch -p 3000 -c "pnpm nx run sva-studio-react:serve" --no-open --workspace ./builder.workspace.json
```

## Zusammenspiel mit Runtime-Profilen

Für einen vollständigen lokalen Builder-Betrieb mit Mock-User ist `local-builder` das kanonische Profil:
...
Die Betriebsdetails dazu sind in [`./runtime-profile-betrieb.md`](./runtime-profile-betrieb.md) dokumentiert.

`pnpm run builder:launch` ergänzt dieses Profil um den Builder.io-Editorstart für die React-App. Es ersetzt nicht die Runtime-Profile, sondern nutzt die bestehende App-Startlogik.
