...
- Meldet die Identity-Prüfung einen Konflikt und gehören alle konkurrierenden aktuellen Bindungen ausschließlich zu endgültig gelöschten Benutzern, löst Studio diesen Zustand im selben Request automatisch auf. Bei einem nach Hard Delete fehlenden Account muss zusätzlich ein unveränderlicher erfolgreicher `user.deleted`-Nachweis in `iam.activity_logs` vorliegen. Archivierte Activity-Logs werden nicht als Löschbeweis akzeptiert. Die alten Bindungen werden `historical`, die bestätigte aktuelle Bindung wird `verified` und die noch nicht an den Mainserver gesendete Mutation läuft weiter.
- Aktive, gesperrte, vorläufig gelöschte, organisatorische oder nicht eindeutig klassifizierbare Konkurrenten verhindern diese Selbstheilung. Studio verändert dann keine Bindung und antwortet weiterhin mit `mainserver_data_provider_identity_conflict`.
...
### Anlassbezogene Selbstheilung nach Account-Hard-Delete

Die Selbstheilung ist kein allgemeiner Rebind und benötigt keine Admin-Oberfläche. Sie wird ausschließlich durch einen betroffenen Mutationsversuch nach einer frischen erfolgreichen `/data_provider.json`-Prüfung ausgelöst. Observation und Reconciliation nehmen in derselben Reihenfolge zuerst den instanzgebundenen Principal-/Credential-Lock und danach den DataProvider-Lock. Anschließend liest Studio alle aktuellen Konfliktbindungen und die Benutzer-Lifecycle-Zustände mit einem frischen Datenbank-Snapshot erneut. Nur wenn der aktuelle Benutzer aktiv und jeder andere beanspruchende Benutzer endgültig gelöscht ist, werden die konkurrierenden Zeilen mit `superseded_at` historisiert und die exakte aktuelle Zeile verifiziert. Ein fehlender Account gilt dabei nur zusammen mit dem tenantgebundenen, erfolgreichen und unveränderlichen `user.deleted`-Auditnachweis als abgeschlossener Hard Delete; bloße Dateninkonsistenz bleibt fail-closed.

Die Zeilen werden nicht gelöscht. Studio setzt sie auch nicht auf `revoked`, weil eine lokale Account-Löschung keinen externen Widerruf der Mainserver-Credentials beweist. Eine später erneut auftretende widersprechende aktive Evidenz erzeugt deshalb wieder einen Konflikt. Strukturierte Logs unterscheiden `resolved` und `not_resolved` mit technischem Grundcode, DataProvider-ID, Credential-Fingerprint und Anzahl historisierter Zeilen; E-Mail, Benutzername, Secrets und rohe Identity-Antwort bleiben ausgeschlossen.
