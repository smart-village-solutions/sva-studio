...
Der anschließende Fleet-Reconcile gleicht für jede Instanz auch bei unveränderten Aktivierungszeilen die aktuell konfigurierten IAM-Verträge idempotent ab. Reine IAM-Vertragsänderungen werden dadurch auf bestehende Zuweisungen angewendet; entfernte Rollen-/Permission-Paare verlieren ausschließlich ihren als `module_sync` markierten Grant, während manuelle Grants und Permission-Definitionen bestehen bleiben. Im Standard-Deployment gehört diese Fleet-Schleife ausschließlich dem App-Prozess mit der `default`-Lane; der privilegierte Provisioner konfiguriert denselben Snapshot und seine Handler, startet aber keinen zweiten Fleet-Reconcile. Ein fehlgeschlagener oder degradierter Fleet-Lauf plant revisions- und prozessgenerationsgebunden selbstständig einen neuen Versuch; er ist nicht von einem späteren HTTP-Request abhängig.

Fleet-Fehler behalten pro Instanz ausschließlich eine begrenzte Fehlerklasse und die Einordnung `retryable` oder `degraded`; rohe Exception-, SQL- oder Secret-Inhalte sind weder Report- noch Metrikbestandteil. Identische retrybare Fehler werden nach 1, 5 und anschließend höchstens 15 Minuten erneut geprüft. Dauerhaft degradierte Zustände erhalten alle 30 Minuten eine Kontrollprobe. Enthält ein Lauf beide Klassen, bestimmt der kürzere retrybare Takt den nächsten Lauf, damit ein vorübergehender Konflikt nicht durch einen gleichzeitig degradierten Zustand ausgebremst wird. Eine Warnung entsteht nur beim Eintritt, bei einer geänderten Fehlermenge oder beim Wechsel der Backoff-Stufe; die Rückkehr zu `ready` erzeugt genau ein Recovery-Signal. Diese prozesslokale Fleet-Schleife ersetzt keine persistenten Plugin-Lifecycle-Retries und führt keinen zweiten fachlichen Statusspeicher ein.
...
- jeder persistierte Lifecycle-Claim plant in derselben Tenant-Transaktion einen separaten Recovery-Task; bleibt der zugehörige Studio-Job nach einem Prozessabbruch im Zustand `queued`, stellt der Task dessen Graphile-Eintrag idempotent über den stabilen Job-Key wieder her
- im suspendierten Zustand dürfen ausschließlich fällige retryable Wiederholungen der persistierten Operationen `suspend` oder `reactivate` automatisch starten; Provisioning und fachfremde Operationen bleiben gesperrt
