...
| `PATCH /api/v1/mainserver/news/$newsId`  | `content.updateMetadata`, `content.updatePayload` | `createNewsItem(id, forceCreate: false)`    | Updates führen das vorhandene `payload` verlustfrei mit; Zielorte dürfen auch nach dem Versand geändert werden, lösen aber keinen erneuten Push aus.                                                              |
