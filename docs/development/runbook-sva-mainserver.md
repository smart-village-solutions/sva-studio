...
- V2-Updates und -Deletes benötigen die vom Detail-Read gelieferte Kontextbindung; reine Reads wie die Transfer-Autorisierung benötigen sie nicht. Fehlt sie für eine Mutation, lädt der aktuelle Client das Detail einmal vor; bleibt die Bindung aus, wird die Mutation vor dem Provider-Write abgebrochen.
...
| `POST /api/v1/mainserver/news`           | `content.create`                                  | `createNewsItem`                            | `publishedAt` ist verpflichtend; `contentBlocks` sind das führende Inhaltsmodell; `pushNotification` ist nur beim Erstellen erlaubt; `payload.wasteLocationKeys` schränkt ausschließlich die Push-Zustellung ein.            |
| `PATCH /api/v1/mainserver/news/$newsId`  | `content.updateMetadata`, `content.updatePayload` | `createNewsItem(id, forceCreate: false)`    | Updates führen das vorhandene `payload` verlustfrei mit; nach bestätigter Push-Zustellung zeigt der Studio-Editor die gespeicherten Zielorte schreibgeschützt, damit das Payload die historische Empfängergruppe beschreibt. |
...
| `GET /api/v1/mainserver/categories?view=management` | `categories.read`   | `categories(includeInactive)` | Liefert das vollständige Management-Modell; eine ungültige Upstream-Antwort führt zu `category_management_invalid_response`, nie zum Active-only-Fallback. |
