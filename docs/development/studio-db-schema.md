...
- `waste_tour_date_shifts.original_date` und `actual_date` sind zeitzonenfreie PostgreSQL-`DATE`-Werte. Partielle Unique-Indizes erzwingen pro Tour entweder genau eine jahresbezogene Regel für das vollständige Ursprungsdatum oder genau eine jährliche Grundregel für Monat und Tag. Der zentrale Studio-Snapshot bleibt davon unberührt, weil diese Tabelle ausschließlich in der externen Waste-Fachdatenbank liegt.
