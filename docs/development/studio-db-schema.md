...
- `waste_tours.status` ist dort die fachliche Source of Truth mit `draft`, `published` und `archived`; nur `published` ist öffentlich und operativ. Die versionierte Migration `20260915_01_add_waste_tour_status` bildet Legacy-`active = true` auf `published` und `false` auf `draft` ab. Bis Studio und Public-Waste nachweislich umgestellt sind, synchronisiert ein befristeter Trigger beide Schreibrichtungen; danach werden Trigger, Legacy-Spalte und Legacy-Index im Contract-Schritt entfernt. Der zentrale Snapshot `studio-db-schema-final.sql` bleibt davon unberührt.
