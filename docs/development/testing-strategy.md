...
Im GitHub-PR-Pfad startet `Unit Fast Feedback` parallel zum vollständigen Restpfad. Der stabile Required Check `Unit` wird erst durch den finalen Aggregator gesetzt, der beide Head-SHA-gebundenen, disjunkten Evidenzen fail-closed prüft. Die beiden Unit-Shards verwenden pro Workflow-Run stabile Artefaktnamen. Bei einem partiellen GitHub-Rerun ersetzt der erneut ausgeführte Shard nur seine eigene Evidenz; die gültige Evidenz eines bereits erfolgreichen, nicht erneut ausgeführten Shards bleibt für die Aggregation verfügbar. `Coverage` verwendet analog einen stabilen Aggregator über versionierte Projekt-Shards und ein vom Run-Attempt unabhängiges Coverage-Artefakt; fehlende, doppelte, veraltete oder überlappende Reports sind Gate-Fehler.
