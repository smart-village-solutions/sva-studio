...
- jede aktive Advanced-Path-Ausnahme wird in `config/plugin-architecture-allowlist.json` mit Owner, Begruendung und Abbauziel dokumentiert
...
Bestandsfaelle werden nicht stillschweigend akzeptiert. Wenn ein bestehendes Plugin noch nicht auf dem Zielvertrag liegt, muss die Abweichung in `config/plugin-architecture-allowlist.json` stehen. `docs/reports/plugin-architecture-boundary-baseline.md` bleibt nur als Brownfield-Historie und Verweis auf diese Allowlist erhalten. Jede Ausnahme braucht:
