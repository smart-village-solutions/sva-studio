...
## Einmaliger Production-Übergang für ein fehlendes Live-Config-Label

Production verwendet einen eigenen H5-Zwei-Run-Handshake und niemals die Staging-Seed-Modi. Direkte Quantum-, Portainer- oder Service-Label-Mutationen bleiben ausgeschlossen. Beide Läufe sind geschützte `workflow_dispatch`-Promotes mit Production, Standardmodus, unverändertem Live-Digest, `change_base=change_head`, Migration/Bootstrap `assert-none` und Backup-Executor `agent`.

1. Für Prepare `PROMOTE_CONFIG_BUILDER_MODE=shadow` sowie `MAIN_E2E_GATE`, `CANDIDATE_PREFLIGHT_GATE` und `BACKUP_CAPABILITY_GATE` auf `shadow` setzen und den Readback prüfen. `prepare-production-live-config-label` muss die Config-Äquivalenz, Production-Readiness, Same-Digest und den atomaren Labelzustand `missing` attestieren. Staging-Parität, Candidate und Agent-Capabilities laufen lesend und müssen trotz Shadow tatsächlich bestehen. Danach endet der Lauf kontrolliert mit `PROMOTE_RECOVERY_CONTEXT_INVALID`; Backup, Migration, Bootstrap und Deploy bleiben übersprungen.
2. Ohne intervenierenden Promote den Builder auf `authoritative` und alle drei Gates auf `enforce` setzen; jeden Wert erneut lesen. `seed-production-live-config-label` referenziert exakte Run-ID und Attempt des Prepare-Laufs. Workflow-ID und Run-Nummern müssen unmittelbar aufeinanderfolgen. Evidenzartefakt, Gatematrix, Source, Digest, Config-Revision, aktueller Run und Live-Snapshot werden vor sowie unmittelbar vor Deploy erneut gebunden.
3. Nur bei weiterhin fehlendem Label ergänzt ein controller-eigenes Production-Overlay ausschließlich `sva.config.revision`. Ein struktureller Rendervergleich verbietet jede weitere Stack-Abweichung. Der Seed führt frische verifizierte Studio-/Waste-Backups, Candidate, Agent-Capability, Deploy, Konvergenz, Production-Smoke und gemeinsamen Digest-/Config-Readback aus. Er erzeugt keine Staging-Parität und erfindet keine vorherige Revision oder Rollback-Evidenz.
4. Erst nach dem erfolgreichen Same-Digest-Seed darf der reguläre Production-Rollout des bereits durch Staging-Parität gebundenen Ziel-Digests erfolgen. Migrations- und Bootstrap-Modus werden aus der tatsächlichen Live-Source-Grenze abgeleitet; für den derzeitigen H5-Übergang sind beide explizit `run`. Required Reviewer, Backups, v2-Parität, Enforce-Gates, Postconditions, Production-Smoke und finaler gemeinsamer Readback bleiben blockierend.

Jede Shadow-Abweichung, ein fehlgeschlagenes lesendes Gate, ein vorhandenes oder ungültiges Label, ein Run-Nummern-Gap, ein geändertes Artefakt oder ein Live-Digest-Drift ist eine Stop-Bedingung. Prepare oder Seed dürfen dann nicht generisch wiederholt werden; der aktuelle Zustand muss neu bewertet und ein neuer expliziter Plan freigegeben werden.
