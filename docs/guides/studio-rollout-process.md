...
Ist `SSF_PLUGIN_DATABASE_ENABLED=true`, bereitet derselbe Migrations-One-shot zusätzlich `sva_studio_ssf` vor, wendet ausschließlich die im Image enthaltenen Migrationen aus `packages/plugin-ssf/migrations` an und reconciliert den minimalen Login-Principal `sva_ssf_runtime`. Datenbankname, Principal und Secret werden über den bestehenden autoritativen Remote-Konfigurationsvertrag geliefert; die laufende App erhält nur `SVA_STUDIO_SSF_DATABASE_URL`. Die getrackten Profile setzen sowohl Datenbank- als auch Runtime-Freigabe auf `false`. Eine Aktivierung erfordert deshalb eine bewusste Konfigurationsänderung und bleibt bis zur verifizierten IAM-Projektionsrevision fachlich fail-closed.
