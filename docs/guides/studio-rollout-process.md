...
Der isolierte Candidate-One-shot läuft blockierend mit dem Zielimage vor Backup, Migration, Bootstrap und Deploy. Er besitzt kein Ingress, kein schreibbares Dateisystem und keine Linux-Capabilities. Seine Datenbankverbindung wird ausschließlich in einer `READ ONLY`-Transaktion verwendet. Geprüft werden Runtime-Profil, externe Secret-Mounts, alle aktiven Tenant-Datensätze der Registry sowie die Entschlüsselbarkeit ihrer Secrets. Das erfolgreiche Pullen und Starten des temporären Stacks attestiert zugleich die Registry-Lesbarkeit des Zielimages. Der Stack wird nach terminalem Erfolg oder Fehler entfernt.
