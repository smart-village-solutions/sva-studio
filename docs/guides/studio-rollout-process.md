...
- Vor jedem Deployment nach Staging oder Production muss ein erfolgreich verifiziertes PostgreSQL-Backup vorliegen. Sobald `WASTE_POSTGRES_BACKUP_ENABLED=true` gesetzt ist, gilt dieses Gate getrennt für `sva_studio` und das vollständige Registry-Inventar aller `ready`- oder `disabled`-Waste-Tenant-Datenbanken. Sobald `SSF_POSTGRES_BACKUP_ENABLED=true` gesetzt ist, wird zusätzlich die getrennte Datenbank `sva_studio_ssf` als eigenes Ziel gesichert und verifiziert.
...
3. Signierten Backup-Auftrag an den Staging-Agenten senden; bei aktiviertem Waste- beziehungsweise SSF-Backup anschließend getrennte Aufträge mit `database: "waste"` beziehungsweise `database: "ssf"` ausführen. Der Agent entdeckt das vollständige Waste-Tenant-Inventar selbst und behandelt die installationsweite SSF-Plugin-Datenbank als separates statisches Ziel.
...
Ist `SSF_PLUGIN_DATABASE_ENABLED=true`, bereitet derselbe Migrations-One-shot zusätzlich `sva_studio_ssf` vor, wendet ausschließlich die im Image enthaltenen Migrationen aus `packages/plugin-ssf/migrations` an und reconciliert getrennte Login-Principals für Tenant-Runtime und Root-Administration. Datenbanknamen, Principals und Secrets liefert der bestehende autoritative Remote-Konfigurationsvertrag; die laufende App erhält `SVA_STUDIO_SSF_DATABASE_URL` für Laufzeit-Lesezugriffe und `SVA_STUDIO_SSF_ROOT_DATABASE_URL` ausschließlich für Systemkonfigurations-Schreibzugriffe. Die getrackten Profile setzen sowohl Datenbank- als auch Runtime-Freigabe auf `false`. Eine Aktivierung erfordert deshalb eine bewusste Konfigurationsänderung und bleibt bis zur verifizierten IAM-Projektionsrevision fachlich fail-closed.
...
Der isolierte Candidate-One-shot läuft blockierend mit dem Zielimage vor Backup, Migration, Bootstrap und Deploy. Er besitzt kein Ingress, kein schreibbares Dateisystem und keine Linux-Capabilities. Seine Datenbankverbindung wird ausschließlich in einer `READ ONLY`-Transaktion verwendet. Geprüft werden Runtime-Profil, erforderliche externe Secret-Mounts und Datenbankverbindung. Tenant-Datensätze und deren Secrets gehören zur Tenant-Verwaltung und sind keine Release-Voraussetzung. Der Stack wird nach terminalem Erfolg oder Fehler entfernt.
...
Vor dem ersten Agent-Auftrag fragt der Workflow blockierend den geschützten read-only Capability-Endpunkt ab. Protokollversion 2, Agent-Revision, benötigte Ergebnisfelder sowie Studio- und gegebenenfalls Waste- und SSF-Unterstützung müssen live vorhanden sein. Backups für Staging und Production laufen ausschließlich über den zentralen Backup-Agenten. Vor dem ersten Promote mit aktiviertem SSF-Backup muss deshalb zunächst der kompatible Backup-Agent über seinen bestehenden geschützten Rollout aktualisiert und live nachgewiesen sein.

Docker-Swarm-Dienste dürfen nach einem Update längere Zeit benötigen, bis alle Probes stabil sind. Die bisherige HTTP-Warmup-Grenze von bis zu fünf Minuten reicht dafür nicht immer aus. Swarm-Konvergenz und HTTP-Warmup sind deshalb getrennte Gates: Zuerst pollt `Promote` ausschließlich allowlistete Service-, Replica-, Update- und Task-Zustände. Erst wenn alle gewünschten Replicas laufen und kein Update mehr aktiv oder pausiert ist, beginnt der Runtime-Smoke mit standardmäßig bis zu 50 Erreichbarkeitsprüfungen im Abstand von zehn Sekunden. Vorübergehende `403`-, `404`- und Gateway-Antworten auf den bekannten Root-, Health-, Login- und IAM-Probes gelten innerhalb dieses begrenzten Fensters als noch nicht terminal. Falsche Realm- oder Callback-Verträge und unerwartete Inhalte bleiben dagegen sofort blockierend. Deshalb gilt:
...
3. In Production danach `health/live`, `health/ready`, den Root-Login-Redirect und den Live-Digest erneut prüfen. Tenant-Redirects bleiben eine operative Prüfung, blockieren aber keinen App-Promote.
...
Ein regulärer Production-Rollout ist nur erfolgreich, wenn der GitHub-Workflow grün ist, der erwartete Digest live läuft und die Root-Probes einschließlich `live`, `ready` und Login bestehen. Tenant-Smokes sind operative Signale und keine Release-Blocker.
...
- Studio, die SSF-Plugin-Datenbank und das dynamische Waste-Inventar bleiben getrennte Backup-/Restore-Ziele. Der kontrollierte Restore-Workflow akzeptiert für das statische SSF-Ziel ausschließlich den Modus `ssf` und Objektpfade unter `<umgebung>/ssf/`; Tenantparameter sind dort unzulässig.
