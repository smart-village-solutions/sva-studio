...
- Vor jedem Deployment nach Staging oder Production muss ein erfolgreich verifiziertes PostgreSQL-Backup vorliegen. Sobald `WASTE_POSTGRES_BACKUP_ENABLED=true` gesetzt ist, gilt dieses Gate getrennt für `sva_studio` und `sva_waste`.
...
3. Signierten Backup-Auftrag an den Staging-Agenten senden; bei aktivierter Waste-Datenbank anschließend einen zweiten, auf `database: "waste"` und das Waste-Präfix gebundenen Auftrag ausführen.
