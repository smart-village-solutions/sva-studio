...
- Ein App-Rollback ist ausschließlich für das Paar aus vorherigem unveränderlichem Digest und der exakt dazugehörigen versionierten nicht-sensitiven Config-Revision zulässig. Der erfolgreiche Promote bindet diese Revision über das Service-Label `sva.config.revision` an den laufenden App-Service; nur diese Live-Bindung darf den Rollback speisen.
