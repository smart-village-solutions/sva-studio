...
| Benutzerprofil       | Die Attribute `instanceId`, `mainserverUserApplicationId`, `mainserverUserApplicationSecret` sowie die Ownership-Attribute `managed_by`, `instance_id` und `artifact_key` sind administrativ geschützt.                         | Bei neuen Realms automatisch additiv ergänzen; fremde und Keycloak-eigene Attribute bleiben erhalten.                                                                                                      | Blockierend für den automatischen Baseline-Abschluss.                                                                  |
...
Neue Realms deklarieren diese drei Attribute vor der Benutzeranlage in der bestehenden Benutzerprofil-Baseline als einwertig und ausschließlich für Administratoren les- und schreibbar. Keycloak ignoriert bei der Standardrichtlinie nicht deklarierte Attribute; siehe [Keycloak: Managed and Unmanaged Attributes](https://www.keycloak.org/docs/latest/server_admin/#_user-profile). Die allgemeine Richtlinie für nicht deklarierte Attribute bleibt unverändert. Der Baseline-Fingerprint ändert sich; zuvor bestätigte Aufträge benötigen daher eine aktuelle Planung.
