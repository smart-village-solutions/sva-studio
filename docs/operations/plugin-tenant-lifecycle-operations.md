...
Der vorgelagerte Aktivierungsrichtlinien-Fleet-Reconcile exportiert ergänzend `sva_plugin_activation_policy_fleet_state`, `sva_plugin_activation_policy_fleet_failure_count` und `sva_plugin_activation_policy_fleet_seconds_since_success`. `state` ist auf `unknown`, `ready`, `retrying` und `degraded` begrenzt; Fehler werden ausschließlich nach dem erlaubten `reason_code` und `retry_class` aggregiert. Instanz-IDs erscheinen weiterhin nur im redigierten Zustandswechsel-Log, nicht als Metriklabel. `retrying` verwendet 1/5/15 Minuten Backoff, `degraded` eine 30-minütige Kontrollprobe. Wiederholt identische Zwischenläufe erzeugen keine neue Warnung.
