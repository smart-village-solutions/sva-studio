...
- Der Keycloak-Worker persistiert den nach der Mutation in einem konsistenten Read gelesenen Status als `status_snapshot` im Provisioning-Run. Preflight und Plan bleiben getrennte Worker-Snapshots; der Abschluss ergänzt sie nicht durch unabhängige Folgeabfragen. Der MCP bewertet den Abschluss zusätzlich anhand des aktuellen Detail-Reads.
