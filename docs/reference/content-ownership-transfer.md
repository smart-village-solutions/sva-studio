...
Fehlende Permission, ungültige Ziel-Principals, fehlende Ziel-Credentials, eine nicht erreichbare Zielidentität und widersprüchliche Zielbindungen werden vor dem Provider-Write abgelehnt. Der aktuelle Quell-DataProvider stammt aus einem frischen Read mit den Credentials des autorisierten Actors. Eine eindeutige Studio-Principal-Bindung des bisherigen Inhabers dient nur der Anzeige und dem Audit und ist mit Scope `all` keine Vorbedingung. Ohne auflösbare Bindung wird der Scope `all` direkt am effektiven Permission-Snapshot geprüft; Kompatibilitäts- und Shadow-Modi dürfen einen engeren Scope dabei nicht erweitern. Das Audit unterscheidet eine regulär fehlende Bindung (`unresolved`) von einem technischen Fehler der Anreicherung (`failed`). Ist der Ausgang nach einem Timeout weder über Ziel- noch Actor-Credentials eindeutig feststellbar, meldet das Studio `reconciliation_required`; der Vorgang darf dann nicht als sicher fehlgeschlagen wiederholt werden.
