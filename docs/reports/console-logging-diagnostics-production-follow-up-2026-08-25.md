...
Die erneute Loki-Abfrage begann erst nach dem Production-Umschaltzeitpunkt `2026-08-25T21:50:30Z`. Die durch den Bedien-Smoke erzeugte Stichprobe enthielt 102 `SVA Mainserver GraphQL operation succeeded`-Ereignisse, einen `tenant_auth_callback` und sieben `audit_event`-Ereignisse. Alle 110 Ereignisse besaßen eine Request-ID; keine vorhandene Trace-ID war ungültig. Die Mainserver-GraphQL-Erfolge erschienen ausschließlich unterhalb von `info`. Alte Callback-Doppellogs, korrelationslose HTTP-Ereignisse und unredigierte `projection_scope_key`-Werte wurden nicht beobachtet.

Im kurzen Nachlauf trat keine natürliche Mainserver-Mutation und kein Jobabschluss auf; hierfür wird daher kein neues Production-Ereignis behauptet. Der Korrektur-PR `#1145` änderte an Produktionscode ausschließlich den gemeinsamen Request-Kontext-Carrier und dessen Reproduktionstest; die bereits zuvor in Production beobachteten Mutation-, Audit- und Jobabschluss-Emitter blieben quellenidentisch. Ihr Erhalt wird deshalb aus der ursprünglichen Production-Stichprobe, dem unveränderten Emissionscode, den weiterhin grünen Vertrags- und Canary-Tests, dem erfolgreichen App-Build und der verifizierten Same-Digest-Auslieferung zusammengesetzt. Der konkrete Carrier-Defekt ist zusätzlich durch 102 post-deployte Mainserver-Ereignisse mit vollständiger Request-Korrelation und den im ausgelieferten Artefakt enthaltenen Cross-Bundle-Reproduktionstest geschlossen. Die bewusst nicht künstlich erzeugte Mutation-/Job-Stichprobe ist eine dokumentierte Beobachtungsgrenze, kein behauptetes Einzelereignis.

Damit liegen für den pragmatischen Abschluss kombinierte Artefakt-, Test-, Same-Digest-Rollout- und Live-Transportnachweise vor. Es besteht kein Rollback-Signal; der zuvor gestoppte Korrelations-Slice ist freigegeben. Ein reproduzierbarer Post-Rollout-Canary für Mutation und Jobabschluss wird als nicht blockierendes Follow-up in Issue `#1156` geführt.
