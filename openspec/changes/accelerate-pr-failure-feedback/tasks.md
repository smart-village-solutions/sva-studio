...
- **A2 – Staging-Consumer:** `implementation-complete, activation pending`; integrierter Implementierungs-HEAD: `b536b613f2874370e282cfec89279387cfb88f00` (Merge von `origin/main` `5652f4a1ea6d4225d8d4f5c5d6385feb17c20e42`); der strikt SHA-/Event-/Branch-/Run-/Attempt-gebundene Consumer liegt read-only vor Backup und Remote-Mutation, filtert den GitHub-Run-Lookup serverseitig auf Main-Push und exaktes Head-SHA, nutzt die H1-Fehler- und Gate-Evidenz einschließlich allowlisteter Main-E2E-Referenz und bleibt ohne explizites `MAIN_E2E_GATE=shadow|enforce` deaktiviert. Disabled und Shadow erhalten die bestehende Legacy-Parität; erfolgreiche Shadow-Auswertung kann zusätzlich v2-Parität mit Attestation erzeugen, während erst Enforce v2 zwingend verlangt. Die Konfliktauflösung erhält zugleich die neuen Worker-Datenbank-Secret-, Injection- und Readiness-Verträge aus Main und bindet die beiden Worker-Secret-Schritte an die kanonische Promote-Evidenz. Gates: `tooling-testing:test:unit` mit fünf expliziten E2E-/Promote-/Workflow-Vertragstestdateien (119 Tests), fünf expliziten Worker-/Readiness-Vertragstestdateien (26 Tests) und dem isolierten Repro zweier PR-fremder Timeout-Dateien (19 Tests), `pnpm exec tsc -p tsconfig.scripts.json --noEmit`, `pnpm nx run tooling-testing:lint`, Prettier-Check, YAML-Parse, `pnpm check:file-placement`, Diff-Check sowie beide strikten OpenSpec-Validierungen grün. `pnpm test:pr` wurde im Coverage-Teil nach zwei nicht reproduzierbaren 5-Sekunden-Lasttimeouts abgebrochen; zu diesem Zeitpunkt waren in `tooling-testing` 90 Testdateien und 827 Tests grün, und beide Timeout-Dateien liefen unmittelbar danach isoliert mit 19 von 19 Tests grün. Der Gesamtgate-Lauf wird deshalb nicht als grün behauptet und Task 8.4 bleibt offen. Tasks 0.4, 6.1 und 6.2 bleiben bis zur Shadow-Auswertung, blockierenden Aktivierung und ihrem operativen Nachweis bewusst offen.
- **A3 – Shadow und Aktivierung:** `ready`; Basis-HEAD: `b536b613f2874370e282cfec89279387cfb88f00`; nächster Block: ausschließlich 7.3 mit expliziter Shadow-Aktivierung, dokumentierter Event-/Branch-/SHA-/OCI-Kettenparität und anschließender blockierender Aktivierung; erst deren Nachweis darf 0.4, 6.1, 6.2 und 7.3 abschließen.
