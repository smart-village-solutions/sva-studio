...
- **A2 – Staging-Consumer:** `implementation-complete, activation pending`; Implementierungs-HEAD: `6d2a2768107a2b0493a3639f4bd7891d9621c708`; der strikt SHA-/Event-/Branch-/Run-/Attempt-gebundene Consumer liegt read-only vor Backup und Remote-Mutation, nutzt die H1-Fehler- und Gate-Evidenz und bleibt ohne explizites `MAIN_E2E_GATE=shadow|enforce` deaktiviert. Fehlende oder ungültige Evidenz erzeugt bei späterer Shadow-Auswertung keine production-fähige Staging-Parität; Production verlangt die validierte E2E-Attestation und Source-SHA im Staging-Paritätsartefakt. Gates: `tooling-testing:test:unit` mit fünf expliziten E2E-/Promote-/Workflow-Vertragstestdateien (115 Tests), `pnpm exec tsc -p tsconfig.scripts.json --noEmit`, `pnpm nx run tooling-testing:lint`, Prettier-Check, YAML-Parse, `pnpm check:file-placement`, Diff-Check sowie beide strikten OpenSpec-Validierungen grün. Tasks 0.4, 6.1 und 6.2 bleiben bis zur Shadow-Auswertung, blockierenden Aktivierung und ihrem operativen Nachweis bewusst offen.
- **A3 – Shadow und Aktivierung:** `ready`; Basis-HEAD: `6d2a2768107a2b0493a3639f4bd7891d9621c708`; nächster Block: ausschließlich 7.3 mit expliziter Shadow-Aktivierung, dokumentierter Event-/Branch-/SHA-/OCI-Kettenparität und anschließender blockierender Aktivierung; erst deren Nachweis darf 0.4, 6.1, 6.2 und 7.3 abschließen.
