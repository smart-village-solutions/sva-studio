...
Die serverseitig paginierte Zielauswahl trennt „Persönliche Accounts“ und „Organisationen“ innerhalb eines gemeinsamen Suchfelds. Organisationen werden über ihre nicht verschlüsselte Anzeige in der IAM-Datenbank gesucht. Für persönliche Accounts delegiert der Server die Namens- und E-Mail-Suche an den tenantgebundenen Keycloak-Provider, statt verschlüsselte Studio-Felder zu durchsuchen oder neue PII-Suchindizes einzuführen. Keycloak-Treffer werden über ihr Subject mit der lokalen Projektion derselben Instanz geschnitten; nur aktive, nicht technische und fachlich zulässige Principals gelangen in den Zielkatalog. Die externen Keycloak-Aufrufe laufen außerhalb tenantlokaler Datenbanktransaktionen; nur die jeweilige lokale Subject-Projektion hält kurzzeitig eine scoped Verbindung. Unbekannte, instanzfremde, inaktive oder technische Treffer sowie der aktuelle Inhaber werden ausgeschlossen. Ist der Keycloak-Provider nicht verfügbar, schlägt der Account-Zweig erkennbar fehl und wird nicht als leere Treffermenge ausgegeben. Erreicht eine breite Suche die feste Scan-Grenze, bleiben bereits bestätigte Treffer sichtbar und die bestehende Pagination fordert über ihr Weitere-Ergebnisse-Signal zu einem engeren Suchbegriff auf.

Die Oberfläche benötigt für Kandidatenlisten keine garantierte exakte Gesamtzahl, solange Pagination und Suchverfeinerung keine verfügbaren Treffer als abschließend nicht vorhanden darstellen. Jeder Treffer besitzt eine textliche Typkennzeichnung und einen Anzeigenamen; technische DataProvider-IDs und E-Mail-Adressen werden nicht angezeigt. Konfliktbehaftete oder credential-lose Principals sind nicht auswählbar. Kandidaten mit verwendbaren Credentials, aber noch fehlender Bindung, kennzeichnet die Oberfläche verständlich als Ziel, dessen DataProvider-Zuordnung beim bestätigten Transfer sicher geprüft wird. Die Suche löst keine Mainserver-Identity-Prüfung je Treffer aus; die bestehende anlassbezogene Binding-Prüfung bleibt ausschließlich Teil der bestätigten Übertragung.
