...
Provider liefern nur Nachrichten, die im aktuellen Instanz-/Accountkontext grundsätzlich sichtbar sein dürfen. Jede `id` ist providerübergreifend eindeutig und folgt verbindlich dem Schema `<provider-id>:<provider-local-id>`; `source` muss exakt dem Provider-Präfix entsprechen. Die Feed-Orchestrierung verwirft nicht konforme oder kollidierende Antworten, dedupliziert ausschließlich anhand der vollständigen globalen ID, führt den Gelesen-Stand zusammen und begrenzt das Ergebnis serverseitig. Metadatenindex, API-Ziele und Gelesen-Belege verwenden dieselbe vollständige ID. Die Orchestrierung darf Inhalte nicht anhand von Clientangaben autorisieren.
...
Neue Changelog-Dokumente erhalten einen validierten RFC-3339-Zeitpunkt `publishedAt`. Bestehende Dokumente bleiben als Legacy-Einträge lesbar. Die bei Einführung im ausgelieferten 20er-Katalog enthaltenen Einträge werden einmalig mit einem belegten Veröffentlichungszeitpunkt ergänzt. Innerhalb verbleibender Legacy-Einträge bleibt die PR-Nummer die deterministische Reihenfolge; neue providerübergreifende Nachrichten benötigen immer `publishedAt`. Zusätzlich zum auf 20 Einträge begrenzten Anzeige-Artefakt erzeugt der Build einen vollständigen, serverseitig verfügbaren Metadatenindex aller belegten Changelog-Einträge mit ausschließlich stabiler ID und `publishedAt`. Dieser Index wird unabhängig von der sichtbaren Feed-Begrenzung in das Runtime-Artefakt aufgenommen. Jeder weitere Provider muss denselben dauerhaften Metadaten-Lookup für jemals belegfähige IDs bereitstellen; das Entfernen aus dem sichtbaren Katalog darf den Lookup nicht entfernen.
...
Alle Antworten mit accountbezogenen Daten setzen einen privaten `no-store`-Cachevertrag. Fehler verwenden stabile Codes und enthalten weder Token noch Nachrichteninhalte. Der Browser kann die Endpunkte über die bestehende `httpOnly`-Session nutzen; der native Client verwendet Bearer-Token ausschließlich für Summary, Liste und Browser-Übergabe. Beide Transportarten werden vor der jeweils erlaubten Fachlogik in denselben instanzgebundenen Identity-Kontext überführt. Cookie-authentifizierte `POST`-Requests müssen zusätzlich den bestehenden CSRF- und erlaubten Origin-Vertrag erfüllen. Bearer-authentifizierte native Requests verwenden stattdessen ausschließlich die Tokenbindung, dürfen weder den Gelesen-Stand verändern noch auf eine beigefügte Browser-Session zurückfallen; mehrdeutige Cookie-/Bearer-Requests werden fail-closed abgelehnt.
...
Eine validierte, konfigurierbare Aufbewahrungsfrist begrenzt Gelesen-Belege standardmäßig auf 365 Tage. Der Nachrichtenfeed darf keine Nachricht ausliefern, deren belegter `publishedAt` außerhalb derselben Frist liegt. Vor dem Löschen eines Belegs löst die periodische Bereinigung dessen Nachrichtenzeitpunkt über den dauerhaften Provider-Metadatenindex auf und schreibt eine persistierte, pro Instanz monotone Sichtbarkeitsgrenze mindestens bis zum neuesten `publishedAt` aller in diesem Lauf gelöschten Belege fort. Der Lookup bleibt auch nach Rotation aus dem sichtbaren 20er-Katalog verfügbar. Kann der Zeitpunkt wegen eines beschädigten oder vertragswidrigen Indexeintrags nicht eindeutig aufgelöst werden, bleibt der Beleg erhalten und der Lauf meldet den Datensatz zur Governance-Nachbearbeitung. Der Feed liefert nur Nachrichten aus, deren `publishedAt` sowohl innerhalb der aktuellen Frist als auch strikt nach dieser Sichtbarkeitsgrenze liegt. Dadurch kann eine spätere Verlängerung der Frist die Grenze nicht zurücksetzen und bereits ausgeblendete Nachrichten nicht ohne Beleg als ungelesen zurückbringen. Der Wert gilt für Feed und Belege gemeinsam, Änderungen und Grenzfortschreibungen werden wie andere DSR-Löschfristen auditiert; eine Verringerung wirkt erst nach erfolgreichem Dry Run und expliziter Freigabe. Legacy-Einträge ohne belegten Zeitpunkt sind nur dann Teil des neuen Feeds, wenn sie vor Einführung der Frist mit einem belegten `publishedAt` nachgetragen wurden.

Membership-Entzug beendet jeden Studio-Zugriff sofort, nicht nur Feed- und Belegzugriff. Dazu erweitert die Migration `iam.instance_memberships` um einen nullable Widerrufszeitpunkt `revoked_at`; sämtliche Membership-Joins, RLS-/Autorisierungspfade, effektiven Session-Rollen und normalen Feed-/Belegabfragen akzeptieren ausschließlich Datensätze mit `revoked_at IS NULL`. Der Widerruf invalidiert synchron alle betroffenen Permission-, Rollen- und Session-Snapshots beziehungsweise Caches; jeder nachfolgende Request prüft den aktiven Membership-Zustand erneut. Ohne aktiven Legal Hold löscht der Governance-Workflow Membership und Belege physisch. Unter Legal Hold setzt er `revoked_at` samt Invalidierung atomar, lässt die durch `legal_holds_account_membership_fk ... ON DELETE RESTRICT` geschützte Membership bestehen und merkt ihre physische Löschung vor. Membership und Belege bleiben instanz- und accountgebunden, für normale Studio-Requests unsichtbar und ausschließlich für autorisierte DSR-/Governance-Prozesse zugänglich, bis der Hold aufgehoben und die ausstehende Löschung nachgeholt wird. Die endgültige Accountlöschung folgt derselben bestehenden Legal-Hold-Sperre. Damit kann weder ein FK-Cascade noch eine periodische Bereinigung gehaltene Belege umgehen.
...
- ausschließlich `studio.messages.read`.

Access- und Refresh-Token werden ausschließlich in einer minimalen Keychain-Access-Group gespeichert, die Container-App und Widget Extension teilen. Diese geteilten Credentials sind strikt read-only; insbesondere akzeptiert `POST /api/v1/account/messages/read` keine native Bearer-Identität. Gelesen-Markierungen erfolgen ausschließlich über die vorhandene Browser-Session nach erfolgreicher Darstellung. Die Credentials werden nie in `UserDefaults`, App-Group-Dateien, Logs, Crash-Metadaten oder Telemetrie geschrieben. Die Anwendung zeigt keine Keycloak-Seite in einem kontrollierbaren eingebetteten WebView.
...
- Unit-Tests für Provider-Normalisierung, Deduplizierung, Sortierung, Klartextauszug, Limits, vollständigen dauerhaften Metadaten-Lookup und Gelesen-Zusammenführung.
...
- PostgreSQL-16-Migrationstest Up/Down/Up sowie RLS-, Membership-FK-, Soft-Revoke-, Upsert- und Query-Plan-Nachweise.
...
- DSR-Tests für vollständigen Selbst-/Adminexport der Gelesen-Belege, konfigurierbare Frist, Dry Run, Legal Hold, monotone Sichtbarkeitsgrenze, Fristverlängerung nach Bereinigung und fristgerechte Bereinigung.
