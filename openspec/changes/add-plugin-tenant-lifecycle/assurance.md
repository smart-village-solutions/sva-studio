...
| OBS-01     | echter PostgreSQL + Metrik-/Alert-/Runbook-Test | `scripts/ci/verify-plugin-lifecycle-database-contract.ts`, `packages/auth-runtime/src/plugin-tenant-lifecycle/observability.test.ts`, `packages/monitoring-client/tests/plugin-lifecycle-metric-export.test.ts`, `tooling/testing/tests/plugin-lifecycle-alerts.test.ts` und [Betriebsrunbook](../../../docs/operations/plugin-tenant-lifecycle-operations.md) | Claim-, Fälligkeits-/Wake-up-, Generation-/Intent- und Lane-Störung   | Der parameterlose DB-Vertrag liefert nur bounded Reason/Count. Metriken verwenden ausschließlich Lane, Status und sichere Reason-Codes; IDs erscheinen nur redigiert in Logs/Traces oder berechtigten Diagnosequeries. Fleet-Aggregate werden pro Reason mit `max` dedupliziert, Lane-Zustände pro eigener Lane ausgewertet; Critical-Alert innerhalb 120 s. | `pnpm nx run auth-runtime:test:integration && pnpm nx run auth-runtime:test:unit --testFiles=src/plugin-tenant-lifecycle/observability.test.ts --testFiles=src/plugin-operations/runner-worker.test.ts && pnpm nx run monitoring-client:test:unit --testFiles=tests/plugin-lifecycle-metric-export.test.ts && pnpm nx run tooling-testing:test:unit --testFiles=tests/plugin-lifecycle-alerts.test.ts && pnpm check:docs` |
