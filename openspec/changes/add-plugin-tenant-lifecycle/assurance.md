...
- **Diagnosezeit**: Ein nicht konvergierender Zustand muss spätestens nach 120
  Sekunden im sicheren Aggregatsnapshot erkennbar sein. Die automatische
  Alert-Zustellung wird erst mit dem produktiven OTEL-Pfad aus
  [Issue #1237](https://github.com/smart-village-solutions/sva-studio/issues/1237)
  Teil dieses Zeitbudgets. Ein externer Seiteneffekt darf länger dauern, muss
  aber Heartbeat und Fencing fortschreiben.
...
| TOP-01  | Default- und privilegierte Lane werden anhand der hostvalidierten Jobregistrierung über getrennte Task-Identifier und bei serieller Konkurrenz über getrennte Queues geroutet; sie erholen sich ohne eingehenden HTTP-Request. |
...
| TOP-01     | echtes PostgreSQL + Graphile-Prozesstest | `scripts/ci/verify-plugin-lifecycle-database-contract.ts`, `tooling/testing/fixtures/plugin-lifecycle-worker-process.ts`, `packages/auth-runtime/src/plugin-operations/runner-worker.test.ts`, `apps/sva-studio-react/src/lib/plugin-operation-runtime.server.test.ts`, `apps/sva-studio-react/src/server.test.ts` und `deploy/portainer/waste-provisioner-deployment.test.ts` | falsche Lane, seriell blockierte Default-Queue, fataler Prozessabbruch und expliziter Shutdown | Default claimt keinen privilegierten Job und eine blockierte Default-Queue verhindert keinen privilegierten Versuch; ein fataler Worker wird nach fail-closed Health und Pool-Retirement mit Fehlerstatus beendet, von Swarm begrenzt neu gestartet und konsumiert den persistenten Job ohne HTTP-Request; expliziter Shutdown beendet sich ohne Neustart                                                        | `pnpm nx run auth-runtime:test:integration && pnpm nx run auth-runtime:test:unit --testFiles=src/plugin-operations/runner-worker.test.ts && pnpm nx run sva-studio-react:test:unit:server --testFiles=src/lib/plugin-operation-runtime.server.test.ts && pnpm nx run sva-studio-react:test:unit --testFiles=src/server.test.ts && pnpm nx run tooling-testing:test:unit --testFiles=../../deploy/portainer/waste-provisioner-deployment.test.ts` |
...
| OBS-01     | echter PostgreSQL + Metrik-/Runbook-Test | `scripts/ci/verify-plugin-lifecycle-database-contract.ts`, `packages/auth-runtime/src/plugin-tenant-lifecycle/observability.test.ts`, `packages/monitoring-client/tests/plugin-lifecycle-metric-export.test.ts` und [Betriebsrunbook](../../../docs/operations/plugin-tenant-lifecycle-operations.md)                                                                          | Claim-, Fälligkeits-/Wake-up-, Generation-/Intent- und Lane-Störung                            | Der parameterlose DB-Vertrag liefert nur bounded Reason/Count. Metriken verwenden ausschließlich Lane, Status und sichere Reason-Codes; IDs erscheinen nur redigiert in Logs/Traces oder berechtigten Diagnosequeries. Fleet-Aggregate werden pro Reason mit `max` dedupliziert und Lane-Zustände pro eigener Lane ausgewertet. Aktive Alerts bleiben bis zum produktiven OTEL-Pfad aus Issue #1237 deaktiviert. | `pnpm nx run auth-runtime:test:integration && pnpm nx run auth-runtime:test:unit --testFiles=src/plugin-tenant-lifecycle/observability.test.ts --testFiles=src/plugin-operations/runner-worker.test.ts && pnpm nx run monitoring-client:test:unit --testFiles=tests/plugin-lifecycle-metric-export.test.ts && pnpm check:docs`                                                                                                                   |
