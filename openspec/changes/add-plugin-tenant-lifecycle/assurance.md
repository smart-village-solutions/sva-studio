...
| TOP-01     | echtes PostgreSQL + Graphile-Prozesstest        | `scripts/ci/verify-plugin-lifecycle-database-contract.ts`, `tooling/testing/fixtures/plugin-lifecycle-worker-process.ts`, `packages/auth-runtime/src/plugin-operations/runner-worker.test.ts`, `apps/sva-studio-react/src/server.test.ts` und `deploy/portainer/waste-provisioner-deployment.test.ts`                                                          | falsche Lane, fataler Prozessabbruch und expliziter Shutdown          | Default claimt keinen privilegierten Job; ein fataler Worker wird nach fail-closed Health und Pool-Retirement mit Fehlerstatus beendet, von Swarm begrenzt neu gestartet und konsumiert den persistenten Job ohne HTTP-Request; expliziter Shutdown beendet sich ohne Neustart                                                                               | `pnpm nx run auth-runtime:test:integration && pnpm nx run auth-runtime:test:unit --testFiles=src/plugin-operations/runner-worker.test.ts && pnpm nx run sva-studio-react:test:unit --testFiles=src/server.test.ts && pnpm nx run tooling-testing:test:unit --testFiles=../../deploy/portainer/waste-provisioner-deployment.test.ts`                                                                                       |
