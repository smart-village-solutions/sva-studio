# System-Assurance: SSF-IAM-Permission-Projektion

## Kritische Invarianten

| ID           | Invariante                                                                                                        | Führender Nachweis                                                                                  |
| ------------ | ----------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| `SSF-IAM-01` | Eine Sollprojektion oder ein Cachewert wird nie als bestätigte `authorizationRevision` veröffentlicht.            | Repository-Unit-Test und PostgreSQL-Integrationstest                                                |
| `SSF-IAM-02` | Readiness setzt identische Soll- und Read-back-Revisionen derselben Generation sowie erneute Client-Aktivierung voraus. | DB-Constraint, Repository-CAS und Reconciler-Unit-Test                                         |
| `SSF-IAM-03` | Projektion, Read-back und Retry bleiben tenantgebunden; ein Tenant beeinflusst keinen zweiten.                    | RLS-Policy, tenantgebundene Advisory-Lock und Zwei-Tenant-Tests                                     |
| `SSF-IAM-04` | Tenant-Benutzertokenclaim, bestätigte Projektion und Runtime-Antwort verwenden dieselbe Revision.                 | Tokenprojektions-, Runtime-Host- und SSF-Session-Vertragstests                                      |
| `SSF-IAM-05` | Studio und SSF verwenden im Tenant-Realm dieselbe Benutzeridentität und dasselbe unveränderte OIDC-`sub`.         | Vertragstest des Tenant-Clients und Staging-Token                                                   |
| `SSF-IAM-06` | Alte SSF-Rechte wirken durch die maximale Access-Token-Laufzeit höchstens 15 Minuten nach.                        | Keycloak-/SSF-Konfiguration und Staging-Tokennachweis                                               |
| `SSF-IAM-07` | Studio→SSF verwendet eine eigene technische Identität und niemals den gegenläufigen SSF→Studio-Runtime-Client.    | Deploymentkonfiguration, Tokenprovider- und Transport-Vertragstest                                  |

## Failure Modes

- Write- oder Read-back-Fehler setzen die exakte Generation auf `blocked`.
- Ein Read-back-Mismatch veröffentlicht keine Revision.
- Ein fehlender oder fehlschlagender optionaler Session-Widerruf blockiert die
  Readiness nicht und wird nicht als erfolgreich persistiert.
- Ein unterlegener konkurrierender Lauf erhält keinen Claim und führt keinen
  externen Write aus.
- Ein Prozessabbruch gibt die tenantgebundene Datenbanksperre frei; der
  verwaiste Zwischenzustand ist durch einen idempotenten Folgelauf wieder
  beanspruchbar.
- Eine neue Sollgeneration macht alte Readiness vor dem externen Write
  unwirksam.
- Eine Access-Token-Laufzeit über 15 Minuten blockiert das produktive
  Enablement, nicht aber die intern bestätigte Projektionsreadiness.

## Freigabegate

Der produktive Schalter bleibt gesperrt, bis Staging für denselben Image-Digest
die Projektion im gemeinsamen Tenant-Realm, vollständigen Read-back,
Benutzertokenclaim, Runtime-Revision und eine maximale Access-Token-Laufzeit
von 15 Minuten nachweist. Das installationsweite Service-Token wird dabei
getrennt auf Identität, Audience und Action geprüft.
