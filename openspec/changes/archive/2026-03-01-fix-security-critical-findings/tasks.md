...
- [x] 1.5 Den `as any`-Cast auf der Route-Config (Finding R-1) dokumentieren und, soweit möglich, durch korrekte TanStack-Router-Typen ersetzen
- [x] 1.6 Unit-Test: Sicherstellen, dass alle bekannten Pfade korrekt aufgelöst werden
- [x] 1.7 Unit-Test: Sicherstellen, dass ein unbekannter Pfad zu einem expliziten Fehler führt (kein stillschweigender Logout)
...
- [x] 2.3 An beiden Stellen: Key-ID als strukturiertes Debug-Log über SDK-Logger ausgeben (falls Logger verfügbar) oder als separaten Error-Kontext mitgeben, der nicht in der Message selbst erscheint
- [x] 2.4 Unit-Test: Prüfen, dass Exception-Messages keine Key-IDs enthalten
...
- [x] 3.6 Manueller Smoke-Test: Künstlichen Fehler in einer Route provozieren → Fallback wird angezeigt
...
- [x] 4.2 Bei Bedarf: pnpm override in Root `package.json` setzen, um eine einheitliche Version workspace-weit zu erzwingen
...
- [x] 4.6 App manuell starten (`pnpm nx run sva-studio-react:serve`) und kritische Flows prüfen
...
- [x] 5.1 Betroffene arc42-Abschnitte prüfen und bei Bedarf aktualisieren:
