...
- [x] 1.1 Gruppen-Entitäten, Zuordnungstabellen und erforderliche Constraints spezifizieren
- [x] 1.2 Geo-Hierarchie-Read-Modell und fachlichen Schlüsselraum spezifizieren
- [x] 1.3 Prioritätsregeln für Rollen-, Gruppen-, Org- und Geo-Effekte dokumentieren
- [x] 1.4 Effektive Berechtigungsauflösung um Gruppenmitgliedschaften erweitern
- [x] 1.5 Hierarchische Geo-Vererbung inklusive Restriktionen spezifizieren
- [x] 1.6 Reasoning- und Transparenzdaten für Gruppen- und Geo-Herkunft ergänzen
- [x] 1.7 Gruppenverwaltung im Admin-Bereich spezifizieren
- [x] 1.8 Gruppenzuweisung in Benutzerdetails und/oder Gruppenansicht spezifizieren
- [x] 1.9 Rechte- und Rollenansichten um Gruppenherkunft ergänzen
- [x] 1.10 Testmatrix für Gruppen- und Geo-Konfliktfälle ergänzen
...
- [x] 2.16 Testmatrix für Vererbung, Restriktionen, Cache, Invalidierung und Migrationspfad als tabellarische Matrix strukturieren (inkl. 3+-Ebenen-Geo, Mixed-State-Migration, Race-Conditions)
...
- [x] 4.1 Pflichttext- und Versionslogik als Login-Vorbedingung spezifizieren
- [x] 4.2 Fail-Closed- und Fehlerkommunikation für unklaren Rechtstextstatus definieren
- [x] 4.3 Guard- und Session-Verhalten für blockierte Nutzer spezifizieren
- [x] 4.4 Blockierenden Akzeptanzflow für offene Pflichttexte spezifizieren
- [x] 4.5 Admin-Oberfläche für Nachweis, Filterung und Export spezifizieren
- [x] 4.6 Zugriffsgates und Deep-Link-Verhalten für Rechtstext-Sichten festlegen
- [x] 4.7 Pflichtfelder für Einzel- und Sammelnachweise von Rechtstext-Akzeptanzen präzisieren
- [x] 4.8 Konsistenzregeln zwischen UI, Export und Auditspur spezifizieren
- [x] 4.9 Test- und Berichtsnachweise für Enforcement und Export definieren
- [x] 4.10 Relevante Guides und Runbooks ergänzen
- [x] 4.11 Betroffene arc42-Abschnitte referenzieren

## 5. Konsolidierung, Qualität und Validierung

- [x] 5.1 Delta-Specs für `account-ui`, `iam-access-control`, `iam-organizations`, `iam-core` und `iam-auditing` konsistent halten
...
- [x] 5.3 Liefernachweise für Pakete 3 bis 5 so dokumentieren, dass Angebotsabnahme und technische Abnahme dieselben Artefakte referenzieren (Paket 3: Testmatrix-Ergebnis; Paket 4: Perf-Bericht + Invalidation-Testprotokoll; Paket 5: Export-Testprotokoll + Screenshot Akzeptanzflow)
- [x] 5.4 `openspec validate complete-iam-offer-packages-3-to-5 --strict` erfolgreich ausführen
- [x] 5.5 ADRs erstellen für: Gruppen als eigenständige IAM-Entität, Prioritätsregel Multi-Scope, Redis als Primary Permission Cache, Rechtstext-Fail-Closed; in `docs/architecture/09-architecture-decisions.md` verlinken
- [x] 5.6 Einzel-Changes `add-iam-organization-management-hierarchy` und weitere konsolidierte Changes mit `openspec archive` schließen und Abschluss in `docs/adr/` als technische Schuld für IAM-Konfigurations-Export dokumentieren
