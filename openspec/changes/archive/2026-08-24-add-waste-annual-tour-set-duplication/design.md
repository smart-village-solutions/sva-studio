...
Kalendergebundene jährliche Touren behalten Monat und Tag ihres ursprünglichen Jahrestags auch dann, wenn ihre Gültigkeit bereits vor dem Quelljahr begonnen hat. Endet der wirksame Quelljahrausschnitt vor diesem Jahrestag, enthält er keinen übertragbaren Jahreslauf und die Tour bleibt wegen ungültiger Planungsdaten blockiert. Konkrete Einzeltermine werden zunächst auf denselben Monat und Tag des Folgejahres übertragen und anschließend auf den nächstgelegenen gleichen Wochentag verschoben. Das Ergebnis muss im Folgejahr liegen; würde die nächstgelegene Wahl die Jahresgrenze überschreiten, wird der nächstgelegene gleiche Wochentag innerhalb des Folgejahres verwendet.

Kann ein Quellmonat/-tag wie der 29. Februar im für die Ressource abgeleiteten Zieljahr nicht dargestellt werden, verlangt die Vorschau ein konkretes Ersatzdatum genau in diesem ausgewiesenen Jahr. Bei jahresübergreifenden Verschiebungen kann dieses Jahr aufgrund des erhaltenen relativen Jahresversatzes nach dem direkten Folgejahr liegen. Dasselbe gilt, wenn mehrere unterschiedliche Quelltermine oder jahresbezogene Verschiebungen auf dieselbe fachliche Zielbeziehung abgebildet würden. Ersatzdaten werden nur für die von dieser Vorschau tatsächlich als nicht abbildbar oder kollidierend gemeldeten Quellressourcen akzeptiert, serverseitig gegen deren jeweiliges Zieljahr validiert und in den Vorschau-Fingerprint aufgenommen; ein Validierungsfehler nennt dieses ressourcenspezifische Zieljahr. Kollisionen unveränderter jahresunabhängiger Verschiebungsregeln lassen sich dadurch nicht auflösen und werden stattdessen als zu bereinigende Quelldaten blockiert. Unbekannte, doppelte oder unnötige Ersetzungen werden in der interaktiven Vorschau abgelehnt. Bei der bestätigten Revalidierung darf eine inzwischen entfallene Ersatzressource zunächst ignoriert werden, damit der geänderte Fingerprint zuverlässig `preview_stale` samt aktualisierter Vorschau statt eines isolierten Validierungsfehlers liefert. Die aktualisierte Vorschau kennzeichnet weiterhin anwendbare Ersatzressourcen, sodass der Client gültige Eingaben beibehält und nur tatsächlich entfallene Entscheidungen entfernt. Ohne eindeutige Auflösung bleibt die Tour blockiert.

### Vorschau-Fingerprint und Konflikte

Die Vorschau zeigt je Tour mindestens Tourname, Klassifikation, Quell- und Zielzeitraum, ersten Zieltermin, den tatsächlichen festen oder benutzerdefinierten Turnus sowie die Anzahl der Abfallarten, Abholorte, konkreten Termine, Einsätze und Verschiebungen. Für konkrete Tourtermine, ortsbezogene Abholtermine und Einsätze liefert sie deterministisch bis zu fünf Abbildungen im Format `Quelle → Folgejahr`; damit bleiben Nutzlast und Darstellung auch bei großen Beständen begrenzt. Sie fasst die Gesamtmengen und ausgeschlossenen Daten zusammen.
...
- Eine andere bestehende Tour mit identischen Mengen von Abfallarten und Abholorten, gleichem effektiven Tagesabstand oder gleichem jährlichen Turnus, überschneidendem Zielzeitraum und mindestens einem gemeinsamen generierten, jährlichen, expliziten oder durch eine Verschiebung wirksamen Termin ist ein möglicher fachlicher Konflikt. Die Quelltour ist zunächst abgewählt, kann aber nach ausdrücklicher Kenntnisnahme bestätigt werden, weil parallele Einsätze fachlich zulässig sein können.
...
Der Server sperrt Erstellungsvorgänge mandanten- und folgejahrbezogen mit einem transaktionalen Advisory Lock. Zusätzlich sperrt er die planungsrelevanten Tabellen des mandanteneigenen Waste-Schemas vor dem Snapshot mit `SHARE ROW EXCLUSIVE`, sodass normale Tour- und Terminänderungen Fingerprint-Prüfung und Schreiben nicht überholen können. Innerhalb derselben Waste-Datenbanktransaktion lädt er den aktuellen Quell- und Zielbestand, prüft Fingerprint und Konflikte und erzeugt anschließend Touren und Beziehungen. Jeder Fehler führt zum vollständigen Rollback.

Ziel-Tour-IDs werden stabil aus Instanz, Quell-Tour-ID und Folgejahr abgeleitet. Beziehungs-IDs werden stabil aus Ziel-Tour-ID, Beziehungstyp und Quell-Beziehungs-ID abgeleitet. Sie hängen nicht vom Akteur oder HTTP-Idempotenzschlüssel ab. So konvergieren wiederholte Jahreswechselversuche fachlich auf dieselben Zielressourcen. Abgelaufene technische Verarbeitungs-Leases werden nur per Compare-and-Swap auf Status, bisherigen Lease-Inhalt und weiterhin abgelaufene Zeit übernommen; ein gleichzeitiger Heartbeat oder Abschluss gewinnt den Wettlauf ohne Überschreiben. Das zusammenfassende Audit verwendet den HTTP-Idempotenzschlüssel als stabile Ereignisreferenz. Ein nach einem Prozessabbruch wiederaufgenommener Request erkennt ein bereits persistiertes Audit und erzeugt keine zweite Zusammenfassung.

Der Erstellungsrequest trägt zusätzlich einen mandantenbezogenen Idempotenzschlüssel. Der zentrale Idempotenzspeicher reserviert den Scope aus Akteur, Instanz, Endpunkt und Schlüssel mit einem kanonischen Payload-Fingerprint, einem zufälligen Ownership-Token und einer fünfminütigen Lease und speichert die abschließende Antwort. Der aktive Request erneuert die Lease minütlich und prüft den Token nochmals direkt vor Audit und Completion. Erst nach Ablauf einer nicht erneuerten Lease darf derselbe Request die verwaiste `IN_PROGRESS`-Reservierung mit einem neuen Token übernehmen; der alte Owner kann danach weder Audit noch Antwort finalisieren. Nach einem Prozessabbruch zwischen Commit der Waste-Transaktion und Abschluss des zentralen Eintrags rekonstruiert der Server das Ergebnis anhand der stabilen Ziel-IDs. Vollständig identische Daten gelten als Replay, fehlende Daten erlauben einen erneuten atomaren Versuch und abweichende Daten führen zu `target_identity_conflict`.

Ein zweiter Request mit demselben noch laufenden Idempotenzschlüssel wird als `idempotency_in_progress` abgelehnt und führt weder die Waste-Operation noch das Audit erneut aus. Der Heartbeat verhindert die Übernahme eines noch aktiven Langläufers. Nach Ablauf einer nicht erneuerten Lease kann derselbe Schlüssel die verwaiste Reservierung übernehmen und anhand der stabilen Ziel-IDs vollständig rekonstruieren, ob das Waste-Ergebnis bereits vorliegt.

Es gelten feste serverseitige Grenzwerte von 1.000 Touren und insgesamt 100.000 zu kopierenden Beziehungen pro Jahresübernahme. Beide Werte sind im API-Vertrag zentral definiert und werden in Vorschau sowie Erstellung identisch mit `batch_limit_exceeded` durchgesetzt. Die Konfliktprüfung indiziert Beziehungen und fachliche Signaturen einmal pro Snapshot; große Beziehungsmengen werden innerhalb der Transaktion in begrenzten JSON-Batches geschrieben. Die Grenzen liegen bewusst deutlich über einem regulären Mandantenbestand, begrenzen aber Arbeitsspeicher, Fingerprint-Bildung und Transaktionsdauer gegen unkontrollierte Eingaben.
...
Die Erstellung erzeugt genau ein zusammenfassendes Audit-Ereignis für Erfolg oder Fehlschlag. Protokolliert werden Quell- und Folgejahr, Klassifikations- und Ergebnismengen sowie technische Ressourcen-IDs, jedoch keine Notizen, Adressdaten oder sonstigen fachlichen Freitexte. Schlägt eine bestätigte Erstellung wegen einer veralteten Vorschau oder eines stabilen Zielkonflikts fehl, stammen die Klassifikationsmengen aus der aktualisierten serverseitigen Vorschau. Vorschau und reine Validierungsfehler vor einer bestätigten Mutation erzeugen kein Audit-Ereignis. Erst nachdem der Audit-Aufruf abgeschlossen ist, wird die zentrale Idempotenzantwort dauerhaft auf `COMPLETED` oder `FAILED` gesetzt; ein vorheriger Prozessabbruch bleibt damit über die Lease wiederaufnehmbar.
