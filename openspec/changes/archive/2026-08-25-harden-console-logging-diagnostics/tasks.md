...
- [x] 6.7 Nach dem Rollout dieselbe Abfrageklasse wie in der Ausgangsstichprobe gegen den tatsächlich aktiven Transport auswerten und belegen, dass alle inventarisierten HTTP-Ereignisse eine Request-ID besitzen, vorhandene Trace-IDs valide sind, repräsentative Fehlerketten genau ein kanonisches Endereignis erzeugen, inventarisierte Hochvolumen-Erfolge nicht auf `info` erscheinen und Mutation-, Audit- sowie Jobabschlussereignisse erhalten bleiben. Die kombinierte Production-Evidenz in `docs/reports/console-logging-diagnostics-production-follow-up-2026-08-25.md` umfasst die ursprüngliche Signalprüfung, den Cross-Bundle-Reproduktionstest des Carrier-Fixes, den verifizierten Same-Digest-Rollout und eine ausschließlich post-deployte Loki-Stichprobe mit 102 vollständig korrelierten Mainserver-Erfolgen, einem Callback und sieben Audit-Ereignissen. Mangels natürlicher Mainserver-Mutation oder Jobausführung im kurzen Nachlauf wird dort ausdrücklich kein zusätzliches Einzelereignis behauptet; für diese quellenidentisch gebliebenen Emitter ist die akzeptierte Nachweisgrenze als Kombination aus vorheriger Production-Stichprobe, unverändertem Emissionscode, grünen Vertragstests, Build und Same-Digest-Rollout dokumentiert. Ein reproduzierbarer Live-Canary wird in `#1156` nicht blockierend nachgeführt.
- [x] 6.8 Die Canary-Redactionstests als primären Leck-Nachweis auswerten und die Laufzeitprüfung auf bekannte verbotene Feldformen begrenzen, ohne sensible Rohwerte zu exportieren. Server-, Browser- und OTEL-Canaries sind grün; die Production-Stichprobe zeigte `projection_scope_key` ausschließlich als `[REDACTED]` und exportierte keine Rohwerte.
- [x] 6.9 Bei fehlender Request-Korrelation, verlorenen kanonischen Ereignissen oder unredigierten sensiblen Werten den betroffenen Delivery-Slice zurückrollen und die nachfolgenden Slices stoppen. Der isolierte Korrelations-Slice blieb bis zum Same-Digest-Production-Rollout und zur erneuten Transportabfrage gestoppt. Danach lagen weder fehlende Korrelation noch verlorene kanonische Ereignisse oder unredigierte sensible Werte als Rollback-Signal vor; der Slice wurde freigegeben.
