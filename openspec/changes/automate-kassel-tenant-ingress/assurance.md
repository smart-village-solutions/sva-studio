...
| `KAS-ACT-01`   | Ein Kasseler Create-Lauf endet erst nach erfolgreichem externem TLS-, Studio-Login-Redirect und maschinenprüfbarem modulabhängigem Readiness-Smoke in `awaiting_activation`; `active` setzt ausschließlich die gemeinsame bestätigte Benutzeraktion.                                | Abbruchtests vor der technischen Abnahme, Aktivierungs-Gate und öffentliche Studio-Smokes |
| `KAS-SSF-01`   | Bei effektiv aktivem SSF umfasst Create-Erfolg Browser- und Ressourcenclient, IAM-Projektion, Runtime-Baseline, Runtime-Readiness und aktuelle Authorization-Revision. Der credentialgebundene Directory → Callback → Gateway-Nachweis bleibt ein separates Kassel-Enablement-Gate. | #1319-Vertragstests; getrennt davon Zwei-Realm-Ende-zu-Ende-Rolloutnachweis               |
| `KAS-IDEM-01`  | Create-Redelivery mit demselben Key und expliziter Retry mit frischem Transport-Key erzeugen höchstens einen wirksamen Elternlauf und Router; korrekte Registry-, Keycloak- oder Lifecycle-Artefakte bleiben unverändert.                                                           | Parallelitäts-, Retry- und Redelivery-Tests für beide Idempotenzebenen                    |
...
| `KAS-ID-01`    | Ungültige oder nicht kleingeschriebene Instanz-IDs erreichen weder Registry-Persistenz noch Realm-, Hostname-, Ingress- oder Lifecycle-Erzeugung.                                                                                                                                   | Create-/Update-Vertragstests mit `Labor`, Punycode und ungültigen DNS-Labels              |
| `KAS-RETRY-01` | Ein unveränderlicher terminaler Lifecycle-Fehler bewahrt seinen stabilen Ursachen-Code und erzeugt ohne neue Generation oder explizite Operator-Aktion keinen weiteren Job.                                                                                                         | Reconciler-, Scheduler- und Repository-Regressionstests                                   |
| `KAS-DEL-01`   | Die freigegebene Löschung von `Labor` betrifft nur vorab inventarisierte, eindeutig korrelierte Studio-, SSF-, Keycloak- und Routerartefakte und wird nach jedem Owner-Schritt read-back-verifiziert.                                                                               | Backup-Readback, Artefaktinventar und negative Fremdartefakt-Prüfung                      |
...
| `BND-09` | Studio → SSF-Readiness           | Client, Baseline, Projektion oder Revision fehlen              | Lifecycle bleibt pending/blocked; Elternlauf endet nicht erfolgreich            |
...
| `BND-12` | Rollout → SSF-Browser-Acceptance | Directory, echter Login, Callback oder Gateway fehlen          | Kassel-Enablement stoppen; keinen Create-Lauf künstlich offen halten            |
...
| `failed`                           | autorisierter expliziter Retry               | Eigener HTTP-Key nimmt denselben Elternlauf auf; vorhandene Artefakte werden idempotent reconciliiert   |
...
- Vertragsnachweis, dass credentialgebundene Browser-Acceptance kein
  Create-Schritt ist und fehlende Rollout-Evidenz das Enablement stoppt.
