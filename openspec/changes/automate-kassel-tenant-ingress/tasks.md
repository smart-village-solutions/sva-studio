...
- [x] 1.1 Den vorhandenen `iam.instance_provisioning_runs`-Datensatz pro
      Instanz, Operation und Idempotency-Key in place fortschreibbar machen
- [x] 1.2 Versionierten Sollsnapshot, Kindlauf-Korrelation, Claim/Lease,
      Wake-up, Attempts, Deadline und terminale Evidenz mit minimaler
      Schemaerweiterung modellieren
- [x] 1.3 Instanz, Elternlauf und ausführbaren Startzustand ohne verlorenes
      Wake-up persistieren
- [x] 1.4 Instanzgebundene Serialisierung und Idempotenz des vorhandenen
      Keycloak-Provisioners für den Elternlauf wiederverwenden
- [x] 1.5 Migration, Down-Pfad, Schema-Snapshot und Schemadokumentation
      aktualisieren
...
- [x] 2.1 Keycloak-Kindläufe explizit mit Elternlauf und Sollsnapshot
...
- [x] 3.1 Eigenen PR für File Provider und dynamisches
      Konfigurationsverzeichnis erstellen
- [x] 3.2 Traefik-Verzeichnis read-only mounten und Docker Provider sowie
      bestehende Router unverändert lassen
- [x] 3.3 Providerqualifizierte Studio-Service-Referenz gegen die reale
      Kasseler Topologie festlegen und testen
- [x] 3.4 Vertragstests für leeres Verzeichnis, atomare Updates, ungültige
...
- [x] 4.1 Expliziten Kassel-Modus mit bestehendem Verhalten als
      standardmäßigem Default ergänzen
- [x] 4.2 Writer-Mount ausschließlich dem vorhandenen Standalone-Provisioner
      geben; App und Traefik ohne Schreibrecht sowie ohne zusätzlichen
      Docker-Socket halten
- [x] 4.3 Hostnormalisierung, reservierte Namen und strikte
      `*.dialog.kassel.de`-Validierung implementieren
- [x] 4.4 Deterministischen Renderer für genau einen expliziten Router und
      tenantbezogenen Zertifikatsvertrag implementieren
- [x] 4.5 Validieren, in temporäre Datei schreiben und innerhalb desselben
      Verzeichnisses atomar umbenennen
- [x] 4.6 Routerübernahme und öffentliches Zertifikat mit begrenztem Retry und
...
- [x] 6.1 Create-Antwort und UI auf Run-ID sowie reine serverseitige
...
- [x] 7.1 ADR für Elternlauf-Orchestrierung und Kassel-spezifische
...
- [x] 7.5 Keinen zweiten kanonischen Studio-Rollout definieren

## 8. Lokale und CI-Gates

- [x] 8.1 Jeden Implementierungsblock mit den kleinsten betroffenen Unit-,
