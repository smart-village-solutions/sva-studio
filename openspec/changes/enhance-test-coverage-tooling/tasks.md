...
- [ ] `nx.json` öffnen und `targetDefaults` erweitern
- [ ] `test:coverage` Target mit `cache: true` konfigurieren
- [ ] `inputs` definieren: `["default", "^production", "{workspaceRoot}/vitest.config.ts"]`
- [ ] `outputs` definieren: `["{projectRoot}/coverage"]`
- [ ] Named Input `testing` erstellen für Test-spezifische Files
- [ ] Named Input zu `test:unit` Target hinzufügen
- [ ] Lokal testen: `pnpm test:coverage` zweimal ausführen (2. Mal sollte "cache hit" zeigen)
- [ ] Cache-Verhalten validieren: `nx show project sdk --verbose`
...
- [ ] `scripts/ci/coverage-gate.mjs` öffnen
- [ ] Color-Helper-Funktionen hinzufügen (oben im File):
  ```javascript
  const colors = {
    reset: '\x1b[0m',
  ...
  ```
- [ ] Erfolgs-Meldungen mit `colors.green()` wrappen
- [ ] Fehler-Meldungen mit `colors.red()` wrappen
- [ ] Warnungen mit `colors.yellow()` wrappen
- [ ] Emojis hinzufügen: ✅ (pass), ❌ (fail), ⚠️ (warn)
- [ ] Lokal testen: `node scripts/ci/coverage-gate.mjs`
- [ ] Fehlerfall testen: temporär Floor erhöhen, Gate sollte farbig fehlschlagen
...
- [ ] `docs/development/testing-coverage.md` öffnen
- [ ] Neue Sektion "## Troubleshooting" am Ende hinzufügen
- [ ] Fehlerszenario 1: "missing coverage-summary.json" dokumentieren
  - Ursache
  - Lösung (vitest/coverage-v8 installieren)
  - Beispiel-Commands
- [ ] Fehlerszenario 2: "affected leer" dokumentieren
  - Ursache
  - Lösung (origin/main aktualisieren, affected Debugging)
  - Beispiel-Commands
- [ ] Fehlerszenario 3: "dropped by X pp" dokumentieren
  - Ursache
  - Lösung (Tests hinzufügen oder Baseline updaten)
  - Wann Baseline-Update legitim ist
- [ ] Fehlerszenario 4: "Exemptions" dokumentieren
  - Bedeutung (excluded aus Gate / global)
  - Prozess zum Entfernen aus Exemptions
- [ ] Fehlerszenario 5: "No tests configured" dokumentieren
  - Ursache
  - Lösung (mindestens ein Unit-Test, optional passWithNoTests entfernen)
- [ ] Neue Sektion "## Migration Guide" hinzufügen
- [ ] Step-by-Step für "Neues Package zur Coverage hinzufügen"
  1. Dependencies installieren
  2. Targets in project.json
  3. Tests schreiben
  4. Policy aktualisieren (exemptProjects entfernen)
  5. Baseline setzen
- [ ] Beispiel-Commands für alle Schritte hinzufügen
- [ ] PR-Checkliste präzisieren: wo Coverage-Artefakte in der GitHub UI zu finden sind (`docs/reports/PR_CHECKLIST.md`)
- [ ] Cross-Links zu `PR_CHECKLIST.md` und `coverage-policy.json` einfügen
...
- [ ] `.github/workflows/test-coverage.yml` öffnen
- [ ] Concurrency-Block nach `name:` hinzufügen:
  ```yaml
  concurrency:
    group: ${{ github.workflow }}-${{ github.ref }}
    cancel-in-progress: ${{ github.ref != 'refs/heads/main' }}
  ```
- [ ] Artifact-Upload anpassen:
  - Name: `coverage-reports-${{ github.run_id }}`
  - `retention-days: 7` hinzufügen
- [ ] Workflow lokal validieren: `actionlint .github/workflows/test-coverage.yml`
...
- [ ] `vitest.workspace.ts` im Root erstellen
- [ ] Workspace-Definition schreiben:
  ```typescript
  ...
  ```
- [ ] Shared Config in Root `vitest.config.ts` erstellen
- [ ] Coverage-Reporter zentral konfigurieren (text-summary, json-summary, lcov)
- [ ] Coverage-Output-Dir standardisieren: `./coverage`
- [ ] Package-Configs vereinfachen (nur noch Projekt-spezifische Overrides behalten)
- [ ] `apps/sva-studio-react/project.json` anpassen:
  - `cwd` Parameter entfernen
  - Command: `pnpm exec vitest run --coverage` (ohne explizite Reporter)
- [ ] `packages/sdk/project.json` analog anpassen
- [ ] Alle Tests lokal ausführen: `pnpm test:unit`
- [ ] Coverage lokal ausführen: `pnpm test:coverage`
- [ ] Verifizieren: Coverage-Files werden an erwarteten Orten generiert
...
- [ ] `scripts/ci/coverage-gate.ts` erstellen (basierend auf .mjs)
- [ ] TypeScript-Interfaces definieren:
  ...
  ```
- [ ] Typed Helper-Funktionen erstellen
- [ ] Original .mjs Logik nach TypeScript portieren
- [ ] `tsx` als Dev-Dependency hinzufügen: `pnpm add -D tsx`
- [ ] Root `package.json` Script aktualisieren:
  ```json
  "coverage-gate": "tsx scripts/ci/coverage-gate.ts"
  ```
- [ ] CI-Workflow anpassen: `node` → `pnpm coverage-gate`
- [ ] Lokal testen: `pnpm coverage-gate`
- [ ] Fehlerszenarien testen (falsche Policy-Struktur sollte Type-Error werfen)
- [ ] Alte .mjs-Datei behalten bis TypeScript-Version validiert ist
...
- [ ] `DEVELOPMENT_RULES.md` öffnen
- [ ] Neue Sektion "## 5. Test Coverage Requirements" nach Sektion 4 einfügen
- [ ] REQUIRED-Block schreiben:
...
- [ ] Beispiele hinzufügen (analog zu Sektion 1):
  ...
  ```
- [ ] Cross-Link zu `docs/development/testing-coverage.md` einfügen
- [ ] Cross-Link zu `PR_CHECKLIST.md` einfügen
...
- [ ] `.github/workflows/test-coverage.yml` erweitern:
  ```yaml
  ...
  ```
- [ ] Codecov-Konfiguration erstellen: `codecov.yml`
  ```yaml
  coverage:
    status:
      project:
...
- [ ] Coverage-Badge in `README.md` einfügen (optional)

**Alternative:** Erweiterte GitHub Actions Summary statt Codecov
- [ ] `scripts/ci/coverage-gate.ts` erweitern
- [ ] Trend-Berechnung vs. letzte 5 Runs (via GitHub API)
- [ ] Visual Chart in Markdown-Table (ASCII-Art)
- [ ] Delta-Anzeige pro Projekt (🟢 +2%, 🔴 -1%)
...
### Unit Tests
- [ ] Jest/Vitest Tests für coverage-gate.ts
- [ ] Mock-Policy/Baseline-Dateien für Testszenarien
- [ ] Edge Cases testen:
...
### Integration Tests
- [ ] Full Coverage-Workflow lokal durchlaufen
- [ ] CI-Workflow in Feature-Branch testen
- [ ] Verifizieren: Gate blockiert bei Coverage-Drop
- [ ] Verifizieren: Gate passiert bei ausreichender Coverage

### Regression Tests
- [ ] Bestehende Tests laufen weiterhin (keine Breaks)
- [ ] Coverage-Artefakte werden weiterhin generiert
- [ ] Nx Cache invalidiert nicht ungewollt
...
Nach Implementation:
- [ ] `docs/development/testing-coverage.md` auf aktuellsten Stand bringen
- [ ] `PR_CHECKLIST.md` mit neuen Requirements updaten
- [ ] `AGENTS.md` mit neuen Skills/Patterns updaten (falls relevant)
- [ ] `README.md` mit Coverage-Badge updaten (bei Codecov)
