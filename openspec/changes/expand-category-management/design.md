...
Client und Host validieren mindestens einen nicht leeren getrimmten Namen, eine ganzzahlige Position im GraphQL-`Int`-Bereich `0..2147483647`, genau eine syntaktisch gültige optionale E-Mail-Adresse sowie getrimmte Datentyp-Identifier aus 1 bis 128 Zeichen. Ein optionales Icon folgt dem bestehenden App-/Mainserver-Vertrag: entweder ein 1 bis 255 Zeichen langer Name aus Buchstaben, Ziffern, Punkt, Unterstrich und Bindestrich, beginnend mit Buchstabe oder Ziffer, oder eine syntaktisch gültige absolute HTTP(S)-URL mit höchstens 255 sichtbaren ASCII-Zeichen. Neue Kategorien starten im Formular aktiv. Gleiche Positionen innerhalb einer Hierarchieebene bleiben zulässig; die Management-Sicht löst sie wie der Mainserver stabil nach Position, Name und ID auf, statt eine nicht vorhandene Eindeutigkeit zu behaupten.

Mainserver-Management-Antworten verwenden für Datentyp-Identifier denselben Vertrag wie Save-Eingaben. Ein abweichender Wert wird als ungültige Upstream-Antwort behandelt und nicht in einen editierbaren Draft übernommen.
