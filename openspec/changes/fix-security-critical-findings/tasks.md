...
- [ ] 1.1 In `packages/routing/src/auth.routes.server.ts`: Alle Auth-Route-Pfade als Union-Type extrahieren (falls nicht vorhanden)
- [ ] 1.2 `resolveAuthHandlers`-Funktion von `if-else`-Kette auf typsicheres `Record<AuthRoutePath, () => Promise<Handler>>`-Mapping umstellen
- [ ] 1.3 `satisfies Record<AuthRoutePath, ...>` nutzen, damit fehlende Pfade zu Compile-Time-Fehlern führen
- [ ] 1.4 Fallback-`else`-Zweig entfernen; bei unbekanntem Pfad expliziten `throw` oder 404-Response einführen
...
- [ ] 2.1 In `packages/core/src/security/field-encryption.ts`: Exception bei `Encryption key '${keyId}' is not configured` (Zeile ~26) generisch formulieren: `Encryption key is not configured`
- [ ] 2.2 Exception bei `IAM_PII_ACTIVE_KEY_ID '${activeKeyId}' not found in keyring` (Zeile ~105) generisch formulieren: `Active encryption key not found in keyring`
...
- [ ] 2.5 Prüfen, ob weitere Stellen im Projekt interne Key-IDs in Exceptions leaken (grep nach `keyId` in `throw`)
...
- [ ] 3.1 Error-Fallback-Komponente erstellen (z. B. `apps/sva-studio-react/src/components/ErrorFallback.tsx`)
- [ ] 3.2 Komponente zeigt benutzerfreundliche Fehlermeldung, Retry-Button und Link zur Startseite
- [ ] 3.3 Alle sichtbaren Texte über i18n-Keys referenzieren (sofern i18n-System vorhanden, sonst als TODO markieren)
- [ ] 3.4 WCAG-Basics: `role="alert"`, `aria-live="assertive"`, semantisches HTML
- [ ] 3.5 In `apps/sva-studio-react/src/routes/__root.tsx`: `errorComponent: ErrorFallback` setzen
...
- [ ] 4.1 `@tanstack/react-router`-Version zwischen `packages/routing/package.json` und `apps/sva-studio-react/package.json` auf eine gemeinsame Version synchronisieren
- [ ] 4.2 Bei Bedarf: pnpm override in Root `package.json` setzen, um eine einheitliche Version workspace-weit zu erzwingen
- [ ] 4.3 `nitro-nightly@latest` in `apps/sva-studio-react/package.json` durch die neueste stabile Nitro-Version ersetzen
- [ ] 4.4 `pnpm install` und `pnpm nx run-many -t build` ausführen – sicherstellen, dass Build grün ist
- [ ] 4.5 `pnpm nx run sva-studio-react:test:unit` und `pnpm nx run sva-studio-react:test:e2e` ausführen
...
- [ ] 5.2 `pnpm nx affected --target=lint` ausführen
- [ ] 5.3 `pnpm nx affected --target=test:unit` ausführen
- [ ] 5.4 `pnpm nx affected --target=build` ausführen
