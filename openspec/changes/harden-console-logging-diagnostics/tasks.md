...
- [ ] 6.7 Nach dem Rollout dieselbe Abfrageklasse wie in der Ausgangsstichprobe gegen den tatsächlich aktiven Transport auswerten und belegen, dass alle inventarisierten HTTP-Ereignisse eine Request-ID besitzen, vorhandene Trace-IDs valide sind, repräsentative Fehlerketten genau ein kanonisches Endereignis erzeugen, inventarisierte Hochvolumen-Erfolge nicht auf `info` erscheinen und Mutation-, Audit- sowie Jobabschlussereignisse erhalten bleiben. Die Production-Nachprüfung in `docs/reports/console-logging-diagnostics-production-follow-up-2026-08-25.md` fand zwei Mainserver-POI-Erfolgsmeldungen ohne Request-ID; der reproduzierte Carrier-Fix benötigt Merge, Rollout und erneute Live-Abfrage.
...
- [ ] 6.9 Bei fehlender Request-Korrelation, verlorenen kanonischen Ereignissen oder unredigierten sensiblen Werten den betroffenen Delivery-Slice zurückrollen und die nachfolgenden Slices stoppen. Der isolierte Korrelations-Slice bleibt bis zum Production-Nachweis gestoppt; die bereits wirksame Redaction- und Severity-Härtung wurde nicht pauschal zurückgerollt.
