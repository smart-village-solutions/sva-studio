...
- **H3 – Recovery- und Live-Revision-Vertrag:** `completed after contract-review fixes`; finaler Implementierungs-HEAD: `7924a8fa70bb2785ad65a84d52c338d1d9976456`. Die Config-Revision wird ausschließlich aus dem tatsächlich selektierten Deploy-Bundle abgeleitet; der Shadow-Candidate kann deshalb keine falsche Live-Revision labeln. Ein Target ohne ausführbaren `sva.config.revision`-Labelvertrag stoppt vor Backup und Mutation, und die finale Verifikation liest Digest und Config-Revision aus demselben Remote-Service-Snapshot zurück. Der frei setzbare Input `recovery_failure_code` wurde entfernt: Same-Digest-Recovery attestiert nur `documented-cause`, niemals eine unbelegte Vor-Evidenz, und erzwingt unverändert Staging-Parität. Der normativ erlaubte Standard-Same-Digest-Fall überspringt Parität weiterhin gemäß Deployment-Topology-Spec 63–78, darf aber ohne bereits live gebundene Config-Revision keinen Legacy-Seed erzeugen. Read-only wurde als vorbestehender Ist-Zustand festgestellt, dass der geschützte Production-Config-Builder bereits autoritativ konfiguriert ist; dieser Block hat weder Production-Variablen gelesen oder verändert noch ein Environment mutiert. Gates: gezielte `tooling-testing:test:unit`-Suite mit elf Config-, Recovery-, Readiness-, Swarm-, Paritäts-, Evidence- und Workflow-Vertragstestdateien (133 Tests), isolierter Regex-Korrektur-Nachlauf (7 Tests), `pnpm exec tsc -p tsconfig.scripts.json --noEmit`, `pnpm nx run tooling-testing:lint`, `pnpm complexity-gate`, Prettier-Check, YAML-Parse, `pnpm check:file-placement`, Diff-Check sowie beide strikten OpenSpec-Validierungen grün. Ein zwischenzeitlicher Workflow-Regex-Test war rot, wurde korrigiert und danach sowohl isoliert als auch in der vollständigen Zielsuite grün bestätigt. Es erfolgte keine Environment-Änderung, kein Deploy und keine Production-Mutation. Nächster Schritt ist unabhängiges Review und Integration; Production-Shadow beziehungsweise Aktivierung aus 6.4 bis 6.6 bleibt ohne neue Freigabe ausdrücklich gesperrt.
