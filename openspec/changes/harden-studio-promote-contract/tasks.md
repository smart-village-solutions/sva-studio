...
- **H4 – Einmaliger Staging-Live-Config-Label-Übergang:** `implementation-complete, operational prepare/seed pending`; Basis: `115cc2108733223a321f2447919723ca7f1d2e2d`; Implementierungs-HEAD: `1e816fe6c6fb1f7f77a15f85f2c26f17ca71e45f`. Der implementierte Zwei-Run-Handshake ist ausschließlich über einen expliziten `workflow_dispatch` für Staging, Standard, Same-Digest, fehlendes Live-Config-Label und den kanonischen Backup-Agenten erreichbar. Der Prepare-Lauf attestiert den fehlenden Labelzustand über ein eigenes H1-Gate und stoppt anschließend deterministisch vor Backup und Mutation; nur der unmittelbar folgende Promote-Run desselben Workflows kann diese fehlgeschlagene Evidenz nach Run-ID, Attempt, Run-Nummer, Artefakt, Gatematrix und wiederholtem Live-Snapshot als Seed autorisieren. Der Seed nutzt ein controller-owned Staging-Overlay, erfindet weder `previousConfigRevision` noch Rollback-Evidenz und erzeugt keine Production-fähige Staging-Parität. Gates: fokussierter `tooling-testing:test:unit`-Lauf mit zehn expliziten H1/H3/H4-, Main-E2E-, Config-, Compose- und Workflow-Vertragstestdateien (246 Tests), `pnpm exec tsc -p tsconfig.scripts.json --noEmit`, `pnpm nx run tooling-testing:lint`, direkter Complexity-Analyzer einschließlich ungetrackter Dateien gegen die `ci-scripts`-Grenzen, Prettier-Check für alle intendierten Dateien, YAML-Parse von Promote-Workflow und Seed-Overlay, `pnpm check:file-placement`, Diff-Check sowie beide strikten OpenSpec-Validierungen grün. Zwei unabhängige Re-Reviews waren ebenfalls ohne Findings und bestätigten jeweils 173 fokussierte Tests samt statischen Gates. Offen bleiben die operative Ausführung und Live-Evidenz des Prepare-/Seed-Handshakes auf Staging; sie benötigen eine neue ausdrückliche Freigabe. Es ist kein Folgeblock freigegeben. Production bleibt unverändert und für diesen Übergang technisch ausgeschlossen; kein Production-Deploy oder Environment-Change wurde durchgeführt.
