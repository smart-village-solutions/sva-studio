...
Die Listenroute darf partielle Zeilen ausliefern. `pagination.total` bleibt aus Kompatibilitätsgründen die exakte Anzahl der aktuell autorisiert verfügbaren lokalen Treffer. Additive Metadaten liefern `availableCount`, `totalCount` nur bei vollständigem Snapshot, `isTotalFinal` und pro Mainserver-Typ den Snapshot-Zustand. Die UI verwendet bei `isTotalFinal = false` weder `pagination.total` zur Berechnung einer endgültigen Seitenzahl noch Navigation auf noch nicht materialisierte Seiten und zeigt einen lokalisierten Hinweis auf den laufenden oder fehlgeschlagenen Aufbau.
