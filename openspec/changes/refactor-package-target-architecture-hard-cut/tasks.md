...
- [ ] 5.3 DSR, Legal Texts und Audit-nahe Governance-Flows in `@sva/iam-governance` verschieben
  - Fortschritt: Read-Models, Legal-Text-HTML/Schemas, Legal-Text-Repository, Legal-Text-Mutation-Handler und Legal-Text-Listen-/Pending-Handler liegen in `@sva/iam-governance`; `auth` nutzt dafür Boundary-Adapter.
  - Offen: Legal-Text-Request-Context, DSR-Export-/Maintenance-Flows und Governance-Workflow-Implementierung liegen noch unter `packages/auth/src/iam-data-subject-rights`, `packages/auth/src/iam-legal-texts` und `packages/auth/src/iam-governance`.
