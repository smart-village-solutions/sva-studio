...
- [ ] 5.1 Login, Logout, OIDC, Cookies, Session und Middleware in `@sva/auth-runtime` bündeln
  - Fortschritt: Auth-/Session-Typen, Scope-Helfer, Runtime-Session-Fehler, Token-Fehler-Guards, Mock-Auth, Token-Verschlüsselung, Request-Host-Auflösung, Runtime-Secrets und Auth-Config inklusive Tenant-Auth-Resolution liegen in `@sva/auth-runtime`; OIDC-, Login-, Callback-, Logout-, Session-, Cookie- und Route-Handler-Code nutzt diese Runtime-Verträge lokal.
  - Offen: `@sva/auth-runtime` importiert noch serverseitige Implementierung aus `@sva/auth/server` für Redis-Client, Audit-Events, Tenant-Host-Middleware-Prüfung, Legal-Text-Compliance, JIT, Mainserver-Credentials und Kompatibilitäts-Routen.
