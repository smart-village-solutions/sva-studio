...
- **AND** ein günstiges Nx-Tag oder eine gemischte Package-Rolle zählt nicht als Freigabe

#### Scenario: Bestehender Altverstoß ist dokumentiert

- **GIVEN** ein bestehender Plugin-Verstoß ist mit Package, Regel, Subject, Owner, Begründung und Folgechange in der Baseline dokumentiert
- **WHEN** der Architektur-Gate läuft
- **THEN** bleibt genau dieser Altfall toleriert
- **AND** jede neue oder veränderte Abweichung blockiert weiterhin den Lauf

### Requirement: Modulare Runtime-Orchestrierung

Das System SHALL die kanonische `runtime-env`-Implementierung intern in dedizierte Module für Diagnose, Deploy-Orchestrierung, Smoke/Warmup, Remote-Verification und Command-Dispatch strukturieren, ohne den externen CLI-Vertrag zu ändern.

#### Scenario: Runtime-CLI bleibt Fassade

- **WHEN** `scripts/ops/runtime-env.ts` einen Runtime-Befehl entgegennimmt
- **THEN** validiert die Datei nur Command und Profil, verdrahtet die benötigten Abhängigkeiten und delegiert danach an modulare Orchestrierungslogik
