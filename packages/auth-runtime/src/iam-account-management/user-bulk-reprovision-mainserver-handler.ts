...
const resolveBulkReprovisionIdentityProvider = (
  provider: unknown
): BulkReprovisionIdentityProvider => provider as BulkReprovisionIdentityProvider;
...
const mapBulkReprovisionFailure = (id: string, error: unknown): BulkReprovisionFailure => {
...
const reprovisionSingleUser = async (input: {
  ...
  if (!detail) {
    return {
      ok: false,
      failure: { id: input.userId, code: 'not_found', message: 'Nutzer nicht gefunden.' },
    };
  }
  ...
  if (!targetAccess.ok) {
    return {
      ok: false,
      failure: { id: detail.id, code: 'forbidden', message: targetAccess.message },
    };
  }
  ...
  try {
    ...
    return {
      ok: false,
      failure: mapBulkReprovisionFailure(detail.id, error),
    };
  }
};
...
const executeBulkReprovisionMainserver = async (
  ...
  withInstanceScopedDb(input.actor.instanceId, async (client) => {
    const identityProvider = resolveBulkReprovisionIdentityProvider(
