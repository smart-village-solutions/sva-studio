...
describe('waste annual tour transfer', () => {
  ...
  it('uses only published tours as annual transfer sources', async () => {
    const preview = await buildWasteAnnualTourTransferPreview({
      instanceId: 'tenant-a',
      sourceYear: 2026,
      currentYear: 2026,
      source: source([
        tour({ id: '11111111-1111-4111-8111-111111111111', status: 'published' }),
        tour({ id: '22222222-2222-4222-8222-222222222222', status: 'draft' }),
        tour({ id: '33333333-3333-4333-8333-333333333333', status: 'archived' }),
      ]),
      target: source([]),
    });

    expect(preview.tours.map((entry) => entry.sourceTourId)).toEqual([
