...
export const optional = (key: string, valueType: ScalarFieldValueType, nullable = true) =>
  ({ key, valueType, transfer: 'included', input: { kind: 'optional', nullable } }) as const;

export const optionalEnum = (key: string, allowedValues: readonly string[], nullable = true) =>
  ({
    key,
    valueType: 'string',
    allowedValues,
...
export const defaultable = (key: string, valueType: ScalarFieldValueType, defaultValue: unknown) =>
  ({ key, valueType, transfer: 'included', input: { kind: 'defaultable', defaultValue } }) as const;

export const defaultableEnum = (
  key: string,
