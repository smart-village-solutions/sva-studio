...
describe('reconcileOwnedContentForAccountDelete', () => {
  it('anonymizes retained content and clears account references', async () => {
    ...
    await reconcileOwnedContentForAccountDelete(
      { query },
      {
  ...
  it('reuses the deleted content tombstone path for lifecycle-managed content', async () => {
    ...
    expect(String(query.mock.calls[1]?.[0])).not.toContain(
      "deletion_lifecycle_state IS DISTINCT FROM 'deleted'"
    );
...
describe('hardDeleteAccount', () => {
  it('purges membership-bound blocker rows before the account row is deleted', async () => {
    ...
    await purgeAccountHardDeleteBlockers(
      { query },
      {
        instanceId: 'de-musterhausen',
  ...
  it('fails closed when the target account is under an active legal hold', async () => {
    ...
    expect(query).toHaveBeenCalledWith(expect.stringContaining('FROM iam.legal_holds'), [
      'de-musterhausen',
      '33333333-3333-4333-8333-333333333333',
    ]);
  });

  it('blocks hard delete while the technical account owns an active organization provisioning lease', async () => {
    const query = vi
      .fn()
    ...
    ).rejects.toThrow('organization_provisioning_delete_protection');
  ...
  it('deletes the scoped account row after content preparation succeeded', async () => {
    ...
    expect(query).toHaveBeenCalledWith(expect.stringContaining('DELETE FROM iam.accounts'), [
      '33333333-3333-4333-8333-333333333333',
      'de-musterhausen',
    ]);
    ...
    expect(String(query.mock.calls[0]?.[0])).toContain("ELSE 'reconciliation_required'");
  });
