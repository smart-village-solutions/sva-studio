...
const normalizeLanguageFilter = (value: string) => value.trim().toLowerCase();
...
export const FaqListPage = () => {
  ...
  const normalizedLanguageCode = normalizeLanguageFilter(languageCode);
  const visibleItems = result.data.filter((item) =>
    normalizedLanguageCode.length === 0 ||
    normalizeLanguageFilter(readFaqPayload(item.payload).languageCode) === normalizedLanguageCode
  );
  ...
  return <StudioOverviewPageTemplate title={pt('list.title')} primaryAction={<Button asChild><Link to="/admin/faq/new">{pt('actions.create')}</Link></Button>}>
    ...
    ]} rowActions={(item) => <Button asChild size="sm" variant="outline"><Link to="/admin/faq/$id" params={{ id: item.id }}>{pt('actions.edit')}</Link></Button>} getRowId={(item) => item.id} selectionMode="none" emptyState={null} labels={{ selectionColumn: pt('fields.question'), actionsColumn: pt('fields.actions'), loading: pt('messages.loading'), selectAllRows: (label) => label, selectRow: ({ label }) => label }} />
