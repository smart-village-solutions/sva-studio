...
export function FaqDetailHistoryTab({ contentId, pt }: Props) {
  ...
  return <div className="overflow-x-auto"><table className="min-w-full border-collapse" aria-label={pt('history.tableLabel')}><thead><tr className="border-b border-border/70 text-left text-sm"><th className="px-3 py-2">{pt('history.columns.time')}</th><th className="px-3 py-2">{pt('history.columns.action')}</th><th className="px-3 py-2">{pt('history.columns.actor')}</th><th className="px-3 py-2">{pt('history.columns.summary')}</th></tr></thead><tbody>{entries.map((entry) => <tr key={entry.id} className="border-b border-border/50 text-sm"><td className="px-3 py-3">{formatDateTimeInEditorTimeZone(entry.createdAt) ?? entry.createdAt}</td><td className="px-3 py-3">{pt(actionKeys[entry.action])}</td><td className="px-3 py-3">{entry.actor}</td><td className="px-3 py-3">{entry.summary || (entry.changedFields.length ? pt('history.changedFields', { fields: entry.changedFields.join(', ') }) : pt('history.emptySummary'))}</td></tr>)}</tbody></table></div>;
}
