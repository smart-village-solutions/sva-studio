...
const FaqEditorForm = ({ form, onSubmit, pt }: Readonly<{ form: ReturnType<typeof useForm<FaqFormValues>>; onSubmit: (values: FaqFormValues) => void; pt: ReturnType<typeof usePluginTranslation> }>) => (
  <form className="space-y-4" noValidate onSubmit={form.handleSubmit(onSubmit)}>
    ...
    <label className="grid gap-1 text-sm font-medium" htmlFor="faq-sort-weight">{pt('fields.sortWeight')}<input id="faq-sort-weight" className="rounded-md border px-3 py-2" type="number" {...form.register('sortWeight', { valueAsNumber: true })} />{form.formState.errors.sortWeight ? <span className="text-destructive">{pt('validation.sortWeight')}</span> : null}</label>
