...
const createPoiMediaUsageLabels = (pt: PoiTranslator) => ({
  title: pt('cards.media.entries.title'),
  description: pt('cards.media.entries.description'),
  empty: pt('messages.imagePickerEmpty'),
  actions: {
    add: pt('messages.mediaPickerTitle'),
    remove: pt('actions.removeImage'),
    moveUp: pt('messages.mediaMoveUp'),
    moveDown: pt('messages.mediaMoveDown'),
    refreshMetadata: pt('messages.mediaRefreshMetadata'),
    cancel: pt('actions.back'),
    apply: pt('messages.mediaApplyMetadata'),
  },
  fields: {
    url: pt('fields.url'),
  ...
  states: {
    linked: pt('messages.mediaLinked'),
    manual: pt('messages.mediaManual'),
    synced: pt('messages.mediaReferenceSynced'),
    pending: pt('messages.mediaReferencePending'),
    missing: pt('messages.mediaReferenceMissing'),
    additional: pt('messages.mediaReferenceAdditional'),
    unresolved: pt('messages.mediaReferenceUnresolved'),
    failed: pt('messages.mediaReferenceFailed'),
  ...
  refresh: {
    ...
    assetValue: pt('messages.mediaAssetValue'),
    contentValue: pt('messages.mediaContentValue'),
  },
});
...
const PoiMediaContentTypeField = ({
  ...
  >
    <Select
      id={`content-media-${usage.uiId}-content-type`}
      value={
        typeof usage.additionalData?.contentType === 'string'
          ? usage.additionalData.contentType
          : ''
      }
      onChange={(event) =>
        update({
          additionalData: { ...usage.additionalData, contentType: event.currentTarget.value },
        })
      }
    >
...
export function PoiDetailMediaTab({
  ...
  onLoadAssetSnapshot?: NonNullable<
  ...
  const mediaErrors = Object.fromEntries(
    resolvedUsages.flatMap((usage, index) => {
      ...
      return typeof message === 'string'
        ? [[`${usage.uiId}.persistentUrl`, pt('validation.webUrls')]]
        : [];
    })
  );
    ...
    >
      <ContentMediaUsageBlock
        ...
        renderAdditionalFields={({ usage, update }) => (
          <PoiMediaContentTypeField usage={usage} update={update} pt={pt} />
        )}
