import type { WasteGlobalDateShiftRecord } from '@sva/plugin-sdk';
...
export const WasteSchedulingGlobalShiftList = ({
  shifts,
  onEdit,
  onOpenCreate,
  page,
            ...
            <tbody>
              {pagedShifts.items.map((shift) => (
                <tr key={shift.id} className="animate-row-hover border-b border-border/60 align-top text-[14px] text-foreground hover:bg-muted/20 last:border-b-0">
                  ...
                  <WasteSchedulingListActionCell ariaLabel={pt('scheduling.global.actions.edit')} onClick={() => onEdit(shift)} />
                </tr>
              ))}
            </tbody>
          </table>
