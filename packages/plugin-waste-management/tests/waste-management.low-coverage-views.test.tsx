...
vi.mock('@sva/studio-ui-react', () => ({
  ...
  Input: ({ id, value, onChange, placeholder, type = 'text' }: React.ComponentProps<'input'>) => (
...
describe('waste-management low coverage views', () => {
  ...
  it('warns before saving the holiday state when imported holidays already exist', () => {
    ...
    expect(
      screen.getByText('settings.messages.holidayStateOverwriteWarningDescription')
    ).toBeTruthy();
    expect(onSubmit).not.toHaveBeenCalled();

    fireEvent.click(
      screen.getAllByRole('button', { name: 'settings.actions.save' }).at(-1) as HTMLButtonElement
    );
  ...
  it('renders the disabled switch branch and toggles an unchecked switch directly', () => {
    ...
    rerender(
      <WasteManagementFormSwitch
        checked
        disabled
        ariaLabel="switch-label-disabled"
        onChange={onChange}
      />
    );

    expect(
      screen.getByRole('switch', { name: 'switch-label-disabled' }).hasAttribute('disabled')
    ).toBe(true);
  });
  ...
  it('renders the locations overview metrics and wires every create action', () => {
    ...
    fireEvent.click(
      screen.getByRole('button', { name: 'masterData.locationsWorkspace.actions.createRegion' })
    );
    fireEvent.click(
      screen.getByRole('button', { name: 'masterData.locationsWorkspace.actions.createCity' })
    );
    fireEvent.click(
      screen.getByRole('button', { name: 'masterData.locationsWorkspace.actions.createStreet' })
    );
    fireEvent.click(
      screen.getByRole('button', {
        name: 'masterData.locationsWorkspace.actions.createHouseNumber',
      })
    );
    fireEvent.click(
      screen.getByRole('button', { name: 'masterData.locationsWorkspace.actions.createLocation' })
    );
  ...
  it('renders hierarchy sections for populated and empty address levels and wires edit/create actions', () => {
    ...
    render(
      <WasteMasterDataLocationsHierarchy
        ...
        cities={[
          { id: 'city-1', name: 'Musterstadt', regionId: 'region-1', createdAt: '', updatedAt: '' },
        ]}
        streets={[]}
        houseNumbers={[
          { id: 'house-1', number: '12', streetId: 'street-1', createdAt: '', updatedAt: '' },
        ]}
    ...
    fireEvent.click(
      screen.getByRole('button', { name: 'masterData.houseNumbers.actions.openCreate' })
    );
  ...
  it('prefers the tour scheduling variant when tours exist, allows switching to global, and wires both form updaters', () => {
    ...
    (globalProps.onChange as (patch: Record<string, unknown>) => void)({
    ...
    (firstTourProps.onChange as (patch: Record<string, unknown>) => void)({
  ...
  it('renders the tour-assignment dialog without an explicit tour or validity-date selector', () => {
    ...
    fireEvent.submit(
      screen.getByRole('button', { name: 'tours.assignments.actions.save' }).closest('form')!
    );
