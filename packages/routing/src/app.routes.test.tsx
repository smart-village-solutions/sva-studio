...
const guardSpies = vi.hoisted(() => ({
  ...
  accountPrivacyDetail: vi.fn(async () => undefined),
  accountRules: vi.fn(async () => undefined),
  ...
  adminTemplates: vi.fn(async () => undefined),
...
describe('app.routes', () => {
  ...
  it('builds client route factories from the canonical UI definitions and plugin definitions', async () => {
    ...
    expect(routeMap.has('/account/privacy/$caseId')).toBe(true);
    ...
    await readRouteOptions(routeMap.get('/account/privacy/$caseId')).beforeLoad?.({
      href: '/account/privacy/case-1',
    });
    await readRouteOptions(routeMap.get('/account/rules')).beforeLoad?.({ href: '/account/rules' });
    await readRouteOptions(routeMap.get('/categories')).beforeLoad?.({
      href: '/categories',
      context: {
        auth: {
          getUser: async () => ({
  ...
  it('materializes platform and tenant plugin routes into separate route trees', () => {
    const plugin = {
      id: 'scope-demo',
      ...
      routes: [
        ...
        {
          id: 'scope-demo.tenant',
          path: '/plugins/scope-demo/tenant',
          accessRequirement: {
            kind: 'tenant',
            moduleId: 'scope-demo',
            actions: { mode: 'allOf', values: ['scope-demo.read'] },
          },
          component: () => 'tenant',
        },
      ],
    ...
    const materializePaths = (scope: 'platform' | 'tenant') =>
      getPluginRouteFactories([plugin], { scope })
        .map((factory) => factory({ id: 'root' } as never))
        .map((route) => readRouteOptions(route).path);

    expect(materializePaths('platform')).toEqual(['/plugins/scope-demo/platform']);
