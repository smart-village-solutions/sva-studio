...
const createUiRouteFactories = (bindings: AppRouteBindings): readonly AppRouteFactory[] =>
  uiRouteDefinitions.map((definition) => (rootRoute: RootRoute) =>
    createRoute({
      getParentRoute: () => rootRoute,
      path: definition.path,
      beforeLoad: definition.guard ? (options) => accountUiRouteGuards[definition.guard]?.(options) : undefined,
