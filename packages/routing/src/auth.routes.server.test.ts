...
const authServerMocks = vi.hoisted(() => {
  ...
  return {
    ...
    instanceRegistryHandlers: {
      ...
      prepareInstanceConfirmation: vi.fn(async () =>
        response('prepareInstanceConfirmationHandler')
      ),
    ...
    wasteManagementHandlers: {
      ...
      createAnnualTourTransfer: vi.fn(async () => response('createWasteAnnualTourTransferHandler')),
      previewAnnualTourTransfer: vi.fn(async () =>
        response('previewWasteAnnualTourTransferHandler')
      ),
    ...
    provisionOrganizationMainserverHandler: vi.fn(async () =>
      response('provisionOrganizationMainserverHandler')
    ),
...
describe('auth.routes.server', () => {
  ...
  it('dispatches waste management routes to the auth runtime', async () => {
    ...
    const annualTourTransferPreviewHandlers = resolveAuthHandlers(
    ...
    expect(holidayRuleDetailHandlers?.DELETE).toBeDefined();
    ...
    await tourAssignmentDetailHandlers.PUT?.({
      request: new Request(
        'http://localhost/api/v1/waste-management/tour-assignments/assignment-1',
        {
          method: 'PUT',
        }
      ),
    ...
    await annualTourTransferHandlers.POST?.({
      request: new Request('http://localhost/api/v1/waste-management/tours/annual-transfer', {
        method: 'POST',
      }),
    });
    await annualTourTransferPreviewHandlers.POST?.({
      request: new Request(
        'http://localhost/api/v1/waste-management/tours/annual-transfer/preview',
        { method: 'POST' }
      ),
    });
    ...
    await provisioningRetryHandlers.POST?.({
      request: new Request('http://localhost/api/v1/waste-management/settings/provisioning/retry', {
        method: 'POST',
      }),
    });
    ...
    await enrichPostalCodesHandlers.POST?.({
      request: new Request('http://localhost/api/v1/waste-management/tools/postal-codes/enrich', {
        method: 'POST',
      }),
    });
    ...
    expect(authServerMocks.wasteManagementHandlers.deleteHolidayRule).toHaveBeenCalled();
    ...
    expect(authServerMocks.wasteManagementHandlers.createAnnualTourTransfer).toHaveBeenCalled();
