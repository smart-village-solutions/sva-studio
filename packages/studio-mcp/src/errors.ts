...
const retryClassFor = (
  ...
  if (status === 429 || code === 'provisioning_retry_conflict') return 'safe';
  if (
    (code === 'keycloak_unavailable' || code === 'keycloak_create_readiness_blocked') &&
    diagnosticStatus !== 'blockiert'
  ) {
    return 'conditional';
  }
  return 'never';
...
export const normalizeError = (error: unknown): McpError => {
  if (error instanceof StudioApiError) {
    ...
    const safeDetails = objectField(contract, 'safeDetails');
    const contractRetryClass = stringField(safeDetails, 'retry_class');
    const retryClass =
      contractRetryClass === 'safe' ||
      contractRetryClass === 'conditional' ||
      contractRetryClass === 'never'
        ? contractRetryClass
        : retryClassFor(error.status, code, diagnosticStatus);
    return {
      version: '1',
      code,
      category,
      retryable: retryClass !== 'never',
      retryClass,
      summary: redactText(
        stringField(contract, 'message') ?? `Studio-API antwortete mit HTTP ${error.status}.`
      ),
      recommendedAction:
        stringField(contract, 'recommendedAction') ??
      ...
      ...(safeDetails
        ? { safeDetails: redact(safeDetails) as Readonly<Record<string, unknown>> }
        : {}),
    };
  }
  return {
