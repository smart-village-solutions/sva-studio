...
const Checkbox = React.forwardRef<HTMLInputElement, CheckboxProps>(({ className, indeterminate = false, type, ...props }, ref) => {
  ...
  return (
    <input
      ...
      type={type ?? 'checkbox'}
      className={cn(
        'h-4 w-4 rounded border border-input bg-background text-primary-foreground accent-primary checked:border-primary checked:bg-primary ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50',
        className
      )}
      {...props}
    />
