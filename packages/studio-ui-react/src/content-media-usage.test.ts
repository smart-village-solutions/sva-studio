...
describe('content media usage', () => {
  ...
  it('rejects insecure and signed URLs', () => {
    ...
    expect(isPersistableContentMediaUrl('https://cdn.example.test/image.jpg?X-Amz-Signature=secret')).toBe(false);
    expect(isPersistableContentMediaUrl('https://cdn.example.test/image.jpg?X-Amz-Expires=900')).toBe(false);
    expect(isPersistableContentMediaUrl('https://cdn.example.test/image.jpg?sv=2024-11-04&sig=secret')).toBe(false);
    expect(isPersistableContentMediaUrl('https://user:secret@cdn.example.test/image.jpg')).toBe(false);
    expect(isPersistableContentMediaUrl('https://cdn.example.test/image.jpg?width=1280')).toBe(true);
  });
  ...
  it('creates references only for asset-backed usages', () => {
    ...
    expect(contentMediaUsageToReference({ ...manual, assetId: 'asset-1', role: 'gallery_item', sortOrder: 2 })).toEqual({
      assetId: 'asset-1',
      role: 'gallery_item',
      sortOrder: 2,
    });
