...
export const createWasteOperations = (executeGraphqlWithConfig: GraphqlExecutor) => ({
  listWasteSyncSnapshotWithConfig: async (
        ...
        return (
          response.wasteLocationTypes
            ...
            .flatMap((locationType) =>
              (locationType.pickUpTimes ?? [])
                .flatMap((pickupTime) =>
                  pickupTime
                    ? [mapPickupTime(locationType, pickupTime)]
                    : []
                )
            ) ?? []
        );
  ...
  createWastePickupTimesWithConfig: async (
  ...
  deleteWastePickupTimesWithConfig: async (
    input: SvaMainserverConnectionInput & { readonly items: readonly SvaMainserverWasteSyncItem[] },
    config: SvaMainserverInstanceConfig
  ): Promise<void> => {
