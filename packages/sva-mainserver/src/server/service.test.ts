...
describe('createSvaMainserverService', () => {
  ...
  it('keeps explicit operator and accessibility clearing values in POI update mutations', async () => {
    state.loadSvaMainserverInstanceConfig.mockResolvedValue(baseConfig);
    state.readEffectiveSvaMainserverCredentialsWithStatus.mockResolvedValue({
      status: 'ok',
      source: 'user',
  ...
  it('supports updatedAt ordering for internal news sync callers', async () => {
    const item = {
      id: 'news-1',
      title: 'News',
      payload: { teaser: 'Kurztext', body: '<p>Body</p>' },
