...
- **Fallow vorher:** `mapEventsDetailFormValuesToInput` — cyclomatic 42, cognitive 53, 146 Zeilen, CRAP 48 critical bei hoher geschätzter Coverage; fünf weitere Findings, Datei 406 Zeilen, MI 75,2, sieben direkte Konsumenten; Medienclone `dup:714de343` 192 Tokens.
...
6. `pnpm exec fallow audit --base origin/main --workspace @sva/plugin-events --explain --format json`: PASS und alle introduced-Zähler inklusive Styling 0; Coverage-Audit falls CRAP attribution coverageabhängig ist.

## Fertig

- Alle Ziel-Findings in `events.detail-form.ts` beseitigt, ohne neuen Moderate-CRAP-Fund.
