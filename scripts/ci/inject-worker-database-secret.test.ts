...
describe('worker database secret injection', () => {
  ...
  it('adds the dedicated secret without changing existing config values', () => {
    expect(
      injectWorkerDatabaseSecret('APP_DB_PASSWORD=existing\nAPP_DB_USER=sva_app\n', secret)
    ).toBe(
  ...
  it('replaces a legacy bundled worker password with the dedicated secret', () => {
    ...
    expect(injectWorkerDatabaseSecret(source, secret)).toContain(
      `STUDIO_JOB_WORKER_DB_PASSWORD=${secret}`
    );
  });
  ...
  it('rejects surrounding whitespace instead of bypassing password separation', () => {
    expect(() => injectWorkerDatabaseSecret(`APP_DB_PASSWORD=${secret}\n`, ` ${secret} `)).toThrow(
      /fehlt, ist zu kurz oder enthält/u
    );
  });

  it('compares protected passwords after trimming their config representation', () => {
    expect(() => injectWorkerDatabaseSecret(`POSTGRES_PASSWORD= ${secret} \n`, secret)).toThrow(
      /unterscheiden/u
    );
  });
