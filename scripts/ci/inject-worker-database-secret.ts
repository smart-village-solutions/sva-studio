...
export const injectWorkerDatabaseSecret = (source: string, secret: string): string => {
  ...
  if (secret !== normalizedSecret || normalizedSecret.length < 32 || /[\r\n]/u.test(secret)) {
  ...
  if (protectedPasswords.includes(normalizedSecret)) {
    throw new Error(
      `${workerPasswordKey} muss sich von den bestehenden Datenbank-Passwörtern unterscheiden`
    );
  }
  ...
  return `${retainedLines.join('\n')}\n${workerPasswordKey}=${normalizedSecret}\n`;
};
...
export const runInjectWorkerDatabaseSecret = (
  ...
  try {
    ...
    writeFileSync(inputPath, injectWorkerDatabaseSecret(source, secret), {
      encoding: 'utf8',
      mode: 0o600,
    });
    return 0;
