...
export const backupCommand = [
  ...
  'require_command wc 87',
  'require_command cut 88',
  'workdir="$(mktemp -d)"',
  ...
  'dump_bytes_file="$workdir/backup.bytes"',
  'export PGPASSWORD="$POSTGRES_PASSWORD"',
  'backup_step pg_dump 91 pg_dump --format=custom --no-owner --no-privileges --host "$POSTGRES_HOST" --port "$POSTGRES_PORT" --username "$POSTGRES_USER" --file "$dump" "$POSTGRES_DB"',
  'backup_step dump_nonempty 92 test -s "$dump"',
  "backup_step dump_bytes 90 sh -c 'wc -c < \"$1\" > \"$2\"' -- \"$dump\" \"$dump_bytes_file\"",
  'read -r dump_bytes < "$dump_bytes_file" || exit 90',
  'backup_event dump_bytes succeeded 0 "$dump_bytes"',
  "backup_step checksum_create 93 sh -c 'sha256sum \"$1\" > \"$2\"' -- \"$dump\" \"$checksum\"",
  'backup_step minio_upload_dump 94 aws --endpoint-url "$S3_ENDPOINT" s3 cp "$dump" "s3://$S3_BUCKET/$S3_OBJECT_KEY" --only-show-errors',
  'backup_step minio_upload_checksum 95 aws --endpoint-url "$S3_ENDPOINT" s3 cp "$checksum" "s3://$S3_BUCKET/$S3_OBJECT_KEY.sha256" --only-show-errors',
  'backup_step minio_download_dump 96 aws --endpoint-url "$S3_ENDPOINT" s3 cp "s3://$S3_BUCKET/$S3_OBJECT_KEY" "$download" --only-show-errors',
  'backup_step download_nonempty 97 test -s "$download"',
  "backup_step size_verify 98 sh -c 'test \"$(wc -c < \"$1\")\" -eq \"$(wc -c < \"$2\")\"' -- \"$dump\" \"$download\"",
  "backup_step checksum_verify 99 sh -c 'printf \"%s  %s\\n\" \"$(cut -d \" \" -f1 \"$1\")\" \"$2\" | sha256sum -c -' -- \"$checksum\" \"$download\"",
  "backup_step archive_validate 100 sh -c 'pg_restore --list \"$1\" >/dev/null' -- \"$download\"",
