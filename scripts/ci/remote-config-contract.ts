export type RemoteEnvironment = 'dev' | 'staging' | 'prod';
export type RemoteConfigKind = 'config' | 'secret-value' | 'secret-reference';

type RemoteConfigKeyContract = Readonly<{
  kind: RemoteConfigKind;
  required?: boolean;
  type?: 'boolean' | 'integer' | 'url' | 'string';
  allowedValues?: readonly string[];
}>;

const configKeys = [
  'APP_DB_USER',
  'ENABLE_OTEL',
  'STUDIO_JOB_WORKER_DB_USER',
  'IAM_ADMIN_ENABLED',
  'IAM_BULK_ENABLED',
  'IAM_CSRF_ALLOWED_ORIGINS',
  'IAM_DEBUG_PROFILE_ERRORS',
  'IAM_PII_ACTIVE_KEY_ID',
  'IAM_UI_ENABLED',
  'KEYCLOAK_ADMIN_BASE_URL',
  'KEYCLOAK_ADMIN_CLIENT_ID',
  'KEYCLOAK_ADMIN_REALM',
  'KEYCLOAK_PROVISIONER_BASE_URL',
  'KEYCLOAK_PROVISIONER_CLIENT_ID',
  'KEYCLOAK_PROVISIONER_REALM',
  'POSTGRES_DB',
  'POSTGRES_USER',
  'SVA_ALLOWED_INSTANCE_IDS',
  'SVA_AUTH_CLIENT_ID',
  'SVA_AUTH_DEBUG_HEADERS',
  'SVA_AUTH_ISSUER',
  'SVA_AUTH_POST_LOGOUT_REDIRECT_URI',
  'SVA_AUTH_REDIRECT_URI',
  'SVA_ENABLE_MONITORING',
  'SVA_ENABLE_SERVER_CONSOLE_LOGS',
  'SVA_KEYCLOAK_PROVISIONER_POLL_INTERVAL_MS',
  'SVA_DEPLOYMENT_ENVIRONMENT',
  'SVA_DOCUMENTATION_BASE_URL',
  'SVA_PARENT_DOMAIN',
  'SVA_STUDIO_ROOT_HOST',
  'SVA_PUBLIC_BASE_URL',
  'SVA_RUNTIME_PROFILE',
  'SVA_STACK_NAME',
  'SVA_STUDIO_BRANDING',
  'SVA_STUDIO_MCP_AUDIENCE',
  'SVA_STUDIO_MCP_CLIENT_ID',
  'SVA_STUDIO_MCP_ENABLED',
  'SVA_STUDIO_MCP_ISSUER',
  'VITE_IAM_ADMIN_ENABLED',
  'VITE_IAM_BULK_ENABLED',
  'VITE_IAM_UI_ENABLED',
  'SSF_PLUGIN_DATABASE_ENABLED',
  'SSF_PLUGIN_DATABASE_NAME',
  'SSF_PLUGIN_RUNTIME_DB_USER',
  'SSF_PLUGIN_ROOT_DB_USER',
  'SVA_STUDIO_SSF_RUNTIME_AUDIENCE',
  'SVA_STUDIO_SSF_RUNTIME_CLIENT_ID',
  'SVA_STUDIO_SSF_RUNTIME_ENABLED',
  'SVA_STUDIO_SSF_LOGIN_ORIGIN',
  'SVA_STUDIO_SSF_RUNTIME_ISSUER',
  'KEYCLOAK_OPERATOR_USER',
  'OTEL_EXPORTER_OTLP_ENDPOINT',
  'OTEL_SERVICE_NAME',
  'RESTORE_IAM_SMOKE_BASE_URL',
  'RESTORE_IAM_SMOKE_USERNAME',
  'S3_BUCKET_PROD',
  'S3_BUCKET_STAGING',
  'S3_URL',
  'SVA_DEPLOY_REVISION',
  'SVA_IMAGE_DIGEST',
  'SVA_IMAGE_REF',
  'SVA_IMAGE_TAG',
  'SVA_MAINSERVER_CLIENT_ID',
  'SVA_MAINSERVER_GRAPHQL_URL',
  'SVA_MAINSERVER_OAUTH_TOKEN_URL',
  'SVA_MAINSERVER_SCOPE_RESOLVER_MODE',
  'SVA_MONITORING_CONFIG_INIT_IMAGE_TAG',
  'SVA_MONITORING_REGISTRY',
  'QUANTUM_ENDPOINT',
  'QUANTUM_ENDPOINT_ID',
  'SVA_IMAGE_REPOSITORY',
  'SVA_MAINSERVER_REQUIRED',
  'SVA_MIGRATION_STATUS_REQUIRED',
  'SVA_PUBLIC_HOST',
  'SVA_QUANTUM_NO_PRE_PULL',
  'SVA_REGISTRY',
  'SVA_TENANT_REALM_OVERRIDES',
] as const;

const secretValueKeys = [
  'APP_DB_PASSWORD',
  'STUDIO_JOB_WORKER_DB_PASSWORD',
  'ENCRYPTION_KEY',
  'IAM_PII_KEYRING_JSON',
  'KEYCLOAK_ADMIN_CLIENT_SECRET',
  'KEYCLOAK_PROVISIONER_CLIENT_SECRET',
  'POSTGRES_PASSWORD',
  'REDIS_PASSWORD',
  'SVA_AUTH_CLIENT_SECRET',
  'SVA_AUTH_STATE_SECRET',
  'KEYCLOAK_OPERATOR_PASSWORD',
  'RESTORE_IAM_SMOKE_PASSWORD',
  'RESTORE_PROD_POSTGRES_PASSWORD',
  'RESTORE_PROD_SIGNING_KEY',
  'RESTORE_STAGING_POSTGRES_PASSWORD',
  'RESTORE_STAGING_SIGNING_KEY',
  'IAM_DATABASE_URL',
  'REDIS_URL',
  'S3_ACCESS_KEY_ID',
  'S3_SECRET_ACCESS_KEY',
  'SVA_MAINSERVER_CLIENT_SECRET',
  'SSF_PLUGIN_RUNTIME_DB_PASSWORD',
  'SSF_PLUGIN_ROOT_DB_PASSWORD',
  'SVA_STUDIO_SSF_DATABASE_URL',
  'SVA_STUDIO_SSF_ROOT_DATABASE_URL',
] as const;

export const remoteConfigContract: Readonly<Record<string, RemoteConfigKeyContract>> = {
  ...Object.fromEntries(configKeys.map((key) => [key, { kind: 'config' as const }])),
  ...Object.fromEntries(secretValueKeys.map((key) => [key, { kind: 'secret-value' as const }])),
  WASTE_DATABASE_PROVISIONER_PASSWORD_SECRET_NAME: { kind: 'secret-reference', required: true },
  ENABLE_OTEL: { kind: 'config', type: 'boolean' },
  QUANTUM_ENDPOINT_ID: { kind: 'config', type: 'integer' },
  SVA_ENABLE_MONITORING: { kind: 'config', type: 'boolean' },
  SVA_MAINSERVER_REQUIRED: { kind: 'config', type: 'boolean' },
  SVA_MIGRATION_STATUS_REQUIRED: { kind: 'config', type: 'boolean' },
  SVA_QUANTUM_NO_PRE_PULL: { kind: 'config', type: 'boolean' },
  SVA_STUDIO_BRANDING: {
    kind: 'config',
    type: 'string',
    allowedValues: ['sva-studio', 'kassel-dialog'],
  },
  SVA_STUDIO_MCP_ENABLED: { kind: 'config', required: true, type: 'boolean' },
  SSF_PLUGIN_DATABASE_ENABLED: { kind: 'config', type: 'boolean' },
  SVA_STUDIO_SSF_RUNTIME_ENABLED: { kind: 'config', type: 'boolean' },
  SVA_KEYCLOAK_PROVISIONER_POLL_INTERVAL_MS: { kind: 'config', required: true, type: 'integer' },
  SVA_PUBLIC_BASE_URL: { kind: 'config', required: true, type: 'url' },
  SVA_DOCUMENTATION_BASE_URL: { kind: 'config', required: true, type: 'url' },
  SVA_AUTH_ISSUER: { kind: 'config', required: true, type: 'url' },
  SVA_AUTH_REDIRECT_URI: { kind: 'config', required: true, type: 'url' },
  SVA_DEPLOYMENT_ENVIRONMENT: {
    kind: 'config',
    required: true,
    type: 'string',
    allowedValues: ['development', 'staging', 'production'],
  },
  SVA_MAINSERVER_SCOPE_RESOLVER_MODE: {
    kind: 'config',
    required: true,
    type: 'string',
    allowedValues: ['shadow', 'automatic', 'compatibility'],
  },
};

const requiredKeys = new Set([
  'APP_DB_PASSWORD',
  'APP_DB_USER',
  'STUDIO_JOB_WORKER_DB_USER',
  'ENCRYPTION_KEY',
  'IAM_ADMIN_ENABLED',
  'IAM_BULK_ENABLED',
  'IAM_CSRF_ALLOWED_ORIGINS',
  'IAM_PII_ACTIVE_KEY_ID',
  'IAM_PII_KEYRING_JSON',
  'IAM_UI_ENABLED',
  'KEYCLOAK_ADMIN_BASE_URL',
  'KEYCLOAK_ADMIN_CLIENT_ID',
  'KEYCLOAK_ADMIN_CLIENT_SECRET',
  'KEYCLOAK_ADMIN_REALM',
  'KEYCLOAK_PROVISIONER_BASE_URL',
  'KEYCLOAK_PROVISIONER_CLIENT_ID',
  'KEYCLOAK_PROVISIONER_CLIENT_SECRET',
  'KEYCLOAK_PROVISIONER_REALM',
  'POSTGRES_DB',
  'POSTGRES_PASSWORD',
  'POSTGRES_USER',
  'REDIS_PASSWORD',
  'SVA_ALLOWED_INSTANCE_IDS',
  'SVA_AUTH_CLIENT_ID',
  'SVA_AUTH_CLIENT_SECRET',
  'SVA_AUTH_ISSUER',
  'SVA_AUTH_POST_LOGOUT_REDIRECT_URI',
  'SVA_AUTH_REDIRECT_URI',
  'SVA_AUTH_STATE_SECRET',
  'SVA_PARENT_DOMAIN',
  'SVA_PUBLIC_BASE_URL',
  'SVA_DEPLOYMENT_ENVIRONMENT',
  'SVA_RUNTIME_PROFILE',
  'SVA_STACK_NAME',
  'SVA_STUDIO_MCP_ENABLED',
]);

export const requiredRemoteConfigKeys = Object.entries(remoteConfigContract)
  .filter(([key, contract]) => contract.required || requiredKeys.has(key))
  .map(([key]) => key)
  .sort();
