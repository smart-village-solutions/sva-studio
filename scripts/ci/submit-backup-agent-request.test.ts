...
import { backupAgentAcceptanceTimeoutMs, buildBackupAgentEvidence, buildBackupAgentRequest } from './submit-backup-agent-request.ts';

describe('backup agent request submission', () => {
  it('creates a short-lived environment-bound request', () => {
    expect(buildBackupAgentRequest({ environment: 'prod', deployImageDigest: `sha256:${'b'.repeat(64)}`, now: new Date('2026-07-30T10:00:00.000Z'), requestId: 'gha-12345678-1' })).toEqual({
  ...
  it.each(['person@example.test', 'https://internal.example.test/jobs/1', 'first line\nsecond line', 'secret-key=should-not-leak'])(
    'rejects unsafe published object keys: %s',
    (sentinel) => {
      const request = buildBackupAgentRequest({ environment: 'prod', deployImageDigest: `sha256:${'d'.repeat(64)}`, now: new Date('2026-07-30T10:00:00.000Z'), requestId: 'gha-12345678-1' });
      expect(() => buildBackupAgentEvidence(request, { bytes: 42, objectKey: sentinel, sha256: 'e'.repeat(64), status: 'succeeded' })).toThrow('Evidence-Vertrag');
    },
  );
  ...
  it.each(['person@example.test', 'https://internal.example.test/jobs/1', 'first line\nsecond line'])(
    ...
    (sentinel) => {
      const result = spawnSync(process.execPath, ['--import', 'tsx', resolve(import.meta.dirname, 'submit-backup-agent-request.ts'), sentinel], { encoding: 'utf8' });
