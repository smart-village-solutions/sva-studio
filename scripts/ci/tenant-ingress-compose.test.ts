...
const profiles = [
  ...
  {
    composeFile: 'deploy/compose.staging.yaml',
    environment: 'staging',
    router: 'studio-staging-app',
  },
...
const parseV2Hosts = (rule: string) =>
  [...rule.matchAll(/Host\(`([^`]+)`\)/gu)].map((match) => match[1]);

describe.each(profiles)(
  '$environment tenant ingress compose contract',
  ({ composeFile, environment, router }) => {
    ...
    it('binds the live app service to the selected non-sensitive config revision', () => {
      expect(readLabel(source, 'sva.config.revision')).toBe('${SVA_CONFIG_REVISION}');
    });
  }
);
