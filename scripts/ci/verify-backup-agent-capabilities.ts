...
export const validateBackupAgentCapabilities = (
  environment: BackupEnvironment,
  value: unknown,
  requireWaste: boolean,
  requireSsf = false
): BackupAgentCapabilities => {
  ...
  const compatible =
    ...
    typeof capabilities.agentRevision === 'string' &&
    capabilities.agentRevision.length > 0 &&
    Array.isArray(capabilities.databaseTargets) &&
    capabilities.databaseTargets.includes('studio') &&
    (!requireSsf || capabilities.databaseTargets.includes('ssf')) &&
    (!requireWaste ||
      (capabilities.databaseTargets.includes('waste') && capabilities.wasteInventory === true)) &&
    Array.isArray(capabilities.resultFields) &&
    requiredResultFields.every((field) => capabilities.resultFields?.includes(field));
  if (!compatible)
    throw new PromoteContractError({
      code: 'PROMOTE_BACKUP_AGENT_INCOMPATIBLE',
      environment,
      phase: 'backup-capabilities',
      retryable: false,
      summary: 'Der laufende Backup-Agent erfuellt den erforderlichen Consumer-Vertrag nicht.',
      nextAction:
        'Zuerst einen kompatiblen Backup-Agenten ausrollen und dessen Capabilities live nachweisen.',
    });
...
const requestOidcToken = async () => {
  ...
  if (!response.ok || typeof payload.value !== 'string')
    throw new Error('GitHub OIDC Token konnte nicht bezogen werden.');
...
const main = async () => {
  ...
  if (environment !== 'staging' && environment !== 'prod')
    throw new Error('Environment muss staging oder prod sein.');
  const url = new URL(
    backupEnvironmentConfig(environment).endpoint.replace('/requests', '/capabilities')
  );
  url.searchParams.set('environment', environment);
  const response = await fetch(url, {
    headers: { authorization: `Bearer ${await requestOidcToken()}` },
    signal: AbortSignal.timeout(10_000),
  });
  ...
  const capabilities = validateBackupAgentCapabilities(
    environment,
    await response.json(),
    process.env.WASTE_POSTGRES_BACKUP_ENABLED === 'true',
    process.env.SSF_POSTGRES_BACKUP_ENABLED === 'true'
  );
  const evidence = {
    agentRevision: capabilities.agentRevision,
    databaseTargets: capabilities.databaseTargets,
    protocolVersions: capabilities.protocolVersions,
    resultFields: capabilities.resultFields,
    wasteInventory: capabilities.wasteInventory,
  };
  process.stdout.write(`${JSON.stringify(evidence)}\n`);
  if (process.env.GITHUB_OUTPUT)
    appendFileSync(process.env.GITHUB_OUTPUT, `backup_agent=${JSON.stringify(evidence)}\n`, 'utf8');
};

if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href)
  main().catch((error: unknown) => {
    const failure = redactPromoteFailure(error, {
      environment: process.argv[2] === 'prod' ? 'prod' : 'staging',
      phase: 'backup-capabilities',
    });
