...
export const assertStandardProductionReadiness = (input: {
  environment: 'dev' | 'staging' | 'prod';
  mode: 'standard' | 'recovery';
  status: number;
}) => {
  if (input.environment === 'prod' && input.mode === 'standard' && input.status !== 200)
    throw new PromoteContractError({
      code: 'PROMOTE_READINESS_NOT_READY',
      environment: 'prod',
      phase: 'static-preflight',
      retryable: false,
      summary: `Production-Readiness ist vor der Mutation nicht bereit (HTTP ${input.status}).`,
      nextAction:
        'Ursache read-only diagnostizieren oder den kontrollierten Recovery-Modus mit dokumentiertem Grund freigeben.',
    });
};
...
const main = async () => {
  ...
  if (!['dev', 'staging', 'prod'].includes(environment))
    throw new Error('Environment ist ungueltig.');
  const mode = validatePromoteMode({
    environment,
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href)
  main().catch((error: unknown) => {
    const failure = redactPromoteFailure(error, {
      environment:
        process.argv[2] === 'prod' ? 'prod' : process.argv[2] === 'staging' ? 'staging' : 'dev',
      phase: 'static-preflight',
    });
