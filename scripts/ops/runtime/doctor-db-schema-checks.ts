...
export const createDoctorDbSchemaChecks = (deps: RuntimeDoctorDbCheckDeps) => {
  ...
  const buildSchemaGuardCheck = (runtimeProfile: RuntimeProfile, env: NodeJS.ProcessEnv): DoctorCheck => {
    ...
    try {
      ...
      return report.ok
        ? deps.toDoctorCheck('schema-guard', 'ok', 'schema_ok', 'Kritische IAM-Schema-Artefakte sind vorhanden.', { checks: report.checks })
        : deps.toDoctorCheck('schema-guard', 'error', 'schema_drift', deps.summarizeSchemaGuardFailures(report) ?? 'Kritische IAM-Schema-Artefakte fehlen oder weichen ab.', { checks: report.checks });
    } catch (error) {
      const recovered = recoverSchemaGuardReportFromOutput(error instanceof Error ? error.message : String(error));
      if (recovered) {
        return recovered.ok
          ? deps.toDoctorCheck('schema-guard', 'ok', 'schema_ok', 'Kritische IAM-Schema-Artefakte sind vorhanden.', { checks: recovered.checks, recoveredFromTransportNoise: true })
          : deps.toDoctorCheck('schema-guard', 'error', 'schema_drift', deps.summarizeSchemaGuardFailures(recovered) ?? 'Kritische IAM-Schema-Artefakte fehlen oder weichen ab.', { checks: recovered.checks, recoveredFromTransportNoise: true });
      }
      return deps.toDoctorCheck('schema-guard', 'error', 'schema_check_failed', error instanceof Error ? error.message : String(error));
    }
  };
  ...
  const buildMigrationStatusCheck = (runtimeProfile: RuntimeProfile, env: NodeJS.ProcessEnv): DoctorCheck => {
    ...
    try {
      ...
      if (!migrationStatusRequired) {
        return deps.toDoctorCheck('migration-status', 'skipped', 'migration_status_optional', 'Remote-Goose-Status ist fuer dieses Runtime-Profil optional und blockiert die fruehe Studio-Testphase nicht.', { gooseVersion: deps.getGooseConfiguredVersion(), reason: error instanceof Error ? error.message : String(error) });
      }
      return deps.toDoctorCheck('migration-status', deps.isRemoteRuntimeProfile(runtimeProfile) ? 'warn' : 'error', deps.isRemoteRuntimeProfile(runtimeProfile) ? 'goose_status_unavailable' : 'goose_status_failed', error instanceof Error ? error.message : String(error), { gooseVersion: deps.getGooseConfiguredVersion() });
    }
  };
  ...
  return { buildMigrationStatusCheck, buildSchemaGuardCheck, buildSchemaSnapshotCheck, runSchemaGuard } as const;
};
