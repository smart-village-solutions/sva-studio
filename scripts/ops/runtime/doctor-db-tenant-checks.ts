...
export const createDoctorDbTenantChecks = (deps: RuntimeDoctorDbCheckDeps) => {
  const buildInstanceHostnameMappingCheck = async (runtimeProfile: RuntimeProfile, env: NodeJS.ProcessEnv): Promise<DoctorCheck> => {
    ...
    const expectedHostnames = tenantTargets.map((tenantTarget) => ({ hostname: tenantTarget.host, instanceId: tenantTarget.instanceId }));
...
SELECT json_build_object(
  ...
  COALESCE(
    (
      ...
      FROM (VALUES ${expectedHostnames.map(({ hostname, instanceId }) => `(${deps.sqlLiteral(hostname)}, ${deps.sqlLiteral(instanceId)})`).join(',\n        ')}) AS expected(hostname, instance_id)
      LEFT JOIN (
    ...
    try {
      const payload = deps.parseJsonFromCommandOutput<{ missing_hostnames?: string[] }>(deps.createDbSqlRunner(runtimeProfile, env)(sql));
  ...
  const buildLocalInstanceIdentityDoctorCheck = (runtimeProfile: RuntimeProfile, env: NodeJS.ProcessEnv): DoctorCheck => {
  ...
  const buildTenantAuthSecretContractCheck = async (runtimeProfile: RuntimeProfile, env: NodeJS.ProcessEnv): Promise<DoctorCheck> => {
    if (!deps.getRuntimeProfileDefinition(runtimeProfile).isLocal || runtimeProfile === 'local-builder') return deps.toDoctorCheck('tenant-auth-secret-contract', 'skipped', 'tenant_auth_secret_not_applicable', 'Tenant-Auth-Secret-Pruefung ist fuer dieses Runtime-Profil nicht anwendbar.');
    try {
      const states = await deps.loadActiveLocalTenantSecretStates(env);
      const unreadableInstanceIds = states.filter((state) => state.authClientSecretConfigured && !state.authClientSecretReadable).map((state) => state.instanceId);
      if (unreadableInstanceIds.length > 0) return deps.toDoctorCheck('tenant-auth-secret-contract', 'error', 'tenant_auth_client_secret_unreadable', 'Mindestens ein aktiver Tenant hat ein konfiguriertes, aber lokal nicht lesbares Auth-Secret.', { unreadableInstanceIds });
      const missingInstanceIds = states.filter((state) => !state.authClientSecretConfigured).map((state) => state.instanceId);
  ...
  const buildTenantAdminSecretContractCheck = async (runtimeProfile: RuntimeProfile, env: NodeJS.ProcessEnv): Promise<DoctorCheck> => {
    if (!deps.getRuntimeProfileDefinition(runtimeProfile).isLocal || runtimeProfile === 'local-builder') return deps.toDoctorCheck('tenant-admin-secret-contract', 'skipped', 'tenant_admin_secret_not_applicable', 'Tenant-Admin-Secret-Pruefung ist fuer dieses Runtime-Profil nicht anwendbar.');
    try {
      const relevantStates = (await deps.loadActiveLocalTenantSecretStates(env)).filter((state) => state.tenantAdminClientConfigured);
      const unreadableInstanceIds = relevantStates.filter((state) => state.tenantAdminClientSecretConfigured && !state.tenantAdminClientSecretReadable).map((state) => state.instanceId);
      if (unreadableInstanceIds.length > 0) return deps.toDoctorCheck('tenant-admin-secret-contract', 'error', 'tenant_admin_client_secret_unreadable', 'Mindestens ein aktiver Tenant hat ein konfiguriertes, aber lokal nicht lesbares Tenant-Admin-Secret.', { unreadableInstanceIds });
      const missingInstanceIds = relevantStates.filter((state) => !state.tenantAdminClientSecretConfigured).map((state) => state.instanceId);
      return missingInstanceIds.length > 0
  ...
  return {
    ...
    buildLocalInstanceIdentityDoctorCheck,
    buildTenantAdminSecretContractCheck,
    buildTenantAuthSecretContractCheck,
  } as const;
};
