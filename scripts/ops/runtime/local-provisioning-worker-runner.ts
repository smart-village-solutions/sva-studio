...
export const parseLocalProvisioningWorkerRunnerArgs = (argv: readonly string[]): RunnerArgs => {
  ...
  if (!profile || !logFile || !stateFile) {
    throw new Error(
      'Usage: tsx scripts/ops/runtime/local-provisioning-worker-runner.ts --profile=<profile> --log-file=<path> --state-file=<path>',
    );
  }
...
export const clearLocalWorkerStateIfOwned = (stateFile: string, pid: number) => {
  if (!existsSync(stateFile)) {
    return;
  }

  try {
    const parsed = JSON.parse(readFileSync(stateFile, 'utf8')) as LocalWorkerRunnerState;
    if (parsed.pid === pid) {
...
const appendRunnerLogLine = (logFile: string, message: string) => {
  appendFileSync(logFile, `[local-provisioning-worker-runner ${new Date().toISOString()}] ${message}\n`, 'utf8');
};
...
export const runLocalProvisioningWorkerRunner = async (argv: readonly string[]) => {
  const { logFile, profile, stateFile } = parseLocalProvisioningWorkerRunnerArgs(argv);
  ...
  if (child.pid === undefined) {
    ...
    clearLocalWorkerStateIfOwned(stateFile, process.pid);
    throw new Error('Kindprozess fuer lokalen Keycloak-Provisioning-Worker konnte nicht gestartet werden.');
  }
...
if (isMainModule()) {
  void runLocalProvisioningWorkerRunner(process.argv.slice(2)).catch((error) => {
