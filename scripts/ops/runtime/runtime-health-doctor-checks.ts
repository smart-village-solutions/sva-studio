...
import type { DoctorCheck, RemoteRuntimeProfile } from '../runtime-env.shared.ts';
import { buildOidcClientSecretProbes, evaluateOidcClientSecretProbeResponse, getObservabilitySummary, normalizeBaseUrl, parseLiveRuntimeFlags, readJsonResponse } from './runtime-health-helpers.ts';
...
export const createRuntimeHealthDoctorChecks = (deps: RuntimeHealthDeps) => {
  const readLiveRuntimeFlags = async (env: NodeJS.ProcessEnv): Promise<LiveRuntimeFlags> => {
    ...
    const serializedEnv = Object.entries(liveContract.env).map(([key, value]) => `${key}=${value}`).join('\n');
  ...
  const queryRecentLokiLinesWithRetry = async (env: NodeJS.ProcessEnv, query: string, options: { attempts?: number; delayMs?: number; limit?: number } = {}) => {
  ...
  const buildLiveRuntimeEnvCheck = async (runtimeProfile: RemoteRuntimeProfile, env: NodeJS.ProcessEnv): Promise<DoctorCheck> => {
    try {
      const liveFlags = await readLiveRuntimeFlags(env);
      const expectedFlags = { ENABLE_OTEL: env.ENABLE_OTEL?.trim() || '', SVA_ENABLE_SERVER_CONSOLE_LOGS: env.SVA_ENABLE_SERVER_CONSOLE_LOGS?.trim() || '', SVA_RUNTIME_PROFILE: runtimeProfile };
      const mismatches = Object.entries(expectedFlags).filter(([key, expectedValue]) => liveFlags[key as keyof LiveRuntimeFlags] !== expectedValue).map(([key, expectedValue]) => ({ actual: liveFlags[key as keyof LiveRuntimeFlags], expected: expectedValue, key }));
  ...
  const buildObservabilityDoctorCheck = async (runtimeProfile: RuntimeProfile, env: NodeJS.ProcessEnv): Promise<DoctorCheck> => {
    ...
    try {
      ...
      const lines = await queryRecentLokiLinesWithRetry(env, `{swarm_stack="${stackName}",swarm_service="${stackName}_app"} |= "observability_"`, { attempts: 3, delayMs: 2_000, limit: 50 });
      ...
      return deps.toDoctorCheck('observability-readiness', 'warn', error instanceof Error && error.message === 'loki_probe_unconfigured' ? 'loki_probe_unconfigured' : 'loki_probe_failed', error instanceof Error ? error.message : String(error), summary);
    }
  };

  const buildTenantAuthProofCheck = async (runtimeProfile: RemoteRuntimeProfile, env: NodeJS.ProcessEnv): Promise<DoctorCheck> => {
    const tenantTargetResolution = await deps.resolveTenantRuntimeTargets(runtimeProfile, env, { limit: 2 });
    if (tenantTargetResolution.targets.length === 0) return deps.toDoctorCheck('tenant-auth-proof', 'skipped', 'tenant_auth_optional', 'Kein Tenant-Auth-Proof konfiguriert.', { source: tenantTargetResolution.source });
    ...
    for (const tenantTarget of tenantTargetResolution.targets.slice(0, 2)) {
      ...
      if (response.status !== 302 || !location.includes(`/realms/${tenantTarget.authRealm}/`)) {
        return deps.toDoctorCheck('tenant-auth-proof', 'error', 'tenant_auth_redirect_failed', `Tenant-Login fuer ${tenantTarget.instanceId} liefert keinen korrekten Realm-Redirect.`, { authRealm: tenantTarget.authRealm, host: tenantTarget.host, instanceId: tenantTarget.instanceId, location, source: tenantTargetResolution.source, status: response.status });
      }
    ...
    try {
      ...
      for (const probe of probeResults) {
        const lines = await queryRecentLokiLinesWithRetry(env, `{swarm_stack="${stackName}",swarm_service="${stackName}_app"} |= "tenant_auth_resolution_summary" |= "${probe.instanceId}"`, { attempts: 3, delayMs: 2_000, limit: 20 });
        if (lines.length === 0) missingEvidence.push(probe.instanceId);
      }
      return missingEvidence.length > 0
        ? deps.toDoctorCheck('tenant-auth-proof', 'warn', 'tenant_auth_log_missing', 'Tenant-Redirects sind korrekt, aber Loki enthaelt noch nicht fuer alle Tenant-Probes die passenden Resolution-Logs.', { missingEvidence, probeResults, source: tenantTargetResolution.source })
        : deps.toDoctorCheck('tenant-auth-proof', 'ok', 'tenant_auth_resolution_logged', 'Tenant-Redirects und zugehoerige Resolution-Logs sind vorhanden.', { probeResults, source: tenantTargetResolution.source });
    } catch (error) {
      return deps.toDoctorCheck('tenant-auth-proof', 'warn', 'tenant_auth_log_probe_failed', error instanceof Error ? error.message : String(error), { probeResults, source: tenantTargetResolution.source });
    }
  };

  const buildKeycloakClientSecretCheck = async (runtimeProfile: RuntimeProfile, env: NodeJS.ProcessEnv): Promise<DoctorCheck> => {
    ...
    if (probes.length === 0) return deps.toDoctorCheck('keycloak-client-secrets', 'warn', 'keycloak_client_secrets_unconfigured', 'Keine pruefbaren Keycloak-Client-Secrets fuer Runtime-Doctor gefunden.');
    try {
      const results = await Promise.all(probes.map((probe) => probeOidcClientSecret(probe)));
      return deps.toDoctorCheck('keycloak-client-secrets', 'ok', 'keycloak_client_secrets_verified', 'Alle pruefbaren Keycloak-Client-Secrets authentifizieren erfolgreich gegen den Token-Endpoint.', { clients: results.map((result) => ({ mode: result.mode, name: result.name, reason: result.reason, status: result.status })) });
    } catch (error) {
  ...
  return { buildKeycloakClientSecretCheck, buildLiveRuntimeEnvCheck, buildObservabilityDoctorCheck, buildTenantAuthProofCheck } as const;
};
