...
export const createRuntimeSmokeOps = (deps: RuntimeSmokeDeps) => {
  const runInternalVerify = async (runtimeProfile: RemoteRuntimeProfile, env: NodeJS.ProcessEnv) => {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
      ...
      if (attempt < maxAttempts && shouldRetryInternalVerifyAttempt({ doctorReport, probes })) {
        await deps.wait(retryDelayMs);
        continue;
      }
      return { doctorReport, probes };
    }

    return { doctorReport: lastDoctorReport ?? (await deps.doctorRuntime(runtimeProfile, env)), probes: lastProbes };
  };

  const runExternalSmoke = async (runtimeProfile: RuntimeProfile, env: NodeJS.ProcessEnv): Promise<readonly AcceptanceProbeResult[]> => {
    const baseUrl = env.SVA_PUBLIC_BASE_URL ?? 'http://localhost:3000';
    const base = new URL(baseUrl);
    const tenantTargetResolution = await deps.resolveTenantRuntimeTargets(runtimeProfile, env, deps.shouldUseStudioReleaseBlockingTenantScope(runtimeProfile, env) ? undefined : { limit: 2 });
    const smokeTenantTargets = deps.selectSmokeTenantTargets(runtimeProfile, tenantTargetResolution.targets, { env, source: tenantTargetResolution.source });
    const tenantProbes = smokeTenantTargets.map((tenantTarget) => deps.runHttpProbe({
    ...
    return await Promise.all([
      ...
      deps.runHttpProbe({ name: 'public-auth-login', scope: 'external', target: new URL('/auth/login', baseUrl).toString(), expect: (response) => { const location = response.headers.get('location') ?? ''; return response.status !== 302 ? `Erwartet Redirect, erhalten ${response.status}.` : deps.isExpectedOidcRedirect(location, env) ? null : `OIDC-Redirect stimmt nicht: ${location}`; } }),
  ...
  const runExternalSmokeWithWarmup = async (env: NodeJS.ProcessEnv, options?: { readonly maxAttempts?: number; readonly retryDelayMs?: number; readonly runtimeProfile?: RuntimeProfile; readonly runner?: (env: NodeJS.ProcessEnv) => Promise<readonly AcceptanceProbeResult[]>; readonly shouldRetry?: (probes: readonly AcceptanceProbeResult[]) => boolean }) => {
    ...
    const runner = options?.runner ?? ((currentEnv: NodeJS.ProcessEnv) => runExternalSmoke(options?.runtimeProfile ?? deps.parseRuntimeProfile(currentEnv.SVA_RUNTIME_PROFILE as RuntimeProfile | undefined) ?? 'local-keycloak', currentEnv));
  ...
  const waitForRemoteSmokeWarmup = async (env: NodeJS.ProcessEnv, options?: { readonly maxAttempts?: number; readonly retryDelayMs?: number; readonly runtimeProfile?: RuntimeProfile; readonly runner?: (env: NodeJS.ProcessEnv) => Promise<readonly AcceptanceProbeResult[]> }) => {
    const runtimeProfile = options?.runtimeProfile ?? deps.parseRuntimeProfile(env.SVA_RUNTIME_PROFILE as RuntimeProfile | undefined) ?? 'local-keycloak';
    const blockingProbeNames = new Set(['public-live', 'public-ready', 'public-auth-login']);
    const probes = await runExternalSmokeWithWarmup(env, { maxAttempts: options?.maxAttempts, retryDelayMs: options?.retryDelayMs, runtimeProfile, runner: options?.runner, shouldRetry: (candidateProbes) => shouldRetryExternalSmoke(candidateProbes.filter((probe) => blockingProbeNames.has(probe.name) || probe.name.startsWith('public-auth-login-'))) });
    const failingProbe = probes.find((probe) => probe.status === 'error' && (blockingProbeNames.has(probe.name) || probe.name.startsWith('public-auth-login-')));
    if (failingProbe) throw new Error(`${failingProbe.name}: ${failingProbe.message}`);
    return probes;
  };

  return { runExternalSmoke, runExternalSmokeWithWarmup, runInternalVerify, waitForRemoteSmokeWarmup } as const;
};
