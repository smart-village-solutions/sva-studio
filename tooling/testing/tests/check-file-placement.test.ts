...
describe('check-file-placement', () => {
  it('returns tracked files from command output', () => {
    ...
    expect(files).toEqual(['README.md', 'packages/server-runtime/src/index.ts']);
  });
  ...
  it('recognizes generated package source artifacts', () => {
    expect(isGeneratedSourceArtifact('packages/auth-runtime/src/auth-server/session.js')).toBe(true);
    expect(isGeneratedSourceArtifact('packages/server-runtime/src/logger/index.server.d.ts')).toBe(true);
    expect(isGeneratedSourceArtifact('packages/server-runtime/dist/index.js')).toBe(false);
  ...
  it('finds generated source artifacts on disk, including ignored local files', () => {
    ...
    fs.mkdirSync(path.join(rootDir, 'packages/auth-runtime/src/auth-server'), { recursive: true });
    fs.mkdirSync(path.join(rootDir, 'packages/server-runtime/dist'), { recursive: true });
    fs.writeFileSync(path.join(rootDir, 'packages/auth-runtime/src/auth-server/session.js'), '');
    fs.writeFileSync(path.join(rootDir, 'packages/auth-runtime/src/auth-server/session.d.ts'), '');
    fs.writeFileSync(path.join(rootDir, 'packages/server-runtime/dist/index.js'), '');

    expect(findGeneratedSourceArtifacts(rootDir)).toEqual([
      'packages/auth-runtime/src/auth-server/session.d.ts',
      'packages/auth-runtime/src/auth-server/session.js',
    ]);
